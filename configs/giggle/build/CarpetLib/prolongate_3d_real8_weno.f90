...
function weno1d(q)
  ...
  REAL*8 :: weno1d
  REAL*8 :: q(5)
  REAL*8 :: zero, one, two, three, four, five, eight, ten, eleven, &
  ...
  REAL*8, dimension(0:2,0:2) :: c
  REAL*8 :: wtildesum, qmax, qmin
  REAL*8, dimension(0:2) :: d, beta, wtilde, w, vr
...
!!$     bbox(:,3) is stride
  ...
  integer ii, jj, kk
  integer d
  REAL*8, dimension(0:4,0:4) :: tmp1
  REAL*8, dimension(0:4) :: tmp2
  external weno1d
  REAL*8 weno1d
  REAL*8 half, zero
  parameter (half = 0.5)
  ...
  do d=1,3
    if (srcbbox(d,3).eq.0 .or. dstbbox(d,3).eq.0 &
      ...
      call CCTK_Warn(0,204,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error: stride is zero")
    end if
    if (srcbbox(d,3).le.regbbox(d,3) &
         .or. dstbbox(d,3).ne.regbbox(d,3)) then
      call CCTK_Warn(0,208,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error: strides disagree")
    end if
    if (mod(srcbbox(d,3), dstbbox(d,3)).ne.0) then
      call CCTK_Warn(0,211,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error: destination strides are not integer multipl&
  &es of the source strides")
    end if
    if (mod(srcbbox(d,1), srcbbox(d,3)).ne.0 &
         .or. mod(dstbbox(d,1), dstbbox(d,3)).ne.0 &
         .or. mod(regbbox(d,1), regbbox(d,3)).ne.0) then
      call CCTK_Warn(0,216,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error: array origins are not integer multiples of &
  &the strides")
    end if
    if (regbbox(d,1).gt.regbbox(d,2)) then
!!$     This could be handled, but is likely to point to an error elsewhere
      call CCTK_Warn(0,220,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error: region extent is empty")
    end if
    ...
    if (regbbox(d,1)-offsetlo.lt.srcbbox(d,1) &
      ...
      call CCTK_Warn(0,243,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error: region extent is not contained in array ext&
  &ent")
    end if
  end do
  if (srciext.ne.(srcbbox(1,2)-srcbbox(1,1))/srcbbox(1,3)+1 &
    ...
    call CCTK_Warn(0,253,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error: array sizes don't agree with bounding boxes")
  end if
...
!!$          Interpolate in all of x, y, and z
          ...
          dst (dstioff+i+1, dstjoff+j+1, dstkoff+k+1) = &
               weno1d(tmp2(0:4))
        case default
          call CCTK_Warn(0,360,"prolongate_3d_real8_weno.F90","CarpetLib", "Internal error in WENO prolongation. Should only be use&
  &d with refinement factor 2!")
        end select
      end do
    end do
  end do
