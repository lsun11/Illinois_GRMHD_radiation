 /*@@
   ...
   @enddesc
   @version  $Id: CoordBase.c,v 1.9 2008/02/20 00:18:59 schnetter Exp $
 @@*/
...
static const char *rcsid = "$Header: /cactusdevcvs/CactusBase/CoordBase/src/CoordBase.c,v 1.9 2008/02/20 00:18:59 schnetter Exp $";
...
CCTK_INT CoordBase_SystemHandle(CCTK_POINTER_TO_CONST GH, CCTK_STRING systemname)
{
                                 ...
                                 CCTK_STRING coordname)
{
  ...
  if (direction<1 || direction>system_dimension)
  {
    CCTK_VWarn(1,__LINE__,__FILE__,CCTK_THORNSTRING, "Invalid 'direction'.  "
               ...
               "dimension %d.", (int)system_dimension);
  ...
  if (coordinate_tables[direction-1]>=0)
  {
    CCTK_VWarn(1,__LINE__,__FILE__,CCTK_THORNSTRING, "Coordinate already "
               "registered for direction %d", (int)direction);
  ...
  for (i=0; i<system_dimension; ++i)
  {
    if (coordinate_tables[i]>=0)
    {
      ...
      {
        CCTK_VWarn(1,__LINE__,__FILE__,CCTK_THORNSTRING, "Coordinate '%s' "
                               ...
                               CCTK_STRING groupname)
{
  ...
  else
  {
    ...
    if(gindex<0)
      CCTK_VWarn ( 0, __LINE__, __FILE__, "CoordBase", "group index is out of bounds");
    ...
    else
    {
      ...
      if (GHex->default_coord_systems[system_dimension-1]>=0)
      {
        CCTK_VWarn(1,__LINE__,__FILE__,CCTK_THORNSTRING, "Default coordinate "
                   ...
                   "Overwriting to '%s'.", (int)system_dimension, systemname);
