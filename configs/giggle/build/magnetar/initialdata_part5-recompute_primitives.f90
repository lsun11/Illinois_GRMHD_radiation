...
!--------------------------------------------------------
subroutine magnetar_recompute_primitives(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_&
  ...
  if (enable_primitives_disk == 0) then
     if(primitives_solver==1) then
        ...
        call primitive_vars_hybrid2(ext,cctk_nghostzones,X,Y,Z, &
             ...
             rho_fail_max_step,M_fail_step,magnetar_rho_star_max, &
             Bx,By,Bz,Ex,Ey,Ez, &
             vx,vy,vz, &
...
!!$
!!$     call CCTK_ReductionHandle(handle,sum)
!!$     call CCTK_VarIndex(vindex,disk::Nfont_l)
!!$     call CCTK_ReduceLocScalar(ierr,cctkGH,-1,handle,Nfont_l,Nfont,102)
!!$
...
!!$          excision_zone_gf,tau_stildefix_enable,tau_atm)
!!$  end if
  if(1==0) then
...
!!$  where(gyz.eq.0.D0) gyz = -1.D-15*gxx
  write(*,*) "tau(1,1,1):",tau(1,1,1)
  write(*,*) "sbt(1,1,1):",sbt(1,1,1)
end subroutine magnetar_recompute_primitives
