...
!------------------------------------
subroutine bbh_cookpfeiffer_rot_diagnostics(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cc&
  ...
  &,zeta_rad_nux, X0coordinates,X0mask,X0radii,X0sf_radius,X0space_mask_group,X1coordinates,X1mask,X1sf_radius,X1space_mask_group,X&
  &2coordinates,X2mask,X2space_mask_group,active_length,num_levels_length,positions_length,radii_length,sf_active_length,sf_coordin&
  &ate_descriptors_length,sf_info_length,sf_maxreflevel_length,sf_minreflevel_length,sf_origin_length,sf_radius_length,sf_shape_des&
  &criptors_length,sf_valid_length,active,coarse_dx,coarse_dy,coarse_dz,emask,num_levels,position_x,position_y,position_z,r,radius,&
  &sf_active,sf_area,sf_centroid_x,sf_centroid_y,sf_centroid_z,sf_delta_phi,sf_delta_theta,sf_max_radius,sf_max_x,sf_max_y,sf_max_z&
  &,sf_maxreflevel,sf_mean_radius,sf_min_radius,sf_min_x,sf_min_y,sf_min_z,sf_minreflevel,sf_nghostsphi,sf_nghoststheta,sf_nphi,sf_&
  &ntheta,sf_origin_phi,sf_origin_theta,sf_origin_x,sf_origin_y,sf_origin_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_xz,sf_q&
  &uadrupole_yy,sf_quadrupole_yz,sf_quadrupole_zz,sf_radius,sf_valid,space_mask,x,y,z)
  implicit none
  INTEGER cctk_dim
  ...
  real*8  :: phi_rot_ang
  if((cctk_iteration.ge.0) .and. (rotate_refinement_boxes.eq.1)) then
!!$     !obviously, this shouldnt be hardcoded
!!$!     Mass_adm = 2.25063034050914 
!!$     Mass_adm = 2.1
!!$     Mass_1 = Mass_adm/2.0 
!!$     Mass_2 = Mass_adm/2.0 
!!$     eta = Mass_1*Mass_2/(Mass_adm*Mass_adm) 
!!$     omega = sqrt(Mass_adm/(binary_separation*binary_separation*binary_separation))*(1.0+Mass_adm/(2.0*binary_separation)*(eta-3.
!     if (set_omega_test .eq. 1) then
!        phi_rot_ang=omega_test*cctk_time
!     else
        phi_rot_ang=binary_orb_freq*cctk_time
!     endif
     position_x(1) = xbh1_initial*cos(phi_rot_ang)
     position_y(1) = xbh1_initial*sin(phi_rot_ang)
     position_z(1) = 0.d0
     position_x(2) = xbh2_initial*cos(phi_rot_ang)
     position_y(2) = xbh2_initial*sin(phi_rot_ang)
     position_z(2) = 0.d0
     write(*,*) "NEW AMR box 1 POSITION: ",position_x(1),position_y(1)
     write(*,*) "NEW AMR box 2 POSITION: ",position_x(2),position_y(2)
  end if
  ext = cctk_lsh
  ...
  write(*,*) "INSIDE bbh_cookpfeiffer_rot diagnostics"
  ...
  call CCTK_Reduce (ierr,cctkGH, -1,handle, 1, 107,export_data(1),1,index)
  if(MOD(cctk_iteration,out_every)==0) then
     ...
     call CCTK_Reduce (ierr,cctkGH, -1,handle, 1, 107,export_data(7),1,index)
...
end subroutine bbh_cookpfeiffer_rot_diagnostics
