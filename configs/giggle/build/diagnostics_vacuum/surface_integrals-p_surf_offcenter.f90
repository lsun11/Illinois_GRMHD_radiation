...
!-----------------------------------------------------------------------------
subroutine P_surf_integral_offcenter(cctkGH,xcenter,ycenter,zcenter,output_integralx,output_integraly,output_integralz)
  implicit none
...
!  call CCTK_InterpHandle (interp_handle, uniform cartesian)
  if (interp_handle .lt. 0) then
     call CCTK_Warn(0,49,"surface_integrals-p_surf_offcenter.F90","diagnostics_vacuum","Cannot get handle for interpolation ! Forgo&
  ...
  if (param_table_handle .lt. 0) then
     call CCTK_Warn(0,56,"surface_integrals-p_surf_offcenter.F90","diagnostics_vacuum","Cannot create parameter table for interpola&
  &tor")
  endif
  coord_system_handle = -1
  call CCTK_CoordSystemHandle (coord_system_handle, "cart3d")
  if (coord_system_handle .lt. 0) then
     call CCTK_Warn(0,62,"surface_integrals-p_surf_offcenter.F90","diagnostics_vacuum","Cannot get handle for cart3d coordinate sys&
  ...
  if(ierr.ge.0) then
     ...
     do i=1,N_interp_points
        ...
        out_x=nxp*AupxxL+nyp*AupyxL+nzp*AupzxL-2./3.*nxp*trkint(i)
        out_y=nxp*AupxyL+nyp*AupyyL+nzp*AupzyL-2./3.*nyp*trkint(i)
        out_z=nxp*AupxzL+nyp*AupyzL+nzp*AupzzL-2./3.*nzp*trkint(i)
...
end subroutine P_surf_integral_offcenter
