...
!-----------------------------------------------------------------------------
subroutine J_surf_integral_offcenter_comp(cctkGH,xcenter,ycenter,zcenter,output_integralx,output_integraly,output_integralz)
  implicit none
...
!  call CCTK_InterpHandle (interp_handle, uniform cartesian)
  if (interp_handle .lt. 0) then
     call CCTK_Warn(0,44,"surface_integrals-j_surf_offcenter_comp.F90","diagnostics_vacuum","Cannot get handle for interpolation ! &
  ...
  if (param_table_handle .lt. 0) then
     call CCTK_Warn(0,51,"surface_integrals-j_surf_offcenter_comp.F90","diagnostics_vacuum","Cannot create parameter table for inte&
  &rpolator")
  endif
  coord_system_handle = -1
  call CCTK_CoordSystemHandle (coord_system_handle, "cart3d")
  if (coord_system_handle .lt. 0) then
     call CCTK_Warn(0,57,"surface_integrals-j_surf_offcenter_comp.F90","diagnostics_vacuum","Cannot get handle for cart3d coordinat&
  &e system ! Forgot to activate an implementation providing coordinates ??")
  endif
  ...
  if(ierr.ge.0) then
     ...
     do i=1,N_interp_points
        ...
        output_integralx = output_integralx + 0.125D0*Psi6int(i)*( &
          nxp*( (yinterp(i)-ycenter)*( Axzint(i)*gupxxint(i) + Ayzint(i)*gupxyint(i) + Azzint(i)*gupxzint(i) )  -&
                (zinterp(i)-zcenter)*( Axyint(i)*gupxxint(i) + Ayyint(i)*gupxyint(i) + Ayzint(i)*gupxzint(i) )) +&
          nyp*( (yinterp(i)-ycenter)*( Axzint(i)*gupxyint(i) + Ayzint(i)*gupyyint(i) + Azzint(i)*gupyzint(i) )  -&
                (zinterp(i)-zcenter)*( Axyint(i)*gupxyint(i) + Ayyint(i)*gupyyint(i) + Ayzint(i)*gupyzint(i) )) +&
          nzp*( (yinterp(i)-ycenter)*( Axzint(i)*gupxzint(i) + Ayzint(i)*gupyzint(i) + Azzint(i)*gupzzint(i) )  -&
                (zinterp(i)-zcenter)*( Axyint(i)*gupxzint(i) + Axyint(i)*gupyzint(i) + Ayzint(i)*gupzzint(i) )))
        output_integraly = output_integraly + 0.125D0*Psi6int(i)*( &
          nxp*( (zinterp(i)-zcenter)*( Axxint(i)*gupxxint(i) + Axyint(i)*gupxyint(i) + Axzint(i)*gupxzint(i) )  -&
                (xinterp(i)-xcenter)*( Axzint(i)*gupxxint(i) + Ayzint(i)*gupxyint(i) + Azzint(i)*gupxzint(i) )) +&
          nyp*( (zinterp(i)-zcenter)*( Axxint(i)*gupxyint(i) + Axyint(i)*gupyyint(i) + Axzint(i)*gupyzint(i) )  -&
                (xinterp(i)-xcenter)*( Axzint(i)*gupxyint(i) + Ayzint(i)*gupyyint(i) + Azzint(i)*gupyzint(i) )) +&
          nzp*( (zinterp(i)-zcenter)*( Axxint(i)*gupxzint(i) + Axyint(i)*gupyzint(i) + Axzint(i)*gupzzint(i) )  -&
                (xinterp(i)-xcenter)*( Axzint(i)*gupxzint(i) + Ayzint(i)*gupyzint(i) + Azzint(i)*gupzzint(i) )))
        output_integralz = output_integralz + 0.125D0*Psi6int(i)*( &
          nxp*( (xinterp(i)-xcenter)*( Axyint(i)*gupxxint(i) + Ayyint(i)*gupxyint(i) + Ayzint(i)*gupxzint(i) )  -&
                (yinterp(i)-ycenter)*( Axxint(i)*gupxxint(i) + Axyint(i)*gupxyint(i) + Axzint(i)*gupxzint(i) )) +&
          nyp*( (xinterp(i)-xcenter)*( Axyint(i)*gupxyint(i) + Ayyint(i)*gupyyint(i) + Ayzint(i)*gupyzint(i) )  -&
                (yinterp(i)-ycenter)*( Axxint(i)*gupxyint(i) + Axyint(i)*gupyyint(i) + Axzint(i)*gupyzint(i) )) +&
          nzp*( (xinterp(i)-xcenter)*( Axyint(i)*gupxzint(i) + Ayyint(i)*gupyzint(i) + Ayzint(i)*gupzzint(i) )  -&
                (yinterp(i)-ycenter)*( Axxint(i)*gupxzint(i) + Axyint(i)*gupyzint(i) + Axzint(i)*gupzzint(i) )))
     end do
...
end subroutine J_surf_integral_offcenter_comp
