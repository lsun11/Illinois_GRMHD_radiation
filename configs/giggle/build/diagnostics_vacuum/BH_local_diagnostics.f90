...
subroutine interpolation_for_BH_local_diagnostics(cctkGH, xinterp,yinterp,zinterp, &
  ...
  integer*8,dimension(N_output_arrays)   :: output_array_pointers
...
!
  ...
  if (interp_handle .lt. 0) then
     call CCTK_Warn(0,213,"BH_local_diagnostics.F90","diagnostics_vacuum","Cannot get handle for interpolation ! Forgot to activate&
  & an implementation providing interpolation operators ??")
  endif
  ...
  if (param_table_handle .lt. 0) then
     call CCTK_Warn(0,220,"BH_local_diagnostics.F90","diagnostics_vacuum","Cannot create parameter table for interpolator")
  endif
  coord_system_handle = -1
  call CCTK_CoordSystemHandle (coord_system_handle, "cart3d")
  if (coord_system_handle .lt. 0) then
     call CCTK_Warn(0,226,"BH_local_diagnostics.F90","diagnostics_vacuum","Cannot get handle for cart3d coordinate system ! Forgot &
  &to activate an implementation providing coordinates ??")
  endif
...
! Now Killing transport xi_a out of the equator along lines of constant phi
  ...
  do j=1,N_phi
    ...
    ! xi(1) = xi_theta, xi(2) = xi_phi, xi(3) = L
     ...
     xi_phi(ind) = xiout(2)
     h = dcostheta
    ! Killing transport to other mus
     do i=N_theta-1,1,-1
        xi(1) = xiout(1)
        xi(2) = xiout(2)
        xi(3) = xiout(3)
...
subroutine compute_axial_Killing_vector_eq(xi_phi,quppp,dphi,N_theta,N_phi,ntot)
  ...
  real*8 :: sqppL,C,dphi
  real*8, parameter :: f1o2pi=0.15915494309189533577d0
!
  C = 0.d0
  do j=1,N_phi
