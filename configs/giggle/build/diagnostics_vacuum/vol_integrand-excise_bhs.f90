subroutine excise_bhs_VolInt(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_time,c&
  ...
  &X0coordinates,X0mask,X0space_mask_group,X1coordinates,X1mask,X1space_mask_group,X2coordinates,X2mask,X2space_mask_group,coarse_d&
  &x,coarse_dy,coarse_dz,emask,r,space_mask,x,y,z)
  implicit none
  !~~~~~> Input parameters:
  INTEGER cctk_dim
  ...
  !~~~~~> Set up integration
  ! IGNORE THE FOLLOWING COMMENT.  WE DIDNT USE THE PRIVATE() BLOCK IN THE OMP DIRECTIVE WHEN THAT COMMENT WAS WRITTEN, AND THATS WH
  ...
  !!    youll get the wrong values for HC, MC, etc violations.
  ...
  do k = 1,cctk_lsh(3)
     do j = 1,cctk_lsh(2)
        do i = 1,cctk_lsh(1)
           ...
           ! if were inside either BH, set the volume integrand to zero
