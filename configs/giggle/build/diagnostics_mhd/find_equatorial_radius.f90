...
!  xradiusvalue = abs(xradiusvalue)
  ...
  call CCTK_ReduceLocalScalar(ierr,cctkGH,-1,handle,xradiusvalue,finalxradiusvalue,107)
  if(j .gt. 0 .and. abs(Z(2,2,k)-dZ*0.5).lt.dZ*1.D-8) then
     do i=1,ext(1)
  ...
  call CCTK_ReduceLocalScalar(ierr,cctkGH,-1,handle,finalvalue0,finalvalue,107)
  call CCTK_ReduceLocalScalar(ierr,cctkGH,-1,handle,ivalue,oivalue,102)
  if(myproc .eq. 0) then
