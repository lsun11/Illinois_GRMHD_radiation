...
!-----------------------------------------------------------------------------
subroutine minternal_hybrid_4surf_integrand(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cc&
  ...
  WhichIntegral = 117
  write(*,*) "vol_integrand-minternal_hybrid_4surf.F90"
           ...
           do i = cctk_nghostzones(1)+1,cctk_lsh(1)-cctk_nghostzones(1)
             rr1 = sqrt((X(i,j,k)-xbh1)*(X(i,j,k)-xbh1)+&
              ...
              if ((rr1 .lt. rah1_initial) .or. (rr2 .lt. rah2_initial)) then
                    ...
                    VolIntegrand2(i,j,k) = rho_star(i,j,k)*eps
              else if (rr .lt. 25.d0) then
                    VolIntegrand3(i,j,k) = rho_star(i,j,k)*eps
              else if (rr .lt. 40.d0) then
                    VolIntegrand4(i,j,k) = rho_star(i,j,k)*eps
              endif
           end do
        end do
     end do
...
end subroutine minternal_hybrid_4surf_integrand
