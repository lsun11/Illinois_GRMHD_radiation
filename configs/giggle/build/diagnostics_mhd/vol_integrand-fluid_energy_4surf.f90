...
!----------------------------
subroutine fluid_energy_4surf_integrand(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_d&
  ...
  &a_rad_nux, X0coordinates,X0mask,X0space_mask_group,X1coordinates,X1mask,X1space_mask_group,X2coordinates,X2mask,X2space_mask_gro&
  &up,coarse_dx,coarse_dy,coarse_dz,emask,r,space_mask,x,y,z)
  implicit none
  !~~~~~> Input parameters:
  INTEGER cctk_dim
  ...
  parameter(AXISYM = 4)
  WhichIntegral = 115
  write(*,*) "vol_integrand-fluid_energy_4surf.F90"
        ...
        do j = cctk_nghostzones(2)+1,cctk_lsh(2)-cctk_nghostzones(2)
           do i = cctk_nghostzones(1)+1,cctk_lsh(1)-cctk_nghostzones(1)
              ...
              if (((rr1 .lt. rah1_initial) .or. (rr2 .lt. rah2_initial)) ) then
                 VolIntegrand(i,j,k) = rho_star(i,j,k)*h(i,j,k)*u_0+alp*psi6*P(i,j,k)
              else if (rr .lt. 10.d0) then
                 VolIntegrand2(i,j,k) = rho_star(i,j,k)*h(i,j,k)*u_0+alp*psi6*P(i,j,k)
              else if (rr .lt. 25.d0) then
                 VolIntegrand3(i,j,k) = rho_star(i,j,k)*h(i,j,k)*u_0+alp*psi6*P(i,j,k)
              else if (rr .lt. 40.d0) then
                 VolIntegrand4(i,j,k) = rho_star(i,j,k)*h(i,j,k)*u_0+alp*psi6*P(i,j,k)
              endif
           end do
        end do
     end do
...
end subroutine fluid_energy_4surf_integrand
