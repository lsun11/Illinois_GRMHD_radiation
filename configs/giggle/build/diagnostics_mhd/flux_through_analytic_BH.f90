subroutine flux_through_analytic_BH(cctkGH,F_E_fluid,F_E_em,F_M0,F_J_fluid,F_J_em,&
     omega,xbh_initial, &
     ah_rad,t,N_theta,N_phi,Symmetry)
  implicit none
  ...
  real*8 :: omega,t,xbh_initial,ah_rad
  ...
  real*8 :: xh_dot,yh_dot,zh_dot
  integer :: i,j, interp_order,n,ntot
  integer :: ind0,ind1,ind2,indm1,indm2,vindex
  integer :: N_theta,N_phi, Symmetry
  ...
  if (Symmetry .ne. EQUATORIAL .and. Symmetry .ne. NO_SYMM) then
     write(*,*) 'Symmetry not supported in M0_flux_through_analytic_BH'
     stop
  end if
  ntot = N_theta*N_phi
  ! allocate memory
  ...
  !for a analytically rotated binary
  if (abs(xbh_initial) .gt. 1.d-12) then
     xh = xbh_initial * cos(omega*t)
     yh = xbh_initial * sin(omega*t)
     zh = 0.d0
     xh_dot = -xbh_initial * omega * sin(omega*t)
     yh_dot = xbh_initial * omega * cos(omega*t)
     zh_dot =  0.d0
     !for a stationary metric with one BH at origin (i.e. Kerr)
  else
     xh = 0.d0
     yh = 0.d0
     zh = 0.d0
...
end subroutine flux_through_analytic_BH
