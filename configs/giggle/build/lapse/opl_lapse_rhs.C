...
extern "C" void CCTK_FCALL opl_lapse_rhs_
  (const cGH **cctkGH,int *nghostzones,int *cctk_lsh,int *Symmetry,int *alap, int *f_of_alpha,
...
extern "C" void opl_lapse_rhs(const cGH *cctkGH,int *nghostzones,int *cctk_lsh,int Symmetry,int alap, int f_of_alpha,
			      double *r,double *PhysicalRadius,double *RadiusDerivative,
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++) for(int j=jstart;j<jend;j++) for(int i=istart;i<iend;i++) {
    ...
    else if (f_of_alpha==1){
      lapse_rhsL = - 2.0 * ( (lapse_oldL + 1.0) * (lapse_oldL + 1.0) * trKL);
    }
...
extern "C" void CCTK_FCALL opl_lapse_rhs_
  (const cGH **cctkGH,int *nghostzones,int *cctk_lsh, int *Symmetry, int *alap, int *f_of_alpha,
   ...
   double *lapse_old, double *lapse_rhs,double *phi, double *trK,double *dx,double *dy,double *dz)
{
  opl_lapse_rhs(*cctkGH,nghostzones,cctk_lsh,*Symmetry,*alap,*f_of_alpha,
