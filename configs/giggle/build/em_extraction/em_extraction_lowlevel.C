...
extern "C" void CCTK_FCALL em_extraction_lowlevel_
  (const cGH **cctkGH,int *cctk_lsh,int *cctk_nghostzones,
   ...
   double *phi0re,double *phi0im,double *phi2re,double *phi2im,
   double *sbt,double *sbx,double *sby,double *sbz,double *vx,double *vy,double *vz,
   double *u0,double *SPx,double *SPy,double *SPz,double *T_0x,double *T_0y,double *T_0z);
  ...
  //  for(k=kstart;k<kend;k++) for(j=jstart;j<jend;j++) for(i=istart;i<iend;i++){ 
	...
	if(r[index] > compute_phi2_min_radius && r[index] < compute_phi2_max_radius) {
	  ...
	  */
	  ...
	  //	  if( xL*yL != 0.){
	    ...
	    */
	    ...
	    //	  else{
	    if( xL*yL == 0.){
	      //     Set values to zero on the z-axis
	  ...
	  // =======================================================================================
	  //          Compute T_0i components of the electromagnetic stress-tensor
	  // =======================================================================================
	  ...
	  */
	  ...
	  // Compute b^\mu
	  double lapseconst = 1./(sqrt(4.*PI)*(lapse));
	  double sbtL = sbt[index] * lapseconst;
	  double sbxL = sbx[index] * lapseconst;
	  double sbyL = sby[index] * lapseconst;
	  double sbzL = sbz[index] * lapseconst;

	  // Compute b_\mu = g_\mu\nu b^\nu
	  
	  double sb_tL = g4tt*sbtL + shift_xL*sbxL + shift_yL*sbyL + shift_zL*sbzL;
	  double sb_xL = shift_xL*sbtL + gxxL*sbxL + gxyL*sbyL + gxzL*sbzL;
	  double sb_yL = shift_yL*sbtL + gxyL*sbxL + gyyL*sbyL + gyzL*sbzL;
	  double sb_zL = shift_zL*sbtL + gxzL*sbxL + gyzL*sbyL + gzzL*sbzL;

	  // Compute b^2 = beta_\mu beta^mu

	  double b2 = sb_tL*sbtL + sb_xL*sbxL + sb_yL*sbyL + sb_zL*sbzL;
	  ...
	  //Compute u_0 = g_0\nu u^nu
	  double u_0L = g4tt*u0L + shift_xL*uxL + shift_yL*uyL + shift_zL*uzL;

	  // Compute T_0i components of the electromagnetic stress-tensor
	  
	  T_0x[index] = b2*u_0L*uxL - sb_tL*sbxL;
	  T_0y[index] = b2*u_0L*uyL - sb_tL*sbyL;
	  T_0z[index] = b2*u_0L*uzL - sb_tL*sbzL;


	  // =======================================================================================
	  //   Compute S^i = (E x B)^i = epsilon_{ijk} E^j B^k components of the Poynting vector
	  // =======================================================================================

	  // Compute E_i
	  double E_xL = gxxL*ExL + gxyL*EyL + gxzL*EzL;
	  double E_yL = gxyL*ExL + gyyL*EyL + gyzL*EzL;
	  double E_zL = gxzL*ExL + gyzL*EyL + gzzL*EzL;

	  SPx[index] = (E_yL*B_zL - E_zL*B_yL)/sqrt(detgam);
	  SPy[index] = (-E_xL*B_zL + E_zL*B_xL)/sqrt(detgam);
	  SPz[index] = (E_xL*B_yL - E_yL*B_xL)/sqrt(detgam);
