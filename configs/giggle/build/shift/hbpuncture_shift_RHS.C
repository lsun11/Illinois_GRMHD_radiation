...
void shift2_get_ah_radius_in_dirn(const double &horizdirn_x,const double &horizdirn_y,const double &horizdirn_z,double &output_radius) {
...
extern "C" void CCTK_FCALL hbpuncture_shiftrhs_
...
#include "../../GenFD_decl_set_varCPP.h"
  ...
  if(eta_falloff_enable==1 && 1==0) {
    double desired_radius=0.43;
    const double horizdirn_x=0.0,horizdirn_y=0.0,horizdirn_z=1000.0;
    double out_radius;
    //int horizon_number = 1;
    //int N_points = 1;
    //HorizonRadiusInDirection(horizon_number,N_points,horizdirn_x, horizdirn_y, horizdirn_z,out_radius);
    shift2_get_ah_radius_in_dirn(horizdirn_x, horizdirn_y, horizdirn_z,out_radius);

    printf("Hi inside shift... horizradius = %e\n",out_radius);

    if(out_radius>0.0) {
      //eta += 1.25*erf(100.0*(desired_radius-out_radius));
      
      if(out_radius<desired_radius) {
	eta+=0.5;
      }
      if(out_radius>desired_radius) {
	eta-=0.5;
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++)
	...
	*/
	...
	/*
	// ZACH SAYS: ETA HERE IS TESTING ONLY:
	shiftx_rhs[index] = 0.75*Gammax[index] - eta*shiftx_oldL;
	shifty_rhs[index] = 0.75*Gammay[index] - eta*shifty_oldL;
	shiftz_rhs[index] = 0.75*Gammaz[index] - eta*shiftz_oldL;
