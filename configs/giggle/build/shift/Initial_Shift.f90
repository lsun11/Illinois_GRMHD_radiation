...
!-----------------------------------------------------------------------------
subroutine Setup_Initial_Shift(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_time&
  ...
  if (Spatial_Gauge.eq.1.or.Spatial_Gauge.eq.5 .or. Spatial_Gauge.eq.7) then
     shiftxt = ZERO
     shiftyt = ZERO
     shiftzt = ZERO
     shiftxt_p = ZERO
  ...
  if(Spatial_Gauge.eq.7 .and. (hbpunc_advect_enable.ne.0) ) then
     write(*,*) "SIMPLE HBPUNCTURE SHIFT (Spatial_Gauge==7) DOES NOT SUPPORT hbpunc_advect_enable.ne.0!"
     write(*,*) ""
     write(*,*) "By three methods we may learn wisdom: First, by reflection, which is noblest; Second, by imitation, which is easie&
  &st; and third by experience, which is the bitterest."
     write(*,*) ""
     write(*,*) "-Confucius"
     stop
  end if
