...
subroutine emfields_bc_Ai(ext,fake_ext, X,Y,Z, Ax,Ay,Az, Ax_new, Ay_new, Az_new, Symmetry, &
  ...
  if(bc==FREEZE) then
    ...
    stop
     ...
     ! We do linear extrapolation here, since we apply copy boundary conditions on Bis,
     !  and we need curl Ai = Bi.  
     ! In other words, if we just used plain copy bcs on A, curl Ai=0 at the boundary, 
