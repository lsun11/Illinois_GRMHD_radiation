...
!---------------------------------------------------------------------
subroutine hyperbolic_divergence_cleaning(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk&
  ...
  real*8                :: dX,dY,dZ,c_h,c_p,Blagrangemultiplierx,Blagrangemultipliery,Blagrangemultiplierz,Bxx,Byy,Bzz,KO_Strength,&
  &r_from_BH
...
real*8 :: dxi,dyi,dzi,dxi2,dyi2,dzi2,hdxi,hdyi,hdzi,hdxi2,hdyi2,hdzi2
  ext = cctk_lsh
...
hdzi2 = 0.5*dzi*dzi;
  ...
  ! Cant OpenMP this loop, since c_h and c_p are local variables!  TODO: rewrite this routine in C++.
  do k=cctk_nghostzones(3),ext(3)-cctk_nghostzones(3)
     do j=cctk_nghostzones(2),ext(2)-cctk_nghostzones(2)
        do i=cctk_nghostzones(1),ext(1)-cctk_nghostzones(1)
           ! default = 1.0, higher and youll need to lower the Courant factor
           c_h = c_h_default
           ...
           if(hyperbolic_divergence_cleaning_centered_differencing==1) then
              Blagrangemultiplierx = ((Blagrangemultiplier(i+1,j,k)          - Blagrangemultiplier(i-1,j,k)) * hdxi)
              Blagrangemultipliery = ((Blagrangemultiplier(i,j+1,k)          - Blagrangemultiplier(i,j-1,k)) * hdyi)
              Blagrangemultiplierz = ((Blagrangemultiplier(i,j,k+1)          - Blagrangemultiplier(i,j,k-1)) * hdzi)
              Bxtilde_or_Ax_rhs(i,j,k) = Bxtilde_or_Ax_rhs(i,j,k) - &
                   exp(4.D0*phi(i,j,k))*( gupxx(i,j,k)*Blagrangemultiplierx + gupxy(i,j,k)*Blagrangemultipliery + gupxz(i,j,k)*Blag&
  &rangemultiplierz)
              Bytilde_or_Ay_rhs(i,j,k) = Bytilde_or_Ay_rhs(i,j,k) - &
                   exp(4.D0*phi(i,j,k))*( gupxy(i,j,k)*Blagrangemultiplierx + gupyy(i,j,k)*Blagrangemultipliery + gupyz(i,j,k)*Blag&
  &rangemultiplierz)
              Bztilde_or_Az_rhs(i,j,k) = Bztilde_or_Az_rhs(i,j,k) - &
                   exp(4.D0*phi(i,j,k))*( gupxz(i,j,k)*Blagrangemultiplierx + gupyz(i,j,k)*Blagrangemultipliery + gupzz(i,j,k)*Blag&
  &rangemultiplierz)
              Bxx = ((Bxtilde(i+1,j,k)          - Bxtilde(i-1,j,k)) * hdxi)
              Byy = ((Bytilde(i,j+1,k)          - Bytilde(i,j-1,k)) * hdyi)
              Bzz = ((Bztilde(i,j,k+1)          - Bztilde(i,j,k-1)) * hdzi)
     ...
     do j = cctk_nghostzones(2), ext(2)-cctk_nghostzones(2)
        do i = cctk_nghostzones(1), ext(1)-cctk_nghostzones(1)
           Blagrangemultiplier_rhs(i,j,k) = Blagrangemultiplier_rhs(i,j,k) - KO_Strength / 16.D0 &
                * (+ (Blagrangemultiplier(i-2,j,k) - 4.D0*Blagrangemultiplier(i-1,j,k) + 6.D0*Blagrangemultiplier(i,j,k) - 4.D0*Bla&
  &grangemultiplier(i+1,j,k) + Blagrangemultiplier(i+2,j,k)) / dx &
                + (Blagrangemultiplier(i,j-2,k) - 4.D0*Blagrangemultiplier(i,j-1,k) + 6.D0*Blagrangemultiplier(i,j,k) - 4.D0*Blagra&
  &ngemultiplier(i,j+1,k) + Blagrangemultiplier(i,j+2,k)) / dy &
                + (Blagrangemultiplier(i,j,k-2) - 4.D0*Blagrangemultiplier(i,j,k-1) + 6.D0*Blagrangemultiplier(i,j,k) - 4.D0*Blagra&
  &ngemultiplier(i,j,k+1) + Blagrangemultiplier(i,j,k+2)) / dz)
           if(num_BHs.ge.1) then
              if((sqrt((x(i,j,k)-bh_posn_x(1))**2 + (y(i,j,k)-bh_posn_y(1))**2 + (z(i,j,k)-bh_posn_z(1))**2) .lt. min_BH_radius).or&
  &. &
                   (num_BHs.eq.2 .and. sqrt((x(i,j,k)-bh_posn_x(2))**2 + (y(i,j,k)-bh_posn_y(2))**2 + (z(i,j,k)-bh_posn_z(2))**2) .&
  &lt. min_BH_radius)) then
                 Bxtilde_or_Ax_rhs(i,j,k) = Bxtilde_or_Ax_rhs(i,j,k) - 1.D0 / 16.D0 &
                      * (+ (Bxtilde(i-2,j,k) - 4.D0*Bxtilde(i-1,j,k) + 6.D0*Bxtilde(i,j,k) - 4.D0*Bxtilde(i+1,j,k) + Bxtilde(i+2,j,&
  &k)) / dx &
                      + (Bxtilde(i,j-2,k) - 4.D0*Bxtilde(i,j-1,k) + 6.D0*Bxtilde(i,j,k) - 4.D0*Bxtilde(i,j+1,k) + Bxtilde(i,j+2,k))&
  & / dy &
                      + (Bxtilde(i,j,k-2) - 4.D0*Bxtilde(i,j,k-1) + 6.D0*Bxtilde(i,j,k) - 4.D0*Bxtilde(i,j,k+1) + Bxtilde(i,j,k+2))&
  & / dz)
                 Bytilde_or_Ay_rhs(i,j,k) = Bytilde_or_Ay_rhs(i,j,k) - 1.D0 / 16.D0 &
                      * (+ (Bytilde(i-2,j,k) - 4.D0*Bytilde(i-1,j,k) + 6.D0*Bytilde(i,j,k) - 4.D0*Bytilde(i+1,j,k) + Bytilde(i+2,j,&
  &k)) / dx &
                      + (Bytilde(i,j-2,k) - 4.D0*Bytilde(i,j-1,k) + 6.D0*Bytilde(i,j,k) - 4.D0*Bytilde(i,j+1,k) + Bytilde(i,j+2,k))&
  & / dy &
                      + (Bytilde(i,j,k-2) - 4.D0*Bytilde(i,j,k-1) + 6.D0*Bytilde(i,j,k) - 4.D0*Bytilde(i,j,k+1) + Bytilde(i,j,k+2))&
  & / dz)
                 Bztilde_or_Az_rhs(i,j,k) = Bztilde_or_Az_rhs(i,j,k) - 1.D0 / 16.D0 &
                      * (+ (Bztilde(i-2,j,k) - 4.D0*Bztilde(i-1,j,k) + 6.D0*Bztilde(i,j,k) - 4.D0*Bztilde(i+1,j,k) + Bztilde(i+2,j,&
  &k)) / dx &
                      + (Bztilde(i,j-2,k) - 4.D0*Bztilde(i,j-1,k) + 6.D0*Bztilde(i,j,k) - 4.D0*Bztilde(i,j+1,k) + Bztilde(i,j+2,k))&
  & / dy &
                      + (Bztilde(i,j,k-2) - 4.D0*Bztilde(i,j,k-1) + 6.D0*Bztilde(i,j,k) - 4.D0*Bztilde(i,j,k+1) + Bztilde(i,j,k+2))&
  & / dz)
              end if
              if(num_BHs.gt.2) then
                 write(*,*) "SORRY, Hyperbolic Divergence Cleaning DOES NOT CURRENTLY SUPPORT num_BHs.gt.2"
