...
!!$#include GenericFD.h
!
subroutine mhd_primitives_and_bcs_v2(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delt&
...
! calcualte CoM for tempearture computation.
 if (Box1denom_VolInt == 0.0) then
  ...
  &s_thermal(i,j,k), P_thermal(i,j,k)
              write(*,*) "tau(i,j,k), mhd_st_x(i,j,k), mhd_st_y(i,j,k), mhd_st_z(i,j,k), u0(i,j,k)=", tau(i,j,k), mhd_st_x(i,j,k), &
  &mhd_st_y(i,j,k), mhd_st_z(i,j,k), u0(i,j,k)
     ...
     do j=1,ext(2)
        do i=1,ext(1)
           if (isnan(tau(i,j,k)) .or. isnan(mhd_st_x(i,j,k))) then
              ...
              write(*,*) "i,j,k =", i,j,k
              write(*,*) "tau_rhs, mhd_st_x_rhs, mhd_st_y_rhs, mhd_st_z_rhs", tau_rhs(i,j,k), mhd_st_x_rhs(i,j,k), mhd_st_y_rhs(i,j&
  &,k), mhd_st_z_rhs(i,j,k)
              write(*,*) "rho_b(i,j,k), P(i,j,k), vx(i,j,k), vy(i,j,k), vz(i,j,k), u0(i,j,k)=", rho_b(i,j,k), P(i,j,k), vx(i,j,k), &
  &u0(i,j,k)
              write(*,*) "lapm1(i,j,k), shiftx(i,j,k), E_rad(i,j,k), F_radx(i,j,k), tau_rad(i,j,k), h(i,j,k)", lapm1(i,j,k), shiftx&
  &(i,j,k), E_rad(i,j,k),F_radx(i,j,k)              , tau_rad(i,j,k), h(i,j,k)
              write(*,*) "eps_cld(i,j,k), eps_thermal(i,j,k), eps_tot(i,j,k), P_cld(i,j,k), P_thermal(i,j,k), T_fluid(i,j,k)=", eps&
  &_cld(i,j,k), eps_thermal(i,j,k), eps_tot(i,j,k), P_cld(i,j,k), P_thermal(i,j,k),T_fluid(i,j,k)
           end if
        end do
     end do
  end do
...
!!! Evolve other specices of neutrino if microphysics_scheme == 1
     if(microphysics_scheme==1) then
             write(*,*) "Inside driver_primitives_and_bcs_v2.F90, microphysics_scheme, rad_evolve_enable, and rad_closure_scheme=",&
  & microphysics_scheme, rad_evolve_enable, rad_closure_scheme
        call  primitive_vars_rad_cpp(ext,cctk_nghostzones, X, Y, Z, &
             rho_star, &
                 ...
                 do j=1,cctk_lsh(2)
                    do i=1,cctk_lsh(1)
                       if(isnan(tau_rad(i,j,k)).or.isnan(S_rad_x(i,j,k))) then
                          write(*,*) "In driver_primitives_and_bcs_v2, before primitive_vars_hybrid2_cpp, tau_rad and S_rad_x=", ta&
  &u_rad(i,j,k), S_rad_x(i,j,k)
                          write(*,*) "E_rad, F_radx, chi_rad, u0, vx=", E_rad(i,j,k), F_radx(i,j,k), chi_rad(i,j,k), u0(i,j,k), vx(&
  &i,j,k)
                 ...
                 do j=1,cctk_lsh(2)
                    do i=1,cctk_lsh(1)
                       if(isnan(tau_rad(i,j,k)).or.isnan(S_rad_x(i,j,k))) then
                          write(*,*) "In driver_primitives_and_bcs_v2, before primitive_vars_hybrid2_cpp, tau_rad and S_rad_x=", ta&
  &u_rad(i,j,k), S_rad_x(i,j,k)
                          write(*,*) "E_rad, F_radx, P_radxx, chi_rad, u0, vx=", E_rad(i,j,k), F_radx(i,j,k), P_radxx(i,j,k), chi_r&
  &ad(i,j,k), u0(i,j,k),vx(i,j,k)
                          write(*,*) "X,Y,Z=", X(i,j,k), Y(i,j,k), Z(i,j,k)
                       end if
                    end do
                 end do
...
!           call CartSymGN(dummy,cctkGH,mhd_evolve::temperatures)
             ...
             thisproc_have_global_bdry_min,thisproc_have_global_bdry_max,Symmetry)
              do k=1,cctk_lsh(3)
                 do j=1,cctk_lsh(2)
                    do i=1,cctk_lsh(1)
                       if(isnan(tau_rad(i,j,k)).or.isnan(S_rad_x(i,j,k))) then
                          write(*,*) "In driver_primitives_and_bcs_v2, before apply_matter_bc_mhd_new, tau_rad and S_rad_x=", tau_r&
  &ad(i,j,k), S_rad_x(i,j,k)
                          write(*,*) "E_rad, F_radx, P_radxx, chi_rad, u0, vx=", E_rad(i,j,k), F_radx(i,j,k), P_radxx(i,j,k), chi_r&
  &ad(i,j,k), u0(i,j,k),vx(i,j,k)
                          write(*,*) "X,Y,Z=", X(i,j,k), Y(i,j,k), Z(i,j,k)
                       end if
  ...
  & &
                            ...
                            thisproc_have_global_bdry_min,thisproc_have_global_bdry_max,compute_microphysics,enable_OS_collapse,rad&
  &_evolve_enable,rad_closure_scheme,microphysics_scheme,Erad_atm_cut)
                    end do
           else
              ...
              call matter_bc_mhd_movbh(ext,bh_posn_x(1),bh_posn_y(1),bh_posn_z(1),mhd_bigP, &
                   ...
                   thisproc_have_global_bdry_min,thisproc_have_global_bdry_max, compute_microphysics,enable_OS_collapse,rad_evolve_&
  &enable, rad_closure_scheme)
           end if
