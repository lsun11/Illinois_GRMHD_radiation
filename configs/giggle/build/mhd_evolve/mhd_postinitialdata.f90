...
!-------------------------------------------------
subroutine mhd_PostInitialData(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_time&
  ...
  write(*,*) "check: compute_microphysics, enable_OS_collapse, rad_opacity_abs = ", compute_microphysics, enable_OS_collapse, rad_o&
  &pacity_abs
  if(really_want_gamma_th_of_1_point_3==0 .and. abs(gamma_th-1.3).lt.1e-9) then
     write(*,*) "ERROR.  Default gamma_th (=1.3) detected.  You must set gamma_th to the appropriate value in your initial data tho&
  &rn, or your .par file!"
     write(*,*) ""
     write(*,*) "If you really want to use gamma_th==1.3, please set mhd_evolve::really_want_gamma_th_of_1_point_3=1 in your .par f&
  &ile!"
     stop
  end if
  ...
  !For emfields, we assume that youve set Bx, By, Bz (the UN-tilded B^is)
  ! or Ax, Ay, Az (if using constrained transport scheme of Del Zanna)
  write(*,*) "------------------inside mhd_postinitialdata.F90-------------------"
  ! Do CartSymGNs on all the MHD quantities:
...
!  write(*,*) ------------------test 3-------------------
  ...
  !$omp end parallel do
  !mhd_evolve thorn assumes that youve set the conservatives & primitives
  !$omp parallel do
  do k=1,ext(3)
     do j=1,ext(2)
        do i=1,ext(1)
     ...
     do j=1,cctk_lsh(2)
        do i=1,cctk_lsh(1)
           if(isnan(tau_rad(i,j,k)) .or. isnan(S_rad_x(i,j,k))) then
              write(*,*) "Inside mhd_postinitialdata.F90, before metric_source_terms_and_misc_vars_initial, tau_rad, S_rad_x, E_rad&
  &, F_rad_x=", tau_rad, S_rad_x, E_rad, F_radx
           end if
        end do
     end do
  end do
