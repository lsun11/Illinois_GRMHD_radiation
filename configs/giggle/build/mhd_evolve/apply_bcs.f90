...
subroutine apply_matter_bc_mhd_new(ext, fake_ext, X,Y,Z, &
     ...
     have_bdry_min,have_bdry_max, compute_microphysics, enable_OS_collapse, rad_evolve_enable, rad_closure_scheme, microphysics_sch&
  &eme, Erad_atm_cut)
  implicit none
  ...
  real*8, dimension(ext(1),ext(2),ext(3)) :: tau_rad_nue, S_rad_x_nue, S_rad_y_nue, S_rad_z_nue, tau_rad_nux, S_rad_x_nux, S_rad_y_&
  &nux, S_rad_z_nux
  ...
  real*8, dimension(ext(1),ext(2),ext(3)) :: E_rad_nue, F_radx_nue, F_rady_nue, F_radz_nue, E_rad_nux, F_radx_nux, F_rady_nux, F_ra&
  &dz_nux
  real*8, dimension(ext(1),ext(2),ext(3)) :: F_rad0, P_radxx, P_radyy, P_radzz, P_radxy, P_radxz, P_radyz
  real*8, dimension(ext(1),ext(2),ext(3)) :: F_rad0_nue, P_radxx_nue, P_radyy_nue, P_radzz_nue, P_radxy_nue, P_radxz_nue, P_radyz_n&
  &ue
  real*8, dimension(ext(1),ext(2),ext(3)) :: F_rad0_nux, P_radxx_nux, P_radyy_nux, P_radzz_nux, P_radxy_nux, P_radxz_nux, P_radyz_n&
  &ux
  ...
  integer                                 :: Symmetry
  integer                                 :: bc
  integer                                 :: neos,ergo_star,enable_OS_collapse,rad_evolve_enable,rad_closure_scheme,microphysics_sc&
  &heme,compute_microphysics
...
! Storage variables:
  ...
  real*8                             :: E_rad_nuel,F_radx_nuel,F_rady_nuel,F_radz_nuel,E_rad_nuxl,F_radx_nuxl,F_rady_nuxl,F_radz_nu&
  &xl
  real*8                             :: F_rad0l, P_radxxl, P_radyyl, P_radzzl, P_radxyl, P_radxzl, P_radyzl
  real*8                             :: F_rad0_nuel, P_radxx_nuel, P_radyy_nuel, P_radzz_nuel, P_radxy_nuel, P_radxz_nuel, P_radyz_&
  &nuel
  real*8                             :: F_rad0_nuxl, P_radxx_nuxl, P_radyy_nuxl, P_radzz_nuxl, P_radxy_nuxl, P_radxz_nuxl, P_radyz_&
  &nuxl
  real*8                             :: eps_totl, eps_cldl, eps_thermall, P_cldl
  real*8                             :: tau_radl, S_rad_xl, S_rad_yl, S_rad_zl, rho_Yel
  real*8                             :: tau_rad_nuel, S_rad_x_nuel, S_rad_y_nuel, S_rad_z_nuel, tau_rad_nuxl, S_rad_x_nuxl, S_rad_y&
  &_nuxl, S_rad_z_nuxl
...
!
  ...
  if(bc==FREEZE) then
    ...
    if (have_bdry_min(3)==1) then
     ...
     !write(*,*) midway bc rho_star = ,rho_b(15,15,5),P(15,15,5),vx(15,15,5),vy(15,15,5),vz(15,15,5),have_bdry_max(3)==1,proc_kmin,Z
        ...
        do k=kmin,kmax
           ...
           call recompute_vars(rho_bl,Pl,vxl,vyl,vzl,Bxl,Byl,Bzl,AUX,m,rho_sl,taul, &
                ...
                gamma_tab, gamma_th,rho_b_atm, rad_const, &
                     compute_microphysics, enable_OS_collapse,rad_evolve_enable, rad_closure_scheme, microphysics_scheme, Erad_atm_&
  &cut)
        ...
        do k=kmin,kmax
           do j=jmin,jmax
              ...
              call recompute_vars(rho_bl,Pl,vxl,vyl,vzl,Bxl,Byl,Bzl,AUX,m,rho_sl,taul, &
                   ...
                   neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab,k_tab, &
                   gamma_tab, gamma_th,rho_b_atm, rad_const,&
                            compute_microphysics, enable_OS_collapse,rad_evolve_enable, rad_closure_scheme, microphysics_scheme, Er&
  &ad_atm_cut)
              ! Assign primitive variables which may have been modified
              vx(i,j,k) = vxl
              vy(i,j,k) = vyl
        ...
        do k=kmin,kmax
           do j=jmin,jmax
              ...
              call recompute_vars(rho_bl,Pl,vxl,vyl,vzl,Bxl,Byl,Bzl,AUX,m,rho_sl,taul, &
                   ...
                   neos, ergo_star, ergo_sigma,rho_tab, P_tab, eps_tab, k_tab, &
                        gamma_tab, gamma_th,rho_b_atm, rad_const,&
                   compute_microphysics, enable_OS_collapse,rad_evolve_enable, rad_closure_scheme, microphysics_scheme, Erad_atm_cu&
  &t)
              ! Assign primitive variables which may have been modified
              vx(i,j,k) = vxl
              vy(i,j,k) = vyl
        ...
        do k=kmin,kmax
           do i=imin,imax
              ...
              call recompute_vars(rho_bl,Pl,vxl,vyl,vzl,Bxl,Byl,Bzl,AUX,m,rho_sl,taul, &
                   ...
                   neos,ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, &
                        gamma_tab, gamma_th,rho_b_atm, rad_const,&
                        compute_microphysics, enable_OS_collapse,rad_evolve_enable, rad_closure_scheme, microphysics_scheme, Erad_a&
  &tm_cut)
              ! Assign primitive variables which may have been modified
              vx(i,j,k) = vxl
              vy(i,j,k) = vyl
