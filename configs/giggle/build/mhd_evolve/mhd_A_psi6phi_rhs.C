...
extern "C" void CCTK_FCALL mhd_a_psi6phi_rhs_cpp_
...
extern "C" void mhd_A_psi6phi_rhs_cpp(const cGH *cctkGH,int *cctk_lsh, int *nghostzones, int Symmetry,
  ...
  //  printf ("Start mhd_A_psi6phi_rhs_cpp!!!! \n");
...
#pragma omp parallel for
  for(int k=0;k<kend;k++) for(int j=0;j<jend;j++) for(int i=0;i<iend;i++) {
	int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	if(isnan(Ax[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<1>> Ax is nan \n");
	if(isnan(Ay[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<1>> Ay is nan \n");
	if(isnan(Az[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<1>> A is nan \n");
      }
...
#pragma omp parallel for
  for(int k=0;k<kend;k++) for(int j=0;j<jend;j++) for(int i=0;i<iend;i++) {
	int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	if(isnan(Ax[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<2>> Ax is nan \n");
	if(isnan(Ay[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<2>> Ay is nan \n");
	if(isnan(Az[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<2>> Az is nan \n");
      }
...
#pragma omp parallel for
  for(int k=0;k<kend;k++) for(int j=0;j<jend;j++) for(int i=0;i<iend;i++) {
	int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	if(isnan(Ax[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<3>> Ax is nan \n");
	if(isnan(Ay[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<3>> Ay is nan \n");
	if(isnan(Az[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<3>> Az is nan \n");
      }
...
#pragma omp parallel for
  for(int k=0;k<kend;k++) for(int j=0;j<jend;j++) for(int i=0;i<iend;i++) {
	int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	if(isnan(Ax[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<4>> Ax is nan \n");
	if(isnan(Ay[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<4>> Ay is nan \n");
	if(isnan(Az[index])) printf("Inside mhd_A_psi6phi_rhs.C, <<4>> Az is nan \n");
      }
...
#pragma omp parallel for
  ...
  printf ("End mhd_A_psi6phi_rhs_cpp!!!! \n");
}

//extern "C" void CCTK_FCALL average_cpp_
//  (const cGH **cctkGH,int *ext,double *f,double *favg, int *m) {
