...
void newt2_cpp(double x[],struct auxarray &aux,
	       void (*function_hybrd)(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
	       void (*jacobian_hybrd)
	       (int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5], int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
	       ...
	       int *indx,double *g,double *p,double *xold,double *fvec,int &MAXITS,double &STPMX,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid);
...
*/
void function_hybrd(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid);
void function_hybrd_font_fix(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid);
void jacobian_hybrd
(int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5], int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid);
...
void lnsrch_newt(int n, double xold[], double fold, double g[], double p[], double x[],
		 ...
		 (double *x,struct auxarray &aux,double *fvec,
		  void (*nrfuncv)(int &n,double *x,double *fvec, struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
		  int &n,int &neos,int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
		  void (*nrfuncv)(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
		 struct auxarray &aux,double *fvec,
		 int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid);
double fmin_newt(double *x,struct auxarray &aux,double *fvec,
		 void (*nrfuncv)(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
		 int &n,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid);

extern "C" void CCTK_FCALL primitive_vars_hybrid2_cpp_
  (int *ext,int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau,
   double *st_x, double *st_y, double *st_z, double *mhd_st_x, double *mhd_st_y, double *mhd_st_z, double *rhoYe,
   ...
   double *E_rad, double *F_radx, double *F_rady, double *F_radz, double *F_rad0, double *F, double *Y_e, double *eps_thermal,
   ...
   double &rad_const, int &enable_OS_collapse, int &compute_microphysics, double &Psi6threshold, double &Erad_atm_cut);
...
(int *ext,int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau,
 ...
 double &rad_const, int &enable_OS_collapse, int &compute_microphysics, double &Psi6threshold, double &Erad_atm_cut) {
  ...
  printf("Inside!!!! start primitive_vars_hybrid2_cpp \n");
  printf("rad_const = %e \n", rad_const);
  //int count = 0;
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++)
	...
	double UUguess[5];
	   ...
	   // set T_fluidl the value at last step as the guess.
	   ...
	   //if (rho_s > Tiny && Psi6<50.0) {
	     ...
	     //	     if (eps < 0.1) {printf("eps < eps_cold = 0.1, eps = %.17g, h_old = %.17g, P=%.17g, rho_bl=%.17g \n", eps, h_old[index], P[index], rho_bl);} 
	     ...
	     if(isnan(AUX.tau_scal_inv)){
	       printf ("AUX.tau_scal_inv is NAN, Psi6=%e, rho_bl=%e, eps_scal=%e, B2=%e, tau[index]=%e, tau_atm=%e \n", Psi6, rho_bl, eps_scal, B2, tau[index], tau_atm);
		 }
	     ...
	     //if (T_fluidl <= 0.0) printf("before newt2_cpp, T_fluid is negtive or zero, X,Y,Z=%e,%e,%e \n", X[index],Y[index],Z[index]);
	     newt2_cpp(UU,AUX,function_hybrd,jacobian_hybrd,nn,neos,ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab,check, 
		       newt_indx,newt_g,newt_p,newt_xold,newt_fvec,MAXITS_beforefontfix,STPMX_beforefontfix, enable_OS_collapse, compute_microphysics, E_rad[index], T_fluidl);
		 ...
		 check = false;
		 newt2_cpp(UU,AUX,function_hybrd,jacobian_hybrd,nn,neos,ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab,check,
			   newt_indx,newt_g,newt_p,newt_xold,newt_fvec,MAXITS_taustildefix,STPMX_taustildefix,enable_OS_collapse, compute_microphysics, E_rad[index], T_fluidl);
	     ...
	     */
	     ...
	     //****************************************************************
	     if(check || UU[4] < 0.0) {
	       //printf("START FONT FIX!!!!!!!!!!!, check = %d, UU[4] = %e, eps = %e, eps_tiny = %e, rho_b_atm = %e, rhobatm = %e \n", check, UU[4] , eps, eps_tiny, rho_b_atm, rhobatm);
		 ...
		 //}
	       ...
	       newt2_cpp(UU_font_fix,AUX,function_hybrd_font_fix,jacobian_hybrd_font_fix,nn,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab,check,
			 newt_indx,newt_g,newt_p,newt_xold,newt_fvec,MAXITS_fontfix,STPMX_fontfix, enable_OS_collapse, compute_microphysics, E_rad[index], T_fluidl);
	       while(check) {
		 check = false;
		 newt2_cpp(UU_font_fix,AUX,function_hybrd_font_fix,jacobian_hybrd_font_fix,nn,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab,check,
			   newt_indx,newt_g,newt_p,newt_xold,newt_fvec,MAXITS_fontfix,STPMX_fontfix,enable_OS_collapse, compute_microphysics, E_rad[index], T_fluidl);
		 ...
		 if (compute_microphysics == 0){
	       ...
	       rho_bl = rho_s/(alpn1*u0[index]*AUX.Psi6);
	       compute_pcold_epscold_cpp(rho_bl, P_cold, eps_cold,neos, ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab,enable_OS_collapse);
	       eps_thermal[index] = eps - eps_cold;
	       
	       /* if (eps_thermal[index] < 0.0){
		 printf("3. eps_thermal negative!!!! eps=%.17g, eps_cold=%.17g, P_cold =%.17g, rho_b=%.17g \n", eps, eps_cold, P_cold, rho_bl);
		 printf("h=%.17g, h_old=%.17g, P_total =%.17g, rho_b=%.17g, h_old-1.0-P/rho = %.17g\n", h_l, h_old[index], P[index], rho_bl, h_old[index]-P[index]/rho_bl-1.0);
	       } 
	       */

	     }	
	     
	     //If used optically thick limit, recover the analytic here since we need radiation pressure for the total pressure.
	     if (compute_microphysics == 1){
	       // Here compute_microphysics == 0 : no microphysics                                                                                                              
	       //                           == 1 : use analysic expression in Baumgarte, Shapiro and Teukolsky 1996.                                                            
	       //                           == 2 : use table reading data. (not done)                                                                                           
               //|||| Now include thermal and nuclear parts   ||||//                                                                                                           
	       T_fluidl = T_fluid[index];

	       if (X[index]<7.75 && X[index]>6.95 && Y[index]<0.05 && Y[index]>-0.05 && Z[index]<0.05 && Z[index]>-0.05) 
		 {printf("Before compute_P_T_microphys, T_fluidl, eps, eps_cold = %e, %e, %e \n", T_fluidl, eps, eps_cold);}

	       compute_P_T_microphys(P_l, T_fluidl, P_cold, eps, eps_cold, rho_bl);

	       if (X[index]<7.75 && X[index]>6.95 && Y[index]<0.05 && Y[index]>-0.05 && Z[index]<0.05 && Z[index]>-0.05)
                 {printf("After compute_P_T_microphys, T_fluidl = %e \n", T_fluidl);}
	       /*
	       double T_fluid_cgs_check = T_fluidl * SQR(c_cgs) * m_n_cgs/kb_cgs;
	       double eps_thermal_check = eps - eps_cold;
	       double eng_thermal_check = rho_bl* eps_thermal_check;
	       double eng_thermal_cgs_check = fasterpow_prim(c_cgs,4.0) / G_cgs * eng_thermal_check * fasterpow_prim(M_sun_geo, -2.0);

	       double T_scal = 1.0e10;
	       double rho_b_cgs = fasterpow_prim(c_cgs,2.0) / G_cgs * rho_bl * fasterpow_prim(M_sun_geo, -2.0);
	       double n_nucleon = rho_b_cgs/m_n_cgs;
	       double rad_constl = rad_const_cgs;
	       double N_nu = 3.0;
	       double eng_th_minus_T = find_T(T_fluid_cgs_check, eng_thermal_cgs_check, rad_constl, n_nucleon, N_nu, T_scal);
	       double Fac_OS = 1.0;
	       double eng_th_T = rad_constl * fasterpow_prim(T_fluid_cgs_check, 4.0) + 1.5*n_nucleon*T_fluid_cgs_check*kb_cgs*Fac_OS;

	       if (X[index]<7.75 && X[index]>6.95 && Y[index]<0.05 && Y[index]>-0.05 && Z[index]<0.05 && Z[index]>-0.05)
		 { printf("T_fluid(i,j,k), T_fluidl_cgs, eng_thermal_cgs, eng_thermal, eps_thermal, eps, eng_th_minus_T, eng_th_T, %.17g, %.17g, %.17g, %.17g, %.17g, %.17g, %.17g, %.17g \n", T_fluidl, T_fluid_cgs_check, eng_thermal_cgs_check, eng_thermal_check, eps_thermal_check, eps, eng_th_minus_T, eng_th_T);
		   printf("Fac_OS, c_cgs, m_n_cgs, kb_cgs, eps_cold, rho_bl, E_rad[index]= %.17g, %.17g, %.17g, %.17g, %.17g, %.17g, %.17g \n", Fac_OS, c_cgs, m_n_cgs, kb_cgs, eps_cold, rho_bl, E_rad[index]); }*/
	     }
	     else{
	     ...
	     if (isnan(abs(ux_l))){printf("ux is nan (=%e), shiftxL=%e, AUX.gupxx_phys=%e, AUX.gupxy_phys=%e, AUX.gupxz_phys=%e, u_xl=%e, u_yl=%e, u_zl=%e u0=%e \n", ux_l, shiftxL, AUX.gupxx_phys, AUX.gupxy_phys, AUX.gupxz_phys, u_xl, u_yl, u_zl, u0[index]);}
	     ...
	     if(AUX.Psi6 > Psi6threshold) {
	       double rho_horiz_cap = 1.0e1*rhobatm;
	       if(rho_bl>= rho_horiz_cap){
		 recom = true;
		 rho_bl = rho_horiz_cap;
	     ...
	     rho_b[index] = rho_bl;
	     P[index]   = P_l;
	     //	     if (X[index]<0.05 && X[index]>-0.05 && Y[index]<0.05 && Y[index]>-0.05 && Z[index]<0.05 && Z[index]>-0.05)
	     //  { printf("T_fluid(i,j,k), T_fluidl_cgs, eng_thermal_cgs, eng_thermal, eps_thermal, eps %e, %e, %e, %e, %e, %e, \n", P[index]);}
	   ...
	   //Add radiation terms to the source IF rad_closure_scheme == 0, otherwise call primitives_rad_cpp.
	  
	   Y_e[index] = rhoYe[index]/rho_star[index];
	   ...
	   if (rad_closure_scheme==0)
		 ...
		 +( (shiftxL*u0[index]+ux_l)*S_rad_xl
	       ...
	       P_radl = E_radl/3.0;	       
	       F_radxl = ((AUX.gupxx_phys*S_rad_xl +
		 ...
		 2.0*(F_rad0l * shiftx[index]) - F_rad0l*vx[index];
	       ...
	       F_radyl = ((AUX.gupxy_phys*S_rad_xl +
		 ...
		 2.0*(F_rad0l * shifty[index]) - F_rad0l*vy[index];
	       ...
	       F_radzl = ((AUX.gupxz_phys*S_rad_xl +
		 ...
		 recom_rad = true;
	       ...
	       double F_rad_xl = Psi4 * (gxxL * F_radx[index] + gxyL * F_rady[index] + gxzL * F_radz[index]) + shift_xl*F_rad0[index];
	       double F_rad_yl = Psi4 * (gxyL * F_radx[index] + gyyL * F_rady[index] + gyzL * F_radz[index]) + shift_yl*F_rad0[index];
	       double F_rad_zl = Psi4 * (gxzL * F_radx[index] + gyzL * F_rady[index] + gzzL * F_radz[index]) + shift_zl*F_rad0[index];
	       ...
	       Sx[index] = Sx[index] + temp_rad*( ( (E_rad[index]+E_rad[index]/3.0)*u0l + F_rad0[index]) * (shift_xl + v_xl) + F_rad_xl);
	       Sy[index] = Sy[index] + temp_rad*( ( (E_rad[index]+E_rad[index]/3.0)*u0l + F_rad0[index]) * (shift_yl + v_yl) + F_rad_yl);
	       Sz[index] = Sz[index] + temp_rad*( ( (E_rad[index]+E_rad[index]/3.0)*u0l + F_rad0[index]) * (shift_zl + v_zl) + F_rad_zl);
	   ...
	   if (compute_microphysics==0){	     
	     if (enable_shocktest_primitive_mode==1) {
	       compute_T_fluid_shock_cpp2(P_l, rho_bl,  T_fluidl);
	     }
	     else if (enable_OS_collapse==1){
	       compute_T_fluid_OS(E_rad[index], rad_const, T_fluidl);
	     }
	     else{
	       T_fluidl = 0.0;
	     }	     
...
void compute_pcold_epscold_cpp(double &rhob, double &P_cold, double &eps_cold, 
  ...
  if (enable_OS_collapse == 1){
    //gamma_OS = 1.0
    P_cold = k_tab[0]*fasterpow_prim(rhob,1.0);
...
void newt2_cpp(double x[],struct auxarray &aux,
	       void (*funcv1)(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
	       void (*fdjac1)
	       (int &n,double *x,struct auxarray &aux,int &neos,int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5], int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid),
	       int &n,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,bool &check,
	       int *indx,double *g,double *p,double *xold,double *fvec,int &MAXITS,double &STPMX, int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid)
{
  ...
  f=fmin_newt(x,aux,fvec,funcv1,n,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab,enable_OS_collapse, compute_microphysics, E_rad, T_fluid);
  ...
  for (its=1;its<=MAXITS;its++) {
    int np=NP;
    fdjac1(n,x,aux,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab,fvec,np,fjac, enable_OS_collapse, compute_microphysics, E_rad, T_fluid);
    ...
    lnsrch_newt(n,xold,fold,g,p,x,&f,stpmax,check,fmin_newt,funcv1,aux,fvec,neos,ergo_star, ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab,enable_OS_collapse, compute_microphysics, E_rad, T_fluid);
		 ...
		 int &n,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid)
{
  ...
  (*nrfuncv)(n,x,fvec,aux,neos,ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab, enable_OS_collapse, compute_microphysics, E_rad, T_fluid);
		 ...
		 int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid)	    
{
  ...
  for (;;) {
    ...
    */
    *f=(*fmin)(x,aux,fvec,nrfuncv,n,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab, enable_OS_collapse, compute_microphysics, E_rad, T_fluid);
    /*
    if (x[4] < 0.0)
      {
...
void function_hybrd(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid) {
  ...
  double eps_thermal_l = eps-eps_cold;
  //  if (eps_thermal_l < 0.0){printf("1. eps_thermal negative!!!! eps=%.17g, eps_cold=%.17g, P_cold =%.17g, rho_b=%.17g \n", eps, eps_cold, P_cold, rhob);}

  if (compute_microphysics == 0){
    P = P_cold + (aux.gamma_th - 1.0)*rhob*(eps - eps_cold);
  } else if (compute_microphysics == 1){
    compute_P_T_microphys(P, T_fluid, P_cold, eps, eps_cold, rhob);
  }

  //printf("INITIAL: inside function_hybrd. u_x = %.16g, aux.mhd_st_x = %.16g, eps = %.16g, u0 = %.16g, rhob = %.16g, rho_s = %.16g, P = %.16g \n", u_x, aux.mhd_st_x, eps, u0, rhob, aux.rho_s, P);
  ...
  //printf ("CHECK inside function_hybrd, h=%e, u_x=%e, u0=%e, sb0=%e, aux.sti_scal_inv=%e, aux.tau_scal_inv= %e, fvec[2]=%e, fvec[4]=%e \n", h, u_x, u0, sb0, aux.sti_scal_inv, aux.tau_scal_inv, fvec[2], fvec[4]);
  //printf ("aux.rho_s = %e, aux.Psi6 = %e, aux.tau = %e \n", aux.rho_s, aux.Psi6, aux.tau);
...
(int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5], int &enable_OS_collapse, int &compute_microphysics, double &E_rad,  double &T_fluid) {
  ...
  //double rhob = fabs(a ux.rho_s/(aux.alpn1*aux.Psi6)*u0_inv);
  ...
  double P, dP_drho, dP_nucldrho;
  /*
  P = P_cold + (aux.gamma_th - 1.0)*rhob*(eps - eps_cold);
  dP_drho = dPcold_drho + (aux.gamma_th-1.0)*(eps-eps_cold-rhob*depscold_drho);
  */
  double eps_thermal_l = eps-eps_cold;
  //    if (eps_thermal_l < 0.0){printf("2. eps_thermal negative!!!! eps=%.17g, eps_cold=%.17g, P_cold =%.17g, rho_b=%.17g \n", eps, eps_cold, P_cold, rhob);}
  ...
  if (compute_microphysics == 0){
    ...
    compute_P_T_microphys(P, T_fluid, P_cold, eps, eps_cold, rhob);
    compute_dP_nucldrho_microphys(dP_nucldrho, T_fluid, rhob);
    // With microphyics, dP_thermaldrho = dP_nucldrho, as the radiation part is independent of density.
    dP_drho = dPcold_drho + dP_nucldrho;
  }
...
extern "C" void CCTK_FCALL primitive_vars_hybrid2_cpp_
  (int *ext,int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau,
   double *st_x, double *st_y, double *st_z, double *mhd_st_x, double *mhd_st_y, double *mhd_st_z, double *rho_Ye,
   double *tau_rad, double *S_rad_x, double *S_rad_y, double *S_rad_z,
   int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab, double *P_tab, double *eps_tab, double *k_tab, double *gamma_tab, double &gamma_th,
   double *w, double *w_old, double *rho_b, double *rho, double *P, double *h, double *Sx, double *Sy, double *Sz,
   double *Sxx, double *Sxy, double *Sxz, double *Syy, double *Syz, double *Szz,
   ...
   double &rad_const, int &enable_OS_collapse, int &compute_microphysics, double &Psi6threshold, double &Erad_atm_cut) {
     primitive_vars_hybrid2_cpp(ext,nghostzones, X, Y, Z, rho_star, tau,
			     st_x, st_y, st_z, mhd_st_x, mhd_st_y, mhd_st_z, rho_Ye,
			     ...
			     E_rad,F_radx,F_rady,F_radz,F_rad0, F, Y_e, eps_thermal,
