...
!--------------------------------------------------------------------------
subroutine reconstruction_v2(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_time,c&
...
!!$     do j=1,ext(2)
!!$        do i=1,ext(1)
!!$           if(abs(Y(i,j,k)+(dY*0.5D0)).lt.dY*0.001 .and. abs(X(i,j,k)+(dX*0.5D0)).lt.dX*0.001 .and. abs(Z(i,j,k)+(dX*9.5D0)).lt.d
!!$              write(*,*) vy:,i,j,k,vy(i,j,k),temp2(i,j,k)
!!$           end if
!!$        end do
!!$     end do
!!$  end do
  ! We must sync all the nabla_Us since Ur, Ul depend on ghostzone values:
  ...
  if(Reconstruction.ne.SPPM) then
     ...
     call compute_Ur_Ul_general(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms4,rho_b,drho_b_m,rho&
  &_br,rho_bl)
     if (reconstruct_Pthermal_instead_of_P==0) then
        call compute_Ur_Ul_general(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms5,P,dP_m,Pr,Pl)
     else
        call compute_Ur_Ul_general(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms5,P_thermal,dP_m,&
  &Pr,Pl)
     end if
  else
     ! Finally, compute Ur and Ul:
     ...
     call compute_Ur_Ul_SPPM(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms4,rho_b,drho_b_m,rho_br&
  &,rho_bl)
     if (reconstruct_Pthermal_instead_of_P==0) then
        call compute_Ur_Ul_SPPM(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms5,P,dP_m,Pr,Pl)
     else
        call compute_Ur_Ul_SPPM(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms5,P_thermal,dP_m,Pr,&
  &Pl)
     end if
  end if
  ...
  if(em_evolve_enable==1) then
     if(Reconstruction.ne.SPPM) then
        ...
        if (m==1 .and. constrained_transport_scheme==3) then
           ...
           call compute_Ur_Ul_general(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms6,Bx,dBx_m,Bxr&
  &,Bxl)
        end if
        if (m==2 .and. constrained_transport_scheme==3) then
           ...
           call compute_Ur_Ul_general(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms7,By,dBy_m,Byr&
  &,Byl)
        end if
        if (m==3 .and. constrained_transport_scheme==3) then
           ...
           call compute_Ur_Ul_general(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms8,Bz,dBz_m,Bzr&
  &,Bzl)
        end if
     else
        ! Note: We dont need to reconstruct B^m for the Del Zanna CT scheme
        if (m==1 .and. constrained_transport_scheme==3) then
           ...
           call compute_Ur_Ul_SPPM(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms6,Bx,dBx_m,Bxr,Bx&
  &l)
        end if
        if (m==2 .and. constrained_transport_scheme==3) then
           !$omp parallel do private (jm1)
           ...
           call compute_Ur_Ul_SPPM(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms7,By,dBy_m,Byr,By&
  &l)
        end if
        if (m==3 .and. constrained_transport_scheme==3) then
           !$omp parallel do private (km1)
           ...
           call compute_Ur_Ul_SPPM(m,cctkGH,cctk_lsh,cctk_nghostzones,X,Y,Z,Symmetry,cell_centering_enabled,U_syms8,Bz,dBz_m,Bzr,Bz&
  &l)
        end if
     end if
  else
...
!  end where
  ...
  ! IF PPM RECONSTRUCTION IS ENABLED, THIS FUNCTION (driver_reconstruction_v2()) IS NOT CALLED.  THEREFORE WE COMMENT THE FOLLOWING 
...
!!$  end if
  ...
  !!        Symmetry,excision_zone_gf)
  !!end if
  ! Implement PPM reconstruction.  IF PPM RECONSTRUCTION IS ENABLED, THIS FUNCTION (driver_reconstruction_v2()) IS NOT CALLED.  THER
