...
!-----------------------------------------------------------------------------
!
! Compute the flux for advecting rho_star, tau (Fonts energy variable), 
!  and S_i .
!
!-----------------------------------------------------------------------------
!
...
!  write(*,*) er,i,er(5,2,2),vxl(5,2,2),vyl(5,2,2),vzl(5,2,2)
  ...
  where (rho_br .le. rho_tab(1))
     where (rho_br .eq. 0.d0)
        ...
        hr1 = 0.d0
     elsewhere
        v02l = k_tab(1)*rho_br**gamma_tab(1)
        cplusl = v02l/rho_br/(gamma_tab(1)-1.d0)
  ...
  do nn=2,neos
     where (rho_br .le. rho_tab(nn) .and. rho_br .gt. rho_tab(nn-1))
        v02l = k_tab(nn)*rho_br**gamma_tab(nn)
...
if (ergo_star .eq. 0) then
 where (rho_br .gt. rho_tab(neos))
       v02l = k_tab(neos+1)*rho_br**gamma_tab(neos+1)
       cplusl = eps_tab(neos) + (v02l/rho_br - P_tab(neos)/rho_tab(neos))/(gamma_tab(neos+1)-1.d0)
 ...
 where (rho_br .gt. rho_tab(neos))
       v02l = ((ergo_sigma* (1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rho_br**(ergo_sigma+1) + P_tab&
  &(neos)- ergo_sigma*(1+eps_tab(neos))*rho_tab(neos))/(ergo_sigma+1)
       cplusl = (((1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rho_br**(ergo_sigma+1) -P_tab(neos) + er&
  &go_sigma*((1+eps_tab(neos))*rho_tab(neos)))/((ergo_sigma+1)*rho_br)-1
  ...
  where (rho_bl .le. rho_tab(1))
     where (rho_bl .eq. 0.d0)
        ...
        hl1 = 0.d0
     elsewhere
        v02l = k_tab(1)*rho_bl**gamma_tab(1)
        cplusl = v02l/rho_bl/(gamma_tab(1)-1.d0)
  ...
  do nn=2,neos
     where (rho_bl .le. rho_tab(nn) .and. rho_bl .gt. rho_tab(nn-1))
        v02l = k_tab(nn)*rho_bl**gamma_tab(nn)
...
if (ergo_star .eq. 0) then
 where (rho_bl .gt. rho_tab(neos))
       v02l = k_tab(neos+1)*rho_bl**gamma_tab(neos+1)
       cplusl = eps_tab(neos) + (v02l/rho_bl - P_tab(neos)/rho_tab(neos))/(gamma_tab(neos+1)-1.d0)
 ...
 where (rho_bl .gt. rho_tab(neos))
       v02l = ((ergo_sigma* (1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rho_bl**(ergo_sigma+1) + P_tab&
  &(neos) - ergo_sigma*(1+eps_tab(neos))*rho_tab(neos))/(ergo_sigma+1)
       cplusl = (((1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rho_bl**(ergo_sigma+1) -P_tab(neos) + er&
  &go_sigma*((1+eps_tab(neos))*rho_tab(neos)))/((ergo_sigma+1)*rho_bl)-1
       dPcold_drho = ergo_sigma*(1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma*rho_bl
       eps_th = (Pl-v02l)/(gamma_th-1.d0)/rho_bl
       hl1 = cplusl + eps_th + Pl/rho_bl
 end where
end if
  ! Here sba = b^a defined in Gammies paper
  ...
  !  write(*,*) inside mhdflux1a, v02l(1,2,2),cplusl(1,2,2),cminusl(1,2,2),sb2l(1,2,2),dPcold_drho(1,2,2),gamma_th,hl1(1,2,2),rho_bl
  if (i==1) then
  ...
  !write(*,*) cplusl,i,cplusl(30,30,2),v02l(30,30,2),u0l(30,30,2),vxl(30,30,2),alpha_f(30,30,2),betax_f(30,30,2),phi_f(30,30,2),gupx
  cmax = max(0.d0,cplusl,cplusr)
  cmin = -min(0.d0,cminusl,cminusr)
  !  write(*,*) cmaxcmin,cmin(2,2,2),cminusl(2,2,2),cminusr(2,2,2)
  !Above lines of code will produce NaNs in the HLL step if cmax=cmin=0...
  !  This problem occurs in axisymmetry when we are in the x<0 ghostzones and have mhd_evolve::em_evolve_enable = no.
  !   Apparently with the B field evolution turned off, cpluss and cminuss are not set properly.
  ...
  ! write(*,*) hi.,(cmin(1,2,2)*Fr(1,2,2) + cmax(1,2,2)*Fl(1,2,2) - cmin(1,2,2)*cmax(1,2,2)*(rhor(1,2,2)-rhol(1,2,2)) ),(cmax(1,2,2)
...
!  write(*,*) u_zl:,u0l(5,2,2)*psi4(5,2,2),gxz_f(5,2,2)*(vxl(5,2,2)+betax_f(5,2,2)),vxl(5,2,2),vyl(5,2,2),vzl(5,2,2)
  ...
  ! write(*,*) before x flux hll:,psi4(17,2,15),gxx_f(17,2,15),sbxl(17,2,15),sbtl(17,2,15),betax_f(17,2,15),gxy_f(17,2,15),sbyl(17,2
  ! write(*,*) before x flux hll:,al(17,2,15),psi6(17,2,15),rho_bl(17,2,15),hl1(17,2,15),sb2l(17,2,15),u0l(17,2,15),vzl(17,2,15),u_x
