...
!-------------------------------------------------------
subroutine driver_advect_rad(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_time,c&
           ...
           do j=1,ext(2)
              do i=1,ext(1)
                 if (isnan(tau_rad(i,j,k)) .or. isnan(S_rad_x(i,j,k))) then
                    ...
                    write(*,*) "i,j,k =", i,j,k
                    write(*,*) "E_rad(i,j,k), F_rad0(i,j,k), F_radx(i,j,k), F_rady(i,j,k), F_radz(i,j,k)=",E_rad(i,j,k), F_rad0(i,j&
  &,k), F_radx(i,j,k), F_rady(i,j,k), F_radz(i,j,k)
                    write(*,*) "tau_rad_rhs(i,j,k), S_rad_x_rhs(i,j,k), tau_rad_scalar_diag(i,j,k) = ", tau_rad_rhs(i,j,k), S_rad_x&
  &_rhs(i,j,k), tau_rad_scalar_diag(i,j,k)
                    write(*,*) "chi_rad(i,j,k), ka_gf(i,j,k), ks_gf(i,j,k), emission_gf(i,j,k), eta_nue(i,j,k) = ", chi_rad(i,j,k),&
  & ka_gf(i,j,k), ks_gf(i,j,k), emission_gf(i,j,k), eta_nue(i,j,k)
                 end if
              end do
           end do
        end do
              ...
              do j=1,ext(2)
                 do i=1,ext(1)
                    if (isnan(tau_rad_nue(i,j,k)) .or. isnan(S_rad_x_nue(i,j,k))) then
                       write(*,*) "inside driver_advect_rad_nue.F90[[2]], tau_rad_nue and S_rad_x_nue=", tau_rad_nue(i,j,k), S_rad_&
  &x_nue(i,j,k)
                       write(*,*) "i,j,k =", i,j,k
                    end if
                 end do
              ...
              do j=1,ext(2)
                 do i=1,ext(1)
                    if (isnan(tau_rad_nux(i,j,k)) .or. isnan(S_rad_x_nux(i,j,k))) then
                       write(*,*) "inside driver_advect_rad_nux.F90[[2]], tau_rad_nux and S_rad_x_nux=", tau_rad_nux(i,j,k), S_rad_&
  &x_nux(i,j,k)
                       write(*,*) "i,j,k =", i,j,k
                    end if
                 end do
