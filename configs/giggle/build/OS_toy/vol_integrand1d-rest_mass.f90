!----------------------------------------
! Set up rest mass integrand for OS toy
!----------------------------------------
subroutine mhd_OS_rest_mass_integrand1d(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_d&
  ...
  &_convfac,cctk_nghostzones,cctk_iteration,cctkGH, X0Tracer,X0analytic,X0mhd_OS_private,X0more_pcle_stuff,X0particle_tracer_coord,&
  &X0pcle_stuff,X0v_previous,X1analytic,X1mhd_OS_private,X1more_pcle_stuff,X1particle_tracer_coord,X1v_previous,X2analytic,X2mhd_OS&
  &_private,X2v_previous,E_anal,E_rad_gradient,E_rad_pcle,Ec_anal,Ec_pcle,F_anal,F_coeff,F_radx_pcle,Fc_anal,Fc_pcle,OS_rest_mass_i&
  &ntegrand,OS_restmass_VolInt,P_pcle,Q,areal_radius,coord,coordt,eta,gxx_pcle,gxz_pcle,gzz_pcle,phi_pcle,pos,rho_b_anal,rho_b_anal&
  &ytic,rho_b_pcle,slope,slopem,slopet,tau_center_OS,u0_int,u0_p,u0_p_int,u0_pcle,vx_int,vx_p,vx_p_int,vy_int,vy_p,vy_p_int,vz_int,&
  &vz_p,vz_p_int,z_anal, X0BSSN_AH,X0BSSN_aux_private,X0BSSN_aux_restrict2,X0BSSN_diag_restrict,X0BSSN_gupij,X0BSSN_matter,X0BSSN_r&
  ...
  ! Compute integrand
  !-----------------------------------------------------------------------------
  OS_rest_mass_integrand = 0.D0
  k = cctk_nghostzones(3)+1
  j = cctk_nghostzones(2)+1
  ! We must multiply by cctk_levfacs here because the Carpet weight function assumes 3D integrals.
  ...
  !    included on the grid, since we multiply the final result by symmfactor=2, and we dont want to double-count
  !    the z=0 points.
  fudgefactor=4.D0*cctk_levfac(2)*cctk_levfac(3)
  do i = 1,cctk_lsh(1)
     OS_rest_mass_integrand(i,j,k) = rho_star(i,j,k)*fudgefactor
  end do
end subroutine mhd_OS_rest_mass_integrand1d
