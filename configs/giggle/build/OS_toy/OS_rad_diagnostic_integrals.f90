...
!
!-----------------------------------------------------------------------------
subroutine OS_rad_diag_integrals(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_ti&
  ...
  &c,cctk_nghostzones,cctk_iteration,cctkGH, X0Tracer,X0analytic,X0mhd_OS_private,X0more_pcle_stuff,X0particle_tracer_coord,X0pcle_&
  &stuff,X0v_previous,X1analytic,X1mhd_OS_private,X1more_pcle_stuff,X1particle_tracer_coord,X1v_previous,X2analytic,X2mhd_OS_privat&
  &e,X2v_previous,E_anal,E_rad_gradient,E_rad_pcle,Ec_anal,Ec_pcle,F_anal,F_coeff,F_radx_pcle,Fc_anal,Fc_pcle,OS_rest_mass_integran&
  &d,OS_restmass_VolInt,P_pcle,Q,areal_radius,coord,coordt,eta,gxx_pcle,gxz_pcle,gzz_pcle,phi_pcle,pos,rho_b_anal,rho_b_analytic,rh&
  &o_b_pcle,slope,slopem,slopet,tau_center_OS,u0_int,u0_p,u0_p_int,u0_pcle,vx_int,vx_p,vx_p_int,vy_int,vy_p,vy_p_int,vz_int,vz_p,vz&
  &_p_int,z_anal, X0BSSN_AH,X0BSSN_aux_private,X0BSSN_aux_restrict2,X0BSSN_diag_restrict,X0BSSN_gupij,X0BSSN_matter,X0BSSN_refbd,X0&
 ...
 if (mod(CCTK_iteration,out_every) .eq. 0) then
  ...
  !--------------------------------------------------------------------------------!
     ...
     areal_radius=exp(2.d0*phi_pcle2)*sqrt(coord(:,4)*coord(:,4)*gxx_pcle2-2.d0*coord(:,2)*coord(:,4)*gxz_pcle2+coord(:,2)*coord(:,&
  &2)*gzz_pcle2)
!     areal_radius=exp(2.d0*phi_pcle2)*sqrt(coord(:,3)*coord(:,3)*gxx_pcle2-2.d0*coord(:,2)*coord(:,3)*gxy_pcle2+coord(:,2)*coord(:,
     E_initial = E_over_rho*rho_b_initial**(gamma_OS)
!     E_initial = E_over_rho*rho_b_initial
     write(*,*) "E_initial, opt_depth_a, R_OS, narr = ", E_initial, opt_depth_a, R_OS, narr
  ...
  if(CCTK_MyProc(CCTKGH) .eq. 0) then
     ...
     if (mod(CCTK_iteration,out_every) .eq. 0) then
        ...
        write(15,'(t2,a6,t17,a3,t31,a3,t45,a5,t59,a1,t73,a1,t87,a1,t101,a2,t115,a2,t129,a3,t143,a1)') '"coord','tau','R_a','rho_b',&
  &'P','E','F','Ec','Fc','eta','z'
        do ii=1,narr
  ...
  if(CCTK_MyProc(CCTKGH) .eq. 0) then
     ...
     if (mod(CCTK_iteration,out_every) .eq. 0) then
        ...
        write(17,'(t2,a6,t17,a3,t31,a3,t45,a5,t59,a1,t73,a1,t87,a1,t101,a2,t115,a2,t129,a3,t143,a1)') '"coord','tau','R_a','rho_b',&
  &'P','E','F','Ec','Fc','eta','z'
        do ii=1,narr
           write(17,'(11e18.10)') coord(ii,2),coord(ii,1),areal_radius(ii),rho_b_pcle2(ii),P_pcle2(ii),E_rad_pcle2(ii),F_rad_scalar&
  &_pcle2(ii),Ec_pcle(ii),Fc_pcle(ii),eta(ii),z_anal(ii)
        end do
     endif
     close(17)
  endif
  ...
  if(CCTK_MyProc(CCTKGH) .eq. 0) then
     ...
     if (mod(CCTK_iteration,out_every) .eq. 0) then
        ...
        write(19,'(t2,a11,t17,a11,t31,a11,t45,a11,t59,a12,t73,a12,t87,a12,t101,a12,t115)') '"coord(ii,2)','coord(ii,1)','slope(ii,2&
  &)','slope(ii,1)','slopet(ii,2)','slopet(ii,1)','slopem(ii,2)','slopem(ii,1)'
        do ii=1,narr
           write(19,'(11e18.10)') coord(ii,2),coord(ii,1),coord(ii,1),slope(ii,2),slope(ii,1),slopet(ii,2),slopet(ii,1),slopem(ii,2&
  &),slopem(ii,1)
        end do
     endif
     close(19)
