subroutine PsiKadelia_postregrid(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_ti&
  ...
  do k=cctk_nghostzones(3)+1,cctk_lsh(3)-cctk_nghostzones(3)
     do j=cctk_nghostzones(2)+1,cctk_lsh(2)-cctk_nghostzones(2)
        do i=cctk_nghostzones(1)+1,cctk_lsh(1)-cctk_nghostzones(1)
           gxxx(i,j,k) = ((-      gxx( i+2,j,k)          + 8. * gxx( i+1,j,k)          - 8. * gxx( i-1,j,k)          +      gxx( i-&
  &2,j,k)) * (dxi / 12.))
           gxxy(i,j,k) = ((-      gxx( i,j+2,k)          + 8. * gxx( i,j+1,k)          - 8. * gxx( i,j-1,k)          +      gxx( i,&
  &j-2,k)) * (dyi / 12.))
           gxxz(i,j,k) = ((-      gxx( i,j,k+2)          + 8. * gxx( i,j,k+1)          - 8. * gxx( i,j,k-1)          +      gxx( i,&
  &j,k-2)) * (dzi / 12.))
           gxyx(i,j,k) = ((-      gxy( i+2,j,k)          + 8. * gxy( i+1,j,k)          - 8. * gxy( i-1,j,k)          +      gxy( i-&
  &2,j,k)) * (dxi / 12.))
           gxyy(i,j,k) = ((-      gxy( i,j+2,k)          + 8. * gxy( i,j+1,k)          - 8. * gxy( i,j-1,k)          +      gxy( i,&
  &j-2,k)) * (dyi / 12.))
           gxyz(i,j,k) = ((-      gxy( i,j,k+2)          + 8. * gxy( i,j,k+1)          - 8. * gxy( i,j,k-1)          +      gxy( i,&
  &j,k-2)) * (dzi / 12.))
        end do
     end do
  end do
...
!!$     end do
!!$     !$omp end parallel do
end subroutine PsiKadelia_postregrid
