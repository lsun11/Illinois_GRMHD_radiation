...
hdzi2 = 0.5*dzi*dzi;
  ...
  do k=kstart,kend
     do j=jstart,jend
        do i=istart,iend
           Gamx(i,j,k) = -(((-      gupxx( i+2,j,k)          + 8. * gupxx( i+1,j,k)          - 8. * gupxx( i-1,j,k)          +     &
  & gupxx( i-2,j,k)) * (dxi / 12.)) + ((-      gupxy( i,j+2,k)          + 8. * gupxy( i,j+1,k)          - 8. * gupxy( i,j-1,k)     &
  &     +      gupxy( i,j-2,k)) * (dyi / 12.)) + ((-      gupxz( i,j,k+2)          + 8. * gupxz( i,j,k+1)          - 8. * gupxz( i,&
  &j,k-1)          +      gupxz( i,j,k-2)) * (dzi / 12.)))
           Gamy(i,j,k) = -(((-      gupxy( i+2,j,k)          + 8. * gupxy( i+1,j,k)          - 8. * gupxy( i-1,j,k)          +     &
  & gupxy( i-2,j,k)) * (dxi / 12.)) + ((-      gupyy( i,j+2,k)          + 8. * gupyy( i,j+1,k)          - 8. * gupyy( i,j-1,k)     &
  &     +      gupyy( i,j-2,k)) * (dyi / 12.)) + ((-      gupyz( i,j,k+2)          + 8. * gupyz( i,j,k+1)          - 8. * gupyz( i,&
  &j,k-1)          +      gupyz( i,j,k-2)) * (dzi / 12.)))
           Gamz(i,j,k) = -(((-      gupxz( i+2,j,k)          + 8. * gupxz( i+1,j,k)          - 8. * gupxz( i-1,j,k)          +     &
  & gupxz( i-2,j,k)) * (dxi / 12.)) + ((-      gupyz( i,j+2,k)          + 8. * gupyz( i,j+1,k)          - 8. * gupyz( i,j-1,k)     &
  &     +      gupyz( i,j-2,k)) * (dyi / 12.)) + ((-      gupzz( i,j,k+2)          + 8. * gupzz( i,j,k+1)          - 8. * gupzz( i,&
  &j,k-1)          +      gupzz( i,j,k-2)) * (dzi / 12.)))
        end do
     end do
  end do
