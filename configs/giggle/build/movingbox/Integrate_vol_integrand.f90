...
!    This is why we multiply by cctk_delta_spaces (the coarsest gridspacing).
!--------------------------------------------------
subroutine Integrate_vol_integrand_movingbox(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,c&
  ...
  &,cctk_convfac,cctk_nghostzones,cctk_iteration,cctkGH, X0diag_movingbox_gf,X1diag_movingbox_gf,X2diag_movingbox_gf,box_denom,box_&
  &x,box_y,box_z, X0BSSN_AH,X0BSSN_aux_private,X0BSSN_aux_restrict2,X0BSSN_diag_restrict,X0BSSN_gupij,X0BSSN_matter,X0BSSN_refbd,X0&
  ...
  &,v02_radl,v02_radr,v02l,v02r,vvec,vx,vxl,vxr,vy,vyl,vyr,vz,vzl,vzr,w,zeta_rad,zeta_rad_nue,zeta_rad_nux, X0coordinates,X0radii,X&
  &0sf_radius,X1coordinates,X1sf_radius,X2coordinates,active_length,num_levels_length,positions_length,radii_length,sf_active_lengt&
  &h,sf_coordinate_descriptors_length,sf_info_length,sf_maxreflevel_length,sf_minreflevel_length,sf_origin_length,sf_radius_length,&
  &sf_shape_descriptors_length,sf_valid_length,active,coarse_dx,coarse_dy,coarse_dz,num_levels,position_x,position_y,position_z,r,r&
  &adius,sf_active,sf_area,sf_centroid_x,sf_centroid_y,sf_centroid_z,sf_delta_phi,sf_delta_theta,sf_max_radius,sf_max_x,sf_max_y,sf&
  &_max_z,sf_maxreflevel,sf_mean_radius,sf_min_radius,sf_min_x,sf_min_y,sf_min_z,sf_minreflevel,sf_nghostsphi,sf_nghoststheta,sf_np&
  &hi,sf_ntheta,sf_origin_phi,sf_origin_theta,sf_origin_x,sf_origin_y,sf_origin_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_x&
  &z,sf_quadrupole_yy,sf_quadrupole_yz,sf_quadrupole_zz,sf_radius,sf_valid,x,y,z)
  implicit none
  INTEGER cctk_dim
  ...
  if(track_bhns.eq.0) then
     ...
     if(mod(cctk_iteration,Compute_VolIntegrands_Every)==0) then
        ...
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegralx, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"movingbox::box_y")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegraly, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"movingbox::box_z")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegralz, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"movingbox::box_denom")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegraldenom, 1, vindex)
     ...
     if(mod(cctk_iteration,Compute_VolIntegrands_Every)==0 .and. cctk_iteration .gt. 1) then
        ...
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"movingbox::box_x")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegralx1, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"movingbox::box_y")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegraly1, 1, vindex)
!     call CCTK_ReductionHandle(handle,sum)
!     call CCTK_VarIndex(vindex,movingbox::box_z)
!     call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegralz1, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"movingbox::box_denom")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegraldenom1, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"mhd_evolve::temp1")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegralx2, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"mhd_evolve::temp2")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegraly2, 1, vindex)
!     call CCTK_ReductionHandle(handle,sum)
!     call CCTK_VarIndex(vindex,mhd_evolve::temp3)
!     call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegralz2, 1, vindex)
        call CCTK_ReductionHandle(handle,"sum")
        call CCTK_VarIndex(vindex,"mhd_evolve::temp4")
        call CCTK_Reduce(ierr,cctkGH, -1, handle, 1, 107, VolIntegraldenom2, 1, vindex)
     !WhichIntegral is set to 1351 in vol_integrands-movingbox.F90
!     if(WhichIntegral==(1351)) then
