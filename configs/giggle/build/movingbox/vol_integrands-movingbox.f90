...
!----------------------------
subroutine compute_movingbox_vol_integrands(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cc&
  ...
  &cctk_convfac,cctk_nghostzones,cctk_iteration,cctkGH, X0diag_movingbox_gf,X1diag_movingbox_gf,X2diag_movingbox_gf,box_denom,box_x&
  &,box_y,box_z, X0BSSN_AH,X0BSSN_aux_private,X0BSSN_aux_restrict2,X0BSSN_diag_restrict,X0BSSN_gupij,X0BSSN_matter,X0BSSN_refbd,X0B&
  ...
  &v02_radl,v02_radr,v02l,v02r,vvec,vx,vxl,vxr,vy,vyl,vyr,vz,vzl,vzr,w,zeta_rad,zeta_rad_nue,zeta_rad_nux, X0coordinates,X0radii,X0&
  &sf_radius,X1coordinates,X1sf_radius,X2coordinates,active_length,num_levels_length,positions_length,radii_length,sf_active_length&
  &,sf_coordinate_descriptors_length,sf_info_length,sf_maxreflevel_length,sf_minreflevel_length,sf_origin_length,sf_radius_length,s&
  &f_shape_descriptors_length,sf_valid_length,active,coarse_dx,coarse_dy,coarse_dz,num_levels,position_x,position_y,position_z,r,ra&
  &dius,sf_active,sf_area,sf_centroid_x,sf_centroid_y,sf_centroid_z,sf_delta_phi,sf_delta_theta,sf_max_radius,sf_max_x,sf_max_y,sf_&
  &max_z,sf_maxreflevel,sf_mean_radius,sf_min_radius,sf_min_x,sf_min_y,sf_min_z,sf_minreflevel,sf_nghostsphi,sf_nghoststheta,sf_nph&
  &i,sf_ntheta,sf_origin_phi,sf_origin_theta,sf_origin_x,sf_origin_y,sf_origin_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_xz&
  &,sf_quadrupole_yy,sf_quadrupole_yz,sf_quadrupole_zz,sf_radius,sf_valid,x,y,z)
  implicit none
  !~~~~~> Input parameters:
  INTEGER cctk_dim
  ...
  !           Youll end up with no improvement in your results, due to time prolongation.
...
!     print *, INSIDE  vol_integrands-movingbox.F90, track_bhns=,track_bhns
  if(.not.(num_matter_refinement_box_centers.eq.1 .or. num_matter_refinement_box_centers.eq.0)) then
     write(*,*) "ERROR.  You must set num_matter_refinement_box_centers.eq.1 or num_matter_refinement_box_centers.eq.0 when the mov&
  &ingbox thorn is enabled!"
     stop
  end if
