subroutine BHNS_setup_emfield_part2p5_local_consrv_aftercp(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_&
  ...
  if(CCTK_ITERATION .eq. ITERATION_TO_INSERT_MAGNETIC_FIELDS .and. ITERATION_TO_INSERT_MAGNETIC_FIELDS.ne.0) then
     ...
     !  calculate As & then Bs. The Bs are then correct on all points except the stencil size, which is:
