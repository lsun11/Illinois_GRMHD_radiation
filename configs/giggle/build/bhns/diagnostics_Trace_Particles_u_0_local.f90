...
!-----------------------------------------------------------------------------
subroutine bhns_diagnostics_tracer_compute_u_0(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox&
  ...
  &el,cctk_convfac,cctk_nghostzones,cctk_iteration,cctkGH, X0Axyzintrgroup,X0Tracer_rhs_group,X0Tracergroup,X0last_ah_data,X1Axyzin&
  &trgroup,X1Tracer_rhs_group,X2Axyzintrgroup,Axintr,Ayintr,Azintr,M0dot_last,Pintr,ah_radii_last,bhnsEthermalNOW,bhns_rk4_particle&
  &_tracer_step,bhnsinsertBNOW,bhnsinsertRADNOW,int_M0dot,local_density,m1_minus_u_lower_zero,rhobintr,t_last,taul_rhs,tracer_taul,&
  &tracer_x,tracer_xc,tracer_y,tracer_yc,tracer_z,u_x_local,u_y_local,x_rhs,xc_rhs,xh_last,y_rhs,yc_rhs,yh_last,z_rhs,zh_last, X0BS&
  ...
  &v02l,v02r,vvec,vx,vxl,vxr,vy,vyl,vyr,vz,vzl,vzr,w,zeta_rad,zeta_rad_nue,zeta_rad_nux, X0coordinates,X0mask,X0radii,X0sf_radius,X&
  &0space_mask_group,X1coordinates,X1mask,X1sf_radius,X1space_mask_group,X2coordinates,X2mask,X2space_mask_group,active_length,num_&
  &levels_length,positions_length,radii_length,sf_active_length,sf_coordinate_descriptors_length,sf_info_length,sf_maxreflevel_leng&
  &th,sf_minreflevel_length,sf_origin_length,sf_radius_length,sf_shape_descriptors_length,sf_valid_length,active,coarse_dx,coarse_d&
  &y,coarse_dz,emask,num_levels,position_x,position_y,position_z,r,radius,sf_active,sf_area,sf_centroid_x,sf_centroid_y,sf_centroid&
  &_z,sf_delta_phi,sf_delta_theta,sf_max_radius,sf_max_x,sf_max_y,sf_max_z,sf_maxreflevel,sf_mean_radius,sf_min_radius,sf_min_x,sf_&
  &min_y,sf_min_z,sf_minreflevel,sf_nghostsphi,sf_nghoststheta,sf_nphi,sf_ntheta,sf_origin_phi,sf_origin_theta,sf_origin_x,sf_origi&
  &n_y,sf_origin_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_xz,sf_quadrupole_yy,sf_quadrupole_yz,sf_quadrupole_zz,sf_radius,&
  ...
  integer :: i,j,k
  real*8  :: phiL,psi4L,psi6L,gxxL,gxyL,gxzL,gyyL,gyzL,gzzL,Bx_L,By_L,Bz_L,x_minus_xBH,y_minus_yBH,partial_phi_magnitude,e_hat_phix&
  &,e_hat_phiy,B_x_L,B_y_L,B_toroidal,B2,B_poloidal,Omega,smallb2L,lambda_MRI,dX,rho_starL
  ...
  if(MOD(cctk_iteration,out_every)==0 .and. em_evolve_enable==1 .and. cctk_iteration.ge.ITERATION_TO_output_MRI_wavelength) then
     ...
     !Heres how we define the toroidal & poloidal components of the magnetic field.
     !
     ...
     do k=1,cctk_lsh(3)
        do j=1,cctk_lsh(2)
           do i=1,cctk_lsh(1)
              ...
              partial_phi_magnitude = sqrt( psi4L * ( gxxL*(-y_minus_yBH)**2 + gyyL*(x_minus_xBH)**2 + 2.D0*gxyL*(x_minus_xBH)*(-y_&
  &minus_yBH) ) )
              ...
              B2 = psi4L * (gxxL*Bx_L*Bx_L + 2.D0*gxyL*Bx_L*By_L + 2.D0*gxzL*Bx_L*Bz_L + gyyL*By_L*By_L + 2.D0*gyzL*By_L*Bz_L + gzz&
  &L*Bz_L*Bz_L)
...
!              if(cctk_iteration.ge.ITERATION_TO_output_MRI_wavelength .and. MOD(cctk_iteration,out_every)==0) then
              ...
              Omega = ((X(i,j,k)-bh_posn_x(1))*vy(i,j,k)-(Y(i,j,k)-bh_posn_y(1))*vx(i,j,k)) / ((X(i,j,k)-bh_posn_x(1))**2+(Y(i,j,k)&
  &-bh_posn_y(1))**2)
              smallb2L = temp2(i,j,k)
              lambda_MRI = 2.D0*3.14159265358979323846D0*sqrt(B_poloidal*B_poloidal / (smallb2L + rho_b(i,j,k)*h(i,j,k)) ) / (abs(O&
  &mega))
...
!                 temp8(i,j,k)=0.D0
 ...
 !             end if
              ...
              !if(rho_b(i,j,k) .lt. 1.D0*rho_b_atm .or. sqrt( (X(i,j,k)-bh_posn_x(1))**2 + (Y(i,j,k)-bh_posn_y(1))**2 + (Z(i,j,k)-bh
              !if(rho_b(i,j,k) .lt. 1.D3*rho_b_atm .or. sqrt( (X(i,j,k)-bh_posn_x(1))**2 + (Y(i,j,k)-bh_posn_y(1))**2 + (Z(i,j,k)-bh
              if(sqrt( (X(i,j,k)-bh_posn_x(1))**2 + (Y(i,j,k)-bh_posn_y(1))**2 + (Z(i,j,k)-bh_posn_z(1))**2 ) .lt. max_horiz_radius&
  &) then
                 temp2(i,j,k) = 0.D0
                 temp3(i,j,k) = 0.D0
                 temp4(i,j,k) = 0.D0
