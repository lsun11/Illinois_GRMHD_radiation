subroutine BHNS_insert_radiation_after_checkpoint(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_b&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!                                                                 
     ! The E_rad are set based on T_fluid. We dont have T_fluid_p or T_fluid_p_p, so we must compute that using the primitives solve
...
!                E_rad_nux(i,j,k) = 7.0/8.0 * rad_const_cgs * T_fluid_cgs**4 * G_cgs / c_cgs**4 * c2k**(-2)
  ...
  &), F_rady(i,j,k), F_radz(i,j,k), F_rad0(i,j,k)
                 write(*,*) "tau_rad(i,j,k), S_rad_x(i,j,k), S_rad_y(i,j,k), S_rad_z(i,j,k)=", tau_rad(i,j,k), S_rad_x(i,j,k), S_ra&
  &d_y(i,j,k), S_rad_z(i,j,k)
