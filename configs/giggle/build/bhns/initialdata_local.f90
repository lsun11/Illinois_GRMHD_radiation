subroutine bhns_initialdata_local(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_t&
  ...
  &ac,cctk_nghostzones,cctk_iteration,cctkGH, X0Axyzintrgroup,X0Tracer_rhs_group,X0Tracergroup,X0last_ah_data,X1Axyzintrgroup,X1Tra&
  &cer_rhs_group,X2Axyzintrgroup,Axintr,Ayintr,Azintr,M0dot_last,Pintr,ah_radii_last,bhnsEthermalNOW,bhns_rk4_particle_tracer_step,&
  &bhnsinsertBNOW,bhnsinsertRADNOW,int_M0dot,local_density,m1_minus_u_lower_zero,rhobintr,t_last,taul_rhs,tracer_taul,tracer_x,trac&
  &er_xc,tracer_y,tracer_yc,tracer_z,u_x_local,u_y_local,x_rhs,xc_rhs,xh_last,y_rhs,yc_rhs,yh_last,z_rhs,zh_last, X0BSSN_AH,X0BSSN_&
  ...
  &c,vx,vxl,vxr,vy,vyl,vyr,vz,vzl,vzr,w,zeta_rad,zeta_rad_nue,zeta_rad_nux, X0coordinates,X0mask,X0radii,X0sf_radius,X0space_mask_g&
  &roup,X1coordinates,X1mask,X1sf_radius,X1space_mask_group,X2coordinates,X2mask,X2space_mask_group,active_length,num_levels_length&
  &,positions_length,radii_length,sf_active_length,sf_coordinate_descriptors_length,sf_info_length,sf_maxreflevel_length,sf_minrefl&
  &evel_length,sf_origin_length,sf_radius_length,sf_shape_descriptors_length,sf_valid_length,active,coarse_dx,coarse_dy,coarse_dz,e&
  &mask,num_levels,position_x,position_y,position_z,r,radius,sf_active,sf_area,sf_centroid_x,sf_centroid_y,sf_centroid_z,sf_delta_p&
  ...
  &_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_xz,sf_quadrupole_yy,sf_quadrupole_yz,sf_quadrupole_zz,sf_radius,sf_valid,spac&
  &e_mask,x,y,z)
  implicit none
  INTEGER cctk_dim
     ...
     do j=1,cctk_lsh(2)
        do i=1,cctk_lsh(1)
           if(isnan(Ax(i,j,k)).eqv. .true.) then
              write(*,*) "1. Inside initialdata_local.F90, Ax is nan!!!"
           end if
        end do
     end do
  end do
  ...
  if(use_new_bhns_initial_data==0) then
     ...
     Ayz= Ayz*psi**(-4)
     Azz=-(Axx+Ayy)
  else if((use_new_bhns_initial_data==6).or.(use_new_bhns_initial_data==8)) then
     print *, "setting up  conformal extrinsic curvature"
     ! kij is the extrinsic curvature. We now convert it to the BSSN traceless, conformal extrinsic curvature
     Axx = kxx*psi**(-4) - gxx*trK/3.d0
     Axy = kxy*psi**(-4) - gxy*trK/3.d0
     Axz = kxz*psi**(-4) - gxz*trK/3.d0
     Ayy = kyy*psi**(-4) - gyy*trK/3.d0
     ...
     do j=1,cctk_lsh(2)
        do i=1,cctk_lsh(1)
           ...
           if ((use_new_bhns_initial_data==1).or. &
                 ...
                 (use_new_bhns_initial_data==4).or. &
                 (use_new_bhns_initial_data==5)) then
              gupxx(i,j,k)=ONE
              gupyy(i,j,k)=ONE
              gupzz(i,j,k)=ONE
              gupxy(i,j,k)=ZERO
  ...
  do k=1,cctk_lsh(3)
     do j=1,cctk_lsh(2)
        do i=1,cctk_lsh(1)
           if(isnan(Ax(i,j,k)).eqv. .true.) then
              write(*,*) "2. Inside initialdata_local.F90, Ax is nan!!!"
           end if
        end do
     end do
  end do
  ...
  print *,"done with bhns_initialdata_local"
...
!  print *, ==== After check_nans
!  do i=1,cctk_lsh(1)
!     print *, X(i,1,1), gxx(i,1,1), gxy(i,1,1)
!  end do
!  print *, ===============bhns_initialdata_local
