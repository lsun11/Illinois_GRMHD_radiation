...
!
  ...
  real*8, dimension(ex(1),ex(2),ex(3))     :: P_cld, eps_cld, eps_thermal
!
! Other variables:
!
        ...
        do k = kmin, kmax
           ...
           if(ergo_qc.eq.0) then
              ...
              if((rho0 - rho_tab(1))/rho_tab(1).le.1.0d-15) then
                 ...
                 Press = (ergo_sigma*((1.0d0 + eps_tab(neos) + P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma)*rho0**(ergo_si&
  &gma + 1.0d0) + &
...
!!!! Experiment: Add a little addiional eps (and pressure) as the thermal part  to make the temperature finite 
           ...
           eps_thermal(i,j,k) = eps-eps_cld(i,j,k)
           Press = Press + (gamma_th-1.d0)*rho0*eps_thermal(i,j,k)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           hm1 = eps + Press/rho0
           ! enth = 1.D0 + hm1
           fac2       = rho0*enth*U*U
