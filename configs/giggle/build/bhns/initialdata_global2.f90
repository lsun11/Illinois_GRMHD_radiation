subroutine bhns_initialdata_global2(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta&
  ...
  &vfac,cctk_nghostzones,cctk_iteration,cctkGH, X0Axyzintrgroup,X0Tracer_rhs_group,X0Tracergroup,X0last_ah_data,X1Axyzintrgroup,X1T&
  &racer_rhs_group,X2Axyzintrgroup,Axintr,Ayintr,Azintr,M0dot_last,Pintr,ah_radii_last,bhnsEthermalNOW,bhns_rk4_particle_tracer_ste&
  &p,bhnsinsertBNOW,bhnsinsertRADNOW,int_M0dot,local_density,m1_minus_u_lower_zero,rhobintr,t_last,taul_rhs,tracer_taul,tracer_x,tr&
  &acer_xc,tracer_y,tracer_yc,tracer_z,u_x_local,u_y_local,x_rhs,xc_rhs,xh_last,y_rhs,yc_rhs,yh_last,z_rhs,zh_last, X0BSSN_AH,X0BSS&
  ...
  &vec,vx,vxl,vxr,vy,vyl,vyr,vz,vzl,vzr,w,zeta_rad,zeta_rad_nue,zeta_rad_nux, X0coordinates,X0mask,X0radii,X0sf_radius,X0space_mask&
  &_group,X1coordinates,X1mask,X1sf_radius,X1space_mask_group,X2coordinates,X2mask,X2space_mask_group,active_length,num_levels_leng&
  &th,positions_length,radii_length,sf_active_length,sf_coordinate_descriptors_length,sf_info_length,sf_maxreflevel_length,sf_minre&
  &flevel_length,sf_origin_length,sf_radius_length,sf_shape_descriptors_length,sf_valid_length,active,coarse_dx,coarse_dy,coarse_dz&
  &,emask,num_levels,position_x,position_y,position_z,r,radius,sf_active,sf_area,sf_centroid_x,sf_centroid_y,sf_centroid_z,sf_delta&
  ...
  &in_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_xz,sf_quadrupole_yy,sf_quadrupole_yz,sf_quadrupole_zz,sf_radius,sf_valid,sp&
  &ace_mask,x,y,z)
  implicit none
  INTEGER cctk_dim
  ...
  if (handle .gt. 0) then
     ...
     call CCTK_Warn(1,51,"initialdata_global2.F90","bhns","TestReduce: Invalid reduction operator")
  end if
