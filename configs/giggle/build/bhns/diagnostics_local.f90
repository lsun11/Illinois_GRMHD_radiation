...
!-----------------------------------------------------------------------------
subroutine bhns_diagnostics_local(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_t&
...
!!$        do j = 1,cctk_lsh(2)
!!$           do i = 1,cctk_lsh(1)
!!$              if(abs(Ax(i,j,k)).gt.1.D-5) then
!!$                 write(*,*) ORIGIN AXYZ:,Ax(i,j,k),Ay(i,j,k),Az(i,j,k)
!!$              end if
!!$           end do
!!$        end do
!!$     end do
...
!!$     !write(*,*) COMPUTING B2 INSIDE BHNS THORN, diagnostics_local.F90, dx=,(cctk_delta_space(1)/cctk_levfac(1))
!!$
!!$     !$omp parallel do
!!$     do k = 1,cctk_lsh(3)
!!$        do j = 1,cctk_lsh(2)
...
!!$     !$omp end parallel do
!!$
!!$     !  call bhns_compute_B_from_A(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delt
!!$
...
!!$              !   write(*,*) BAD B2... Dist from horizon centroid:,sqrt((x(i,j,k)-bh_posn_x(1))**2 + (y(i,j,k)-bh_posn_y(1))**2 +
