...
!-----------------------------------------------------------------------------
subroutine bhns_diagnostics_local2(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_&
  ...
  if(mod(cctk_iteration,out_every)==0) then
     ...
     !$omp end parallel do
     write(*,*) "bhns_max_b2 = ",bhns_max_b2
     if(bhns_max_b2.gt.0.D0) then
        !$omp parallel do
        do k=1,cctk_lsh(3)
           do j=1,cctk_lsh(2)
              do i=1,cctk_lsh(1)
                 if(abs((smallb2(i,j,k)-bhns_max_b2)/bhns_max_b2).lt.1e-8) then
                    ! make sure were out of a ghostzone here:
                    temp9(7,7,7)  = X(i,j,k)
     ...
     if(bhns_max_rho_b.gt.0.D0) then
      !$omp parallel do
        do k=1,cctk_lsh(3)
           do j=1,cctk_lsh(2)
              do i=1,cctk_lsh(1)
                 if(abs((rho_b(i,j,k)-bhns_max_rho_b)/bhns_max_rho_b).lt.1e-8) then
                    ! make sure were out of a ghostzone here:
                    temp12(7,7,7)  = X(i,j,k)
                    temp13(7,7,7)  = Y(i,j,k)
                    temp14(7,7,7)  = Z(i,j,k)
                 end if
              end do
           end do
        end do
      !$omp end parallel do
        ...
        write(*,*) "It looks like the density is <= zeo.", bhns_max_rho_b
     end if
  end if
end subroutine bhns_diagnostics_local2
