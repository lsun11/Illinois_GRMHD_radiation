subroutine bhns_initialdata_local2(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_delta_&
  ...
  &fac,cctk_nghostzones,cctk_iteration,cctkGH, X0Axyzintrgroup,X0Tracer_rhs_group,X0Tracergroup,X0last_ah_data,X1Axyzintrgroup,X1Tr&
  &acer_rhs_group,X2Axyzintrgroup,Axintr,Ayintr,Azintr,M0dot_last,Pintr,ah_radii_last,bhnsEthermalNOW,bhns_rk4_particle_tracer_step&
  &,bhnsinsertBNOW,bhnsinsertRADNOW,int_M0dot,local_density,m1_minus_u_lower_zero,rhobintr,t_last,taul_rhs,tracer_taul,tracer_x,tra&
  &cer_xc,tracer_y,tracer_yc,tracer_z,u_x_local,u_y_local,x_rhs,xc_rhs,xh_last,y_rhs,yc_rhs,yh_last,z_rhs,zh_last, X0BSSN_AH,X0BSSN&
  ...
  &ec,vx,vxl,vxr,vy,vyl,vyr,vz,vzl,vzr,w,zeta_rad,zeta_rad_nue,zeta_rad_nux, X0coordinates,X0mask,X0radii,X0sf_radius,X0space_mask_&
  &group,X1coordinates,X1mask,X1sf_radius,X1space_mask_group,X2coordinates,X2mask,X2space_mask_group,active_length,num_levels_lengt&
  &h,positions_length,radii_length,sf_active_length,sf_coordinate_descriptors_length,sf_info_length,sf_maxreflevel_length,sf_minref&
  &level_length,sf_origin_length,sf_radius_length,sf_shape_descriptors_length,sf_valid_length,active,coarse_dx,coarse_dy,coarse_dz,&
  &emask,num_levels,position_x,position_y,position_z,r,radius,sf_active,sf_area,sf_centroid_x,sf_centroid_y,sf_centroid_z,sf_delta_&
  &phi,sf_delta_theta,sf_max_radius,sf_max_x,sf_max_y,sf_max_z,sf_maxreflevel,sf_mean_radius,sf_min_radius,sf_min_x,sf_min_y,sf_min&
  &_z,sf_minreflevel,sf_nghostsphi,sf_nghoststheta,sf_nphi,sf_ntheta,sf_origin_phi,sf_origin_theta,sf_origin_x,sf_origin_y,sf_origi&
  &n_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_xz,sf_quadrupole_yy,sf_quadrupole_yz,sf_quadrupole_zz,sf_radius,sf_valid,spa&
  &ce_mask,x,y,z)
  implicit none
  INTEGER cctk_dim
...
if ((use_new_bhns_initial_data/=5)) then
   ! the vs are really the u_is when use_new_bhns_initial_data<2
   if ((use_new_bhns_initial_data==1).or. &
      ...
      ! The vs are the Us determined through u^a = W(n^a + U^a) when use_new_bhns_initial_data==2
  ...
  if(ergo_star.eq.0) then
     ...
     enddo
     where((rho_b - rho_tab(1))/rho_tab(1).le.1.0d-15)
        P    = k_tab(1)*rho_b**gamma_tab(1)
        h    = 1.0d0 + P/rho_b*gamma_tab(1)/(gamma_tab(1) - 1.0d0)
     elsewhere((rho_b - rho_tab(neos))/rho_tab(neos).ge.1.0d-15)
     ...
     h    = 1.0d0 + P/rho_b*gamma_tab(1)/(gamma_tab(1) - 1.0d0)
     elsewhere((rho_b - rho_tab(neos))/rho_tab(neos).ge.1.0d-15)
      P = (ergo_sigma*((1.0d0+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma)*rho_b**(ergo_sigma+1) + P_tab(neo&
  &s) - ergo_sigma*(rho_tab(neos)*(eps_tab(neos)+1)))/(ergo_sigma+1)
...
!!!! Experiment: Add a little addiional eps (and pressure) as the thermal part to make the temperature finite
             do k=1,ext(3)
                 do j=1,ext(2)
                        do i=1,ext(1)
                     eps_thermal(i,j,k) = (eps_thermal_bhns -1.0) * eps_cld(i,j,k)
                     eps_tot(i,j,k) = eps_thermal(i,j,k) + eps_cld(i,j,k)
                         call compute_P_T_microphys(P(i,j,k), T_fluid(i,j,k), P_cld(i,j,k), eps_tot(i,j,k), eps_cld(i,j,k), rho_b(i&
  &,j,k))
                        end do
                 end do
            end do
...
if (use_new_bhns_initial_data==5) then
            ...
            do j=1,ext(2)
               do i=1,ext(1)
                  ...
                  if (rr .ne. 0) then
                     phia = atan2(y(i,j,k), x(i,j,k))
                     if (y(i,j,k)<0)    phia = 2.0d0*pai + phia
                     !            if ( dabs(x(i,j,k))<=2.0 .and. dabs(y(i,j,k))<=2.0 .and. dabs(z(i,j,k))<=1.0   )  write(6,(a6,1p,1
                     if (nperturb==1)  rho_b(i,j,k) = rho_b(i,j,k)*(1.0d0-ampl_perturb + ampl_perturb*cos(2.0d0*phia))
                     if (nperturb==2)  rho_b(i,j,k) = rho_b(i,j,k)*(1.0d0-ampl_perturb + ampl_perturb*cos(Omega_value*phia))
                  end if
               end do
  ...
  if(fisheye_enable .eq. 1) then
     ...
     call CCTK_Warn(1,581,"initialdata_local2.F90","bhns","TestReduce: Invalid reduction operator")
     end if
    ...
    do j=1,ext(2)
      do i=1,ext(1)
           ...
           if(u_0l .eq. 0.0 .or. isnan(u_0l)) then
              write(*,*) "Inside BHNS initial2, u_0l is zero or nan!!!!, u_0l, g_00l, u0(i,j,k), udotbeta are", u_0l, g_00l, u0(i,j&
  &,k), udotbeta
           end if
           ...
           u_x = u0(i,j,k)*psiL4*(gxx(i,j,k)*(vx(i,j,k) + shiftx(i,j,k)) + gxy(i,j,k)*(vy(i,j,k) + shifty(i,j,k)) + gxz(i,j,k)*(vz(&
  &i,j,k) + shiftz(i,j,k)))
           u_y = u0(i,j,k)*psiL4*(gxy(i,j,k)*(vx(i,j,k) + shiftx(i,j,k)) + gyy(i,j,k)*(vy(i,j,k) + shifty(i,j,k)) + gyz(i,j,k)*(vz(&
  &i,j,k) + shiftz(i,j,k)))
           u_z = u0(i,j,k)*psiL4*(gxz(i,j,k)*(vx(i,j,k) + shiftx(i,j,k)) + gyz(i,j,k)*(vy(i,j,k) + shifty(i,j,k)) + gzz(i,j,k)*(vz(&
  &i,j,k) + shiftz(i,j,k)))
           F_rad0(i,j,k) = - (F_radx(i,j,k)*u_x + F_rady(i,j,k)*u_y + F_radz(i,j,k)*u_z)/u_0l
           if (isnan(F_rad0(i,j,k))) then
              write(*,*) "Inside BHNS initial2, F_rad0(i,j,k) is nan!!!!, F_radx(i,j,k), u_x, F_rady(i,j,k), u_y, F_radz(i,j,k), u_&
  &z, u_0l are",  F_radx(i,j,k), u_x, F_rady(i,j,k), u_y, F_radz(i,j,k), u_z, u_0l
           end if
           F_rad_xl = psiL4 * (gxx(i,j,k) * F_radx(i,j,k) + gxy(i,j,k) * F_rady(i,j,k) + gxz(i,j,k) * F_radz(i,j,k)) + shift_x* F_r&
  &ad0(i,j,k)
           F_rad_yl = psiL4 * (gxy(i,j,k) * F_radx(i,j,k) + gyy(i,j,k) * F_rady(i,j,k) + gyz(i,j,k) * F_radz(i,j,k)) + shift_y* F_r&
  &ad0(i,j,k)
           F_rad_zl = psiL4 * (gxz(i,j,k) * F_radx(i,j,k) + gyz(i,j,k) * F_rady(i,j,k) + gzz(i,j,k) * F_radz(i,j,k)) + shift_z* F_r&
  &ad0(i,j,k)
           ...
           if (rad_closure_scheme.eq.0) then
              ...
              tau_rad(i,j,k) = (lapm1(i,j,k)+1.0)*(lapm1(i,j,k)+1.0)*psiL**6*(E_rad(i,j,k)*u0(i,j,k)*u0(i,j,k)+2.0*F_rad0(i,j,k)*u0&
  &(i,j,k)+P_radl*u0(i,j,k)*u0(i,j,k))-psiL**6*P_radl
              S_rad_x(i,j,k) = (lapm1(i,j,k)+1.0)*psiL**6*((E_rad(i,j,k)+P_radl)*u0(i,j,k)*u_x + F_rad0(i,j,k)*u_x + F_rad_xl * u0(&
  &i,j,k))
              S_rad_y(i,j,k) = (lapm1(i,j,k)+1.0)*psiL**6*((E_rad(i,j,k)+P_radl)*u0(i,j,k)*u_y + F_rad0(i,j,k)*u_y + F_rad_yl * u0(&
  &i,j,k))
              S_rad_z(i,j,k) = (lapm1(i,j,k)+1.0)*psiL**6*((E_rad(i,j,k)+P_radl)*u0(i,j,k)*u_z + F_rad0(i,j,k)*u_z + F_rad_zl * u0(&
  &i,j,k))
              rho(i,j,k) = rho(i,j,k) + temp_rad1
              Sx(i,j,k) = Sx(i,j,k) + temp_rad*( ( (E_rad(i,j,k) + P_radl) * u0(i,j,k) + F_rad0(i,j,k)) * (shift_x + v_xl) + F_rad_&
  &xl)
              Sy(i,j,k) = Sy(i,j,k) + temp_rad*( ( (E_rad(i,j,k) + P_radl) * u0(i,j,k) + F_rad0(i,j,k)) * (shift_y + v_yl) + F_rad_&
  &yl)
              Sz(i,j,k) = Sz(i,j,k) + temp_rad*( ( (E_rad(i,j,k) + P_radl) * u0(i,j,k) + F_rad0(i,j,k)) * (shift_z + v_zl) + F_rad_&
  &zl)
              Sxx(i,j,k) = Sxx(i,j,k) + (E_rad(i,j,k)+P_radl)*(u0(i,j,k)*(shift_x + v_xl))**2 + 2.0*F_rad_xl*u0(i,j,k)*(shift_x + v&
  &_xl) + psiL4 * P_radl * gxx(i,j,k)
              Syy(i,j,k) = Syy(i,j,k) + (E_rad(i,j,k)+P_radl)*(u0(i,j,k)*(shift_y + v_yl))**2 + 2.0*F_rad_yl*u0(i,j,k)*(shift_y + v&
  &_yl) + psiL4 * P_radl * gyy(i,j,k)
              Szz(i,j,k) = Szz(i,j,k) + (E_rad(i,j,k)+P_radl)*(u0(i,j,k)*(shift_z + v_zl))**2 + 2.0*F_rad_zl*u0(i,j,k)*(shift_z + v&
  &_zl) + psiL4 * P_radl * gzz(i,j,k)
              Sxy(i,j,k) = Sxy(i,j,k) + (E_rad(i,j,k)+P_radl)*u0(i,j,k)**2*(shift_x + v_xl)*(shift_y + v_yl) + u0(i,j,k)*(F_rad_xl*&
  &(shift_y + v_yl)+F_rad_yl*(shift_x + v_xl)) + psiL4 * P_radl * gxy(i,j,k)
              Sxz(i,j,k) = Sxz(i,j,k) + (E_rad(i,j,k)+P_radl)*u0(i,j,k)**2*(shift_x + v_xl)*(shift_z + v_zl) + u0(i,j,k)*(F_rad_xl*&
  &(shift_z + v_zl)+F_rad_zl*(shift_x + v_xl)) + psiL4 * P_radl * gxz(i,j,k)
              Syz(i,j,k) = Syz(i,j,k) + (E_rad(i,j,k)+P_radl)*u0(i,j,k)**2*(shift_y + v_yl)*(shift_z + v_zl) + u0(i,j,k)*(F_rad_yl*&
  &(shift_z + v_zl)+F_rad_zl*(shift_y + v_yl)) + psiL4 * P_radl * gyz(i,j,k)
           else
              Fasq = F_rad_0l*F_rad0(i,j,k) + F_rad_xl*F_radx(i,j,k) +  F_rad_yl*F_rady(i,j,k) +  F_rad_zl*F_radz(i,j,k)
              zeta_temp = sqrt(abs(F_rad_0l*F_rad0(i,j,k) + F_rad_xl*F_radx(i,j,k) +  F_rad_yl*F_rady(i,j,k) +  F_rad_zl*F_radz(i,j&
  &,k))/E_rad(i,j,k)**2)
!              zeta_cut = Erad_atm_cut*1.5
              ...
              if (E_rad(i,j,k) .lt. Erad_atm_cut) then
                 ...
                 if (Fasq .le. 0) then
                    P_radxx(i,j,k) = E_rad(i,j,k)*(gupxx(i,j,k)/psiL4 - shiftx(i,j,k)*shiftx(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uxl**2)&
  &/2.0*(1.0-chil)
                    P_radyy(i,j,k) = E_rad(i,j,k)*(gupyy(i,j,k)/psiL4 - shifty(i,j,k)*shifty(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uyl**2)&
  &/2.0*(1.0-chil)
                    P_radzz(i,j,k) = E_rad(i,j,k)*(gupzz(i,j,k)/psiL4 - shiftz(i,j,k)*shiftz(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uzl**2)&
  &/2.0*(1.0-chil)
                    P_radxy(i,j,k) = E_rad(i,j,k)*(gupxy(i,j,k)/psiL4 - shiftx(i,j,k)*shifty(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uxl*uyl&
  &)/2.0*(1.0-chil)
                    P_radxz(i,j,k) = E_rad(i,j,k)*(gupxz(i,j,k)/psiL4 - shiftx(i,j,k)*shiftz(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uxl*uzl&
  &)/2.0*(1.0-chil)
                    P_radyz(i,j,k) = E_rad(i,j,k)*(gupyz(i,j,k)/psiL4 - shifty(i,j,k)*shiftz(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uyl*uzl&
  &)/2.0*(1.0-chil)
                 else
                    P_radxx(i,j,k) = E_rad(i,j,k)*((F_radx(i,j,k)**2/Fasq)*(3.0*chil -1.0)/2.0 + (gupxx(i,j,k)/psiL4 - shiftx(i,j,k&
  &)*shiftx(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uxl**2)/2.0*(1.0-chil))
                    P_radyy(i,j,k) = E_rad(i,j,k)*((F_rady(i,j,k)**2/Fasq)*(3.0*chil -1.0)/2.0 + (gupyy(i,j,k)/psiL4 - shifty(i,j,k&
  &)*shifty(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uyl**2)/2.0*(1.0-chil))
                    P_radzz(i,j,k) = E_rad(i,j,k)*((F_radz(i,j,k)**2/Fasq)*(3.0*chil -1.0)/2.0 + (gupzz(i,j,k)/psiL4 - shiftz(i,j,k&
  &)*shiftz(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uzl**2)/2.0*(1.0-chil))
                    P_radxy(i,j,k) = E_rad(i,j,k)*((F_radx(i,j,k)*F_rady(i,j,k)/Fasq)*(3.0*chil -1.0)/2.0 + (gupxy(i,j,k)/psiL4 - s&
  &hiftx(i,j,k)*shifty(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uxl*uyl)/2.0*(1.0-chil))
                    P_radxz(i,j,k) = E_rad(i,j,k)*((F_radx(i,j,k)*F_radz(i,j,k)/Fasq)*(3.0*chil -1.0)/2.0 + (gupxz(i,j,k)/psiL4 - s&
  &hiftx(i,j,k)*shiftz(i,j,k)/(lapm1(i,j,k)+1.0)**2 + uxl*uzl)/2.0*(1.0-chil))
                    P_radyz(i,j,k) = E_rad(i,j,k)*((F_rady(i,j,k)*F_radz(i,j,k)/Fasq)*(3.0*chil -1.0)/2.0 + (gupyz(i,j,k)/psiL4 - s&
  &hifty(i,j,k)*shiftz(i,j,k)/(lapm1(i,j,k)+1.0)**2 ++ uyl*uzl)/2.0*(1.0-chil))
                 end if
              end if
              ...
              tau_rad(i,j,k) = (lapm1(i,j,k)+1.0)*(lapm1(i,j,k)+1.0)*psiL**6*(E_rad(i,j,k)*u0(i,j,k)*u0(i,j,k)+2.0*F_rad0(i,j,k)*u0&
  &(i,j,k)+P_rad00l)
              S_rad_x(i,j,k) = (lapm1(i,j,k)+1.0)*psiL**6*(E_rad(i,j,k)*u0(i,j,k)*u_x + F_rad0(i,j,k)*u_x + F_rad_xl * u0(i,j,k) + &
  &P_rad00l*shift_x + psiL4*(P_rad0xl*gxx(i,j,k) + P_rad0yl*gxy(i,j,k) + P_rad0zl*gxz(i,j,k)))
              S_rad_y(i,j,k) = (lapm1(i,j,k)+1.0)*psiL**6*(E_rad(i,j,k)*u0(i,j,k)*u_y + F_rad0(i,j,k)*u_y + F_rad_yl * u0(i,j,k) + &
  &P_rad00l*shift_y + psiL4*(P_rad0xl*gxy(i,j,k) + P_rad0yl*gyy(i,j,k) + P_rad0zl*gyz(i,j,k)))
              S_rad_z(i,j,k) = (lapm1(i,j,k)+1.0)*psiL**6*(E_rad(i,j,k)*u0(i,j,k)*u_z + F_rad0(i,j,k)*u_z + F_rad_zl * u0(i,j,k) + &
  &P_rad00l*shift_z + psiL4*(P_rad0xl*gxz(i,j,k) + P_rad0yl*gyz(i,j,k) + P_rad0zl*gzz(i,j,k)))
              rho(i,j,k) = rho(i,j,k) + (lapm1(i,j,k)+1.0)**2.0*(E_rad(i,j,k)*u0(i,j,k)**2 + 2.0 * F_rad0(i,j,k) * u0(i,j,k) + P_ra&
  &d00l)
              ...
              Sxx(i,j,k) = Sxx(i,j,k) + E_rad(i,j,k)*u_x*u_x + 2.0*F_rad_xl*u_x +&
                   ...
                   2.0*(gxx(i,j,k)*gxy(i,j,k)*P_radxy(i,j,k)+gxx(i,j,k)*gxz(i,j,k)*P_radxz(i,j,k)+gxy(i,j,k)*gxz(i,j,k)*P_radyz(i,j&
  &,k)) )
              Syy(i,j,k) = Syy(i,j,k) + E_rad(i,j,k)*u_y*u_y + 2.0*F_rad_yl*u_y +&
                   ...
                   2.0*(gxy(i,j,k)*gyy(i,j,k)*P_radxy(i,j,k)+gxy(i,j,k)*gyz(i,j,k)*P_radxz(i,j,k)+gyy(i,j,k)*gyz(i,j,k)*P_radyz(i,j&
  &,k)) )
              Szz(i,j,k) = Szz(i,j,k) + E_rad(i,j,k)*u_z*u_z + 2.0*F_rad_zl*u_z +&
                   ...
                   2.0*(gxz(i,j,k)*gyz(i,j,k)*P_radxy(i,j,k)+gxz(i,j,k)*gzz(i,j,k)*P_radxz(i,j,k)+gyz(i,j,k)*gzz(i,j,k)*P_radyz(i,j&
  &,k)) )
              Sxy(i,j,k) = Sxy(i,j,k) + E_rad(i,j,k)*u_x*u_y + F_rad_xl*u_y + F_rad_yl*u_x +&
                   shift_x*shift_y*P_rad00l +&
                   psiL4*(shift_x*(gxy(i,j,k)*P_rad0xl + gyy(i,j,k)*P_rad0yl + gyz(i,j,k)*P_rad0zl) + (shift_y*(gxx(i,j,k)*P_rad0xl&
  & + gxy(i,j,k)*P_rad0yl + gxz(i,j,k)*P_rad0zl)) )+&
                   psiL4**2.0*(gxx(i,j,k)*gxy(i,j,k)*P_radxx(i,j,k) + gxy(i,j,k)*gyy(i,j,k)*P_radyy(i,j,k) + gxz(i,j,k)*gyz(i,j,k)*&
  &P_radzz(i,j,k) +&
                   (gxx(i,j,k)*gyy(i,j,k) + gxy(i,j,k)*gxy(i,j,k))*P_radxy(i,j,k) + (gxx(i,j,k)*gyz(i,j,k) + gxz(i,j,k)*gxy(i,j,k))&
  &*P_radxz(i,j,k) + (gxy(i,j,k)*gyz(i,j,k) + gxz(i,j,k)*gyy(i,j,k))*P_radyz(i,j,k))
              Sxz(i,j,k) = Sxz(i,j,k) + E_rad(i,j,k)*u_x*u_z + F_rad_xl*u_z + F_rad_zl*u_x +&
                   shift_x*shift_z*P_rad00l +&
                   psiL4*(shift_x*(gxz(i,j,k)*P_rad0xl + gyz(i,j,k)*P_rad0yl + gzz(i,j,k)*P_rad0zl) + (shift_z*(gxx(i,j,k)*P_rad0xl&
  & + gxy(i,j,k)*P_rad0yl + gxz(i,j,k)*P_rad0zl)) )+&
                   psiL4**2.0*(gxx(i,j,k)*gxz(i,j,k)*P_radxx(i,j,k) + gxy(i,j,k)*gyz(i,j,k)*P_radyy(i,j,k) + gxz(i,j,k)*gzz(i,j,k)*&
  &P_radzz(i,j,k) +&
                   (gxx(i,j,k)*gyz(i,j,k) + gxy(i,j,k)*gxz(i,j,k))*P_radxy(i,j,k) + (gxx(i,j,k)*gzz(i,j,k) + gxz(i,j,k)*gxz(i,j,k))&
  &*P_radxz(i,j,k) + (gxy(i,j,k)*gzz(i,j,k) + gxz(i,j,k)*gyz(i,j,k))*P_radyz(i,j,k))
              Syz(i,j,k) = Syz(i,j,k) + E_rad(i,j,k)*u_y*u_z + F_rad_yl*u_z + F_rad_zl*u_y +&
                   shift_y*shift_z*P_rad00l +&
                   psiL4*(shift_y*(gxz(i,j,k)*P_rad0xl + gyz(i,j,k)*P_rad0yl + gzz(i,j,k)*P_rad0zl) + (shift_z*(gxy(i,j,k)*P_rad0xl&
  & + gyy(i,j,k)*P_rad0yl + gyz(i,j,k)*P_rad0zl)) )+&
                   psiL4**2.0*(gxy(i,j,k)*gxz(i,j,k)*P_radxx(i,j,k) + gyy(i,j,k)*gyz(i,j,k)*P_radyy(i,j,k) + gyz(i,j,k)*gzz(i,j,k)*&
  &P_radzz(i,j,k) +&
                   (gxy(i,j,k)*gyz(i,j,k) + gxz(i,j,k)*gyy(i,j,k))*P_radxy(i,j,k) + (gxy(i,j,k)*gzz(i,j,k) + gyz(i,j,k)*gxz(i,j,k))&
  &*P_radxz(i,j,k) + (gyy(i,j,k)*gzz(i,j,k) + gyz(i,j,k)*gyz(i,j,k))*P_radyz(i,j,k))
              if(isnan(tau_rad(i,j,k)) .and. isnan(S_rad_x(i,j,k))) then
                 write(*,*) "after BHNS write(*,*) initialdata_local2.F90, tau_rad or S_rad_x is nan", tau_rad(i,j,k), S_rad_x(i,j,&
  &k)
                 write(*,*) "E_rad, F_radx, P_radxx, zeta_rad, chi_rad, u_0l", E_rad(i,j,k), F_radx(i,j,k), P_radxx(i,j,k), zeta_ra&
  &d(i,j,k), chi_rad(i,j,k), u_0l
