...
extern "C" void CCTK_FCALL bhns_regridder_out_allgfs_
...
extern "C" void bhns_regridder_out_allgfs(const cGH *cctkGH,int &checksum,
  ...
  double *output = (double *)malloc(sizeof(double)*numpoints*35);
  ...
  int N_input_arrays = 31;
  int N_output_arrays = 31;

  CCTK_INT input_array_indices[31];
  ...
  input_array_indices[31] = CCTK_VarIndex("mhd_evolve::tau_rad");
  input_array_indices[28] = CCTK_VarIndex("mhd_evolve::S_rad_x");
  input_array_indices[29] = CCTK_VarIndex("mhd_evolve::S_rad_y");
  input_array_indices[30] = CCTK_VarIndex("mhd_evolve::S_rad_z");
  ...
  CCTK_INT output_array_type_codes[31];
  //static const CCTK_INT output_array_type_codes[31];
  for(int i=0;i<31;i++) output_array_type_codes[i] = CCTK_VARIABLE_REAL;

  void *output_arrays[31];
  for(int i=0;i<31;i++) output_arrays[i] = (void *) &output[(4+i)*numpoints];
  ...
  if(myproc==0) {
    ...
    printf("Attempting to write 35 gridfunctions (each with %d points) to %s now.\n",numpoints,filename);
    
    ofstream outfile;
    outfile.open (filename, ios::out | ios::binary);

    outfile.write ((char *)&numpoints, sizeof(int));                         // <-- Store the number of points on one grid.
    outfile.write ((char *)output, sizeof(double)*35*numpoints); // <-- Write all grids' worth of data
...
extern "C" void CCTK_FCALL bhns_regridder_in_allgfs_
...
extern "C" void bhns_regridder_in_allgfs(int &numpoints,int &checksum,double *Ax,double *Ay,double *Az,double *psi6phi,
  ...
  infile.read ((char *)tau_rad, sizeof(double)*numpoints);
  infile.read ((char *)S_rad_x, sizeof(double)*numpoints);
  infile.read ((char *)S_rad_y, sizeof(double)*numpoints);
  infile.read ((char *)S_rad_z, sizeof(double)*numpoints);
...
void bhns_interp_driver_carp(const cGH *cctkGH,int &num_points,double *pointcoordsx,double *pointcoordsy,double *pointcoordsz,int &gridfunc_varindex,double *output) {
  ...
  printf ("Start bhns_interp_driver_carp\n!!!");
  ...
  printf ("Before CCTK_InterpGridArrays\n!!!");
  int status =
    CCTK_InterpGridArrays(cctkGH,
                          N_dims,
                          interp_handle, Util_TableCreateFromString("order=4"),
                          ...
                          output_array_type_codes,
                          output_arrays);
  printf ("After CCTK_InterpGridArrays\n!!!");
}
