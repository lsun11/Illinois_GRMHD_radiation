subroutine bhns_initialdata_read_binfile_driver(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbo&
...
! read COCAL ID for NSNS
     ...
     call read_inputfile_cocal(cctkGH, num_CO, piecewise, ergo_star, &
...
!---------------------
! read COCAL ID for black hole-disk systems
  else if(use_new_bhns_initial_data==8) then
!  Notice that the 3-metric is the conformal metric
!  kij is the full extrinsic curvature
!  and the metric is not conformally flat and trk is not zero
     call read_inputfile_cocal_ncf(cctkGH, num_CO, piecewise, ergo_star,  &
           genID_cmdline_output_enable,fisheye_enable,reset_shift_lapse,  &
           X(1,1,1),Y(1,1,1),Z(1,1,1),                                    &
           dX,dY,dZ,cctk_lsh,                                             &
           lapm1,shiftx,shifty,shiftz,                                    &
           gxx,gxy,gxz,gyy,gyz,gzz,                                       &
           gupxx,gupxy,gupxz,gupyy,gupyz,gupzz,                           &
           phi,trK,kxx,kxy,kxz,kyy,kyz,kzz,                               &
           rho_b,vx,vy,vz,                                                &
           nphase, abc, abi, rhoi, tedi)
     write(6,*) "~~~~~~~~~~~~~~~~~~   FROM read_inputfile_driver.F90   ~~~~~~~~~~~~~~~~~~~~~~~~~"
     write(6,*) "Number of phases nphase=", nphase(0)
...
!!           do i=1,ext(1)
!              print *,=================
!              print *,lapm1(i,j,k),gxx(i,j,k),gxy(i,j,k),gxz(i,j,k),gyy(i,j,k),gyz(i,j,k),gzz(i,j,k)
!              print *,=================
!              print *,kxx(i,j,k),kxy(i,j,k),kxz(i,j,k),kyy(i,j,k),kyz(i,j,k),kzz(i,j,k)
!           end do
!        end do
!     end do
!     do k=1,cctk_lsh(3)
!        do j=1,cctk_lsh(2)
!           do i=1,cctk_lsh(1)
!              print *,gxx(i,j,k), gyy(i,j,k),  gzz(i,j,k)
!           end do
!        end do
!     end do
     print *,"**********************************************************************"
     print *,"Cocal ID no conf flat has been read in. We need to compute more stuff ... "
     print *,"**********************************************************************"
!     stop
!---------------------
        ...
        do k=1,cctk_lsh(3)
           do j=1,cctk_lsh(2)
              do i=1,cctk_lsh(1)
                 ...
                 ! We do not know whether the data have trK=0 so we compute trK here, remember Aij is the full extrinsic curvature s
...
! define lapse -1 array
  ...
  if((use_new_bhns_initial_data.eq.3).or.(use_new_bhns_initial_data.eq.4).or.(use_new_bhns_initial_data.eq.6).or.(use_new_bhns_init&
  &ial_data.eq.8)) then
     k_poly=kappa1
     gamma_th=1.+1./gamma1
     ! note that Im assuming that Cocal gives us gamma no n
     if((use_new_bhns_initial_data.eq.4).or.(use_new_bhns_initial_data.eq.6).or.(use_new_bhns_initial_data.eq.8)) gamma_th=gamma1
  endif
...
!        print  *,loc_eps1,2 = before loop,loc_eps(1),loc_eps(2), neos
        ...
        print *, "^^^^^^^^^^^^^^^^^^^^^^ Reading transitons for an Ergo Star ^^^^^^^^^^^^^^^^^^^^^^^^ "
       neos = neos+1
       do ia=1, neos+1
          loc_gamma(ia) = abi(ia)
          loc_kappa(ia) = abc(ia)
...
!       print  *,loc_eps1,2 = before loop,loc_eps(1),loc_eps(2), neos
        do ia=2,neos
          ...
          loc_eps(ia) = loc_eps(ia-1) + (loc_pre(ia)/loc_rho(ia) - loc_pre(ia-1)/loc_rho(ia-1))/(loc_gamma(ia)-1.0d0)
        enddo
        !loc_eps(neos+1) = loc_eps(neos) +  (loc_pre(neos+1)/loc_rho(neos) - loc_pre(neos+1)/loc_rho(neos))/(loc_gamma(neos+1)-1.0d0
      end if
