subroutine BHNS_setup_emfield_part2_local_setBi_after_checkpoint(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from&
  &,cctk_to,cctk_bbox,cctk_delta_time,cctk_time,cctk_delta_space,cctk_origin_space,cctk_levfac,cctk_levoff,cctk_levoffdenom,cctk_ti&
  &mefac,cctk_convlevel,cctk_convfac,cctk_nghostzones,cctk_iteration,cctkGH, X0Axyzintrgroup,X0Tracer_rhs_group,X0Tracergroup,X0las&
  &t_ah_data,X1Axyzintrgroup,X1Tracer_rhs_group,X2Axyzintrgroup,Axintr,Ayintr,Azintr,M0dot_last,Pintr,ah_radii_last,bhnsEthermalNOW&
  &,bhns_rk4_particle_tracer_step,bhnsinsertBNOW,bhnsinsertRADNOW,int_M0dot,local_density,m1_minus_u_lower_zero,rhobintr,t_last,tau&
  &l_rhs,tracer_taul,tracer_x,tracer_xc,tracer_y,tracer_yc,tracer_z,u_x_local,u_y_local,x_rhs,xc_rhs,xh_last,y_rhs,yc_rhs,yh_last,z&
  &_rhs,zh_last, X0BSSN_AH,X0BSSN_aux_private,X0BSSN_aux_restrict2,X0BSSN_diag_restrict,X0BSSN_gupij,X0BSSN_matter,X0BSSN_refbd,X0B&
  ...
  &v02_radl,v02_radr,v02l,v02r,vvec,vx,vxl,vxr,vy,vyl,vyr,vz,vzl,vzr,w,zeta_rad,zeta_rad_nue,zeta_rad_nux, X0coordinates,X0mask,X0r&
  &adii,X0sf_radius,X0space_mask_group,X1coordinates,X1mask,X1sf_radius,X1space_mask_group,X2coordinates,X2mask,X2space_mask_group,&
  &active_length,num_levels_length,positions_length,radii_length,sf_active_length,sf_coordinate_descriptors_length,sf_info_length,s&
  &f_maxreflevel_length,sf_minreflevel_length,sf_origin_length,sf_radius_length,sf_shape_descriptors_length,sf_valid_length,active,&
  &coarse_dx,coarse_dy,coarse_dz,emask,num_levels,position_x,position_y,position_z,r,radius,sf_active,sf_area,sf_centroid_x,sf_cent&
  &roid_y,sf_centroid_z,sf_delta_phi,sf_delta_theta,sf_max_radius,sf_max_x,sf_max_y,sf_max_z,sf_maxreflevel,sf_mean_radius,sf_min_r&
  &adius,sf_min_x,sf_min_y,sf_min_z,sf_minreflevel,sf_nghostsphi,sf_nghoststheta,sf_nphi,sf_ntheta,sf_origin_phi,sf_origin_theta,sf&
  &_origin_x,sf_origin_y,sf_origin_z,sf_quadrupole_xx,sf_quadrupole_xy,sf_quadrupole_xz,sf_quadrupole_yy,sf_quadrupole_yz,sf_quadru&
  &pole_zz,sf_radius,sf_valid,space_mask,x,y,z)
  implicit none
  INTEGER cctk_dim
  ...
  ! Note that if ITERATION_TO_INSERT_MAGNETIC_FIELDS is set to 0, we will instead just use the regular BHNS_setup_emfield_part2_loca
  ...
  &You have to write your own initial data for the toroidal configuration."
     ...
     if(unequalmass==1) then
        write(*,*) "INSIDE EMFIELDS SETUP: Pmax2:",bhns_P_max2
     end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ! FIRST, WE SET UP MAGNETIC FIELDS ON _P LEVEL:
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ! The magnetic fields are set based on pressure. We dont have P_p or P_p_p, so we must compute that using the primitives solver
  ...
  &, T_fluid,&
             ...
             primitives_debug,Psi6threshold,rad_const,horizon_enforce_rho_profile,enable_OS_collapse, rad_evolve_enable,compute_mic&
  &rophysics, &
     ...
     write(*,*) "1. Done primitives_generic!!!!!!!!!!!!!!!!!1"
     !
     ...
     if (em_field_type .ne. 1) then
        ...
        call BHNS_compute_Aphi(ext,X,Y,Z,PhysicalRadius,P,A_phi,Ax_p,Ay_p,Az_p,  &
             ...
             phi_p,Con_Pol,Con_Tor,superposition,r0)
     end if
     !
     ...
     write(*,*) "1. Done BHNS_compute_A_toroidal_constrained_transport_eq_3!!!!!!!!!!!!!!!!!1"
     ! Compute B^is based on values of A_is.  
     ...
     write(*,*) "1. Done reset_rhob_atm_f90!!!!!!!!"
     ! Set the st_is:
     st_x = mhd_st_x_p
     st_y = mhd_st_y_p
     st_z = mhd_st_z_p
     ...
     ! Reset magnetic fields to zero, so that in the below primitives solver call, the conservatives & primitives are consistent wit
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  &, T_fluid,&
             neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab, &
             temp1,temp2,temp3,temp4,temp5, &
             primitives_debug,Psi6threshold,rad_const,horizon_enforce_rho_profile, enable_OS_collapse, rad_evolve_enable,compute_mi&
  &crophysics, &
             microphysics_scheme, T_fluid_cgs_atm, rad_fix)
     ...
     if (em_field_type .ne. 1) then
        call BHNS_compute_Aphi(ext,X,Y,Z,PhysicalRadius,P,A_phi,Ax_p_p,Ay_p_p,Az_p_p, &
             ...
             use_new_bhns_initial_data,x_NS_CoM_coord2,y_NS_CoM_coord2,               &
             bhns_P_max2,unequalmass,betam1_2,rloop2,Iloop2,conloop1,conloop2,        &
             phi_p_p,Con_Pol,Con_Tor,superposition,r0)
     end if
     !
     ! Compute the vector potential Ax and Ay for near toroidal configuration
     !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  &crophysics, &
             microphysics_scheme, T_fluid_cgs_atm, rad_fix)
        write(*,*) "3. Done primitives_generic!!!!!!!!!"
        !SKIP COMPUTATION OF METRIC SOURCE TERMS.  THAT WOULD BE UNNECESSARY!
     else
        write(*,*) "ERROR. Primitives solver != 11 NOT ALLOWED when setting B fields from checkpoint."
        stop
     ...
     do k=1,cctk_lsh(3)
        do j=1,cctk_lsh(2)
           do i=1,cctk_lsh(1)
              ...
              rho_match  = (0.5d0*b2(i,j,k)*beta_ext/kloc)**(1.d0/gamma_tab(1))
              if (rho_b(i,j,k) <= rho_match) then
                 ...
                 if (atm_type==0) then
                    ...
                    !P(i,j,k)     = kloc*rho_b(i,j,k)*rho_b(i,j,k)
                    rho_b(i,j,k) = (beta_ext*0.5d0*b2(i,j,k)/kloc)**(1.d0/gamma_tab(1))
                    P(i,j,k)     = kloc*rho_b(i,j,k)**gamma_tab(1)
                 end if
