...
extern "C" void CCTK_FCALL read_inputfile_cocal_ncf_
    ...
    else
{
	...
	for(int i=0;i<nx;i++){
	  ...
	  double g= -g_xz*g_xz*g_yy + 2*g_xy*g_xz*g_yz - g_xx*g_yz*g_yz 
	            -g_xy*g_xy*g_zz + g_xx*g_yy*g_zz;
	  ...
	  lapse[ind] = sqrt((-g_tt + g_xx*shiftx[ind]*shiftx[ind] + g_yy*shifty[ind]*shifty[ind] +  g_zz*shiftz[ind]*shiftz[ind]
	  + 2.*g_xy*shiftx[ind]*shifty[ind] + 2.*g_xz*shiftx[ind]*shiftz[ind]
			      + 2.*g_yz*shifty[ind]*shiftz[ind]));
