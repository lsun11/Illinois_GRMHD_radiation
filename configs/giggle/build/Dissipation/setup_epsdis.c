/* $Header: /numrelcvs/AEIThorns/Dissipation/src/setup_epsdis.c,v 1.9 2008/02/19 04:37:35 schnetter Exp $ */
...
setup_epsdis (CCTK_ARGUMENTS)
{
  ...
  if (epsdis_for_level[reflvl] > 0.0) {
    ...
    for(k=0;k<nk;k++)
      for(j=0;j<nj;j++) 
	for(i=0;i<ni;i++) {
	  epsdisA[CCTK_GFINDEX3D(cctkGH,i,j,k)] = epsdis_for_level[reflvl];
  ...
  for(k=0;k<nk;k++)
    for(j=0;j<nj;j++) 
      for(i=0;i<ni;i++) {
	epsdisA[CCTK_GFINDEX3D(cctkGH,i,j,k)]=epsdis;
	if(KO_strength_falloff_enable==1) {
	  //if(r[CCTK_GFINDEX3D(cctkGH,i,j,k)]>KO_strength_falloff_radius) {
	  epsdisA[CCTK_GFINDEX3D(cctkGH,i,j,k)] = (-erf((r[CCTK_GFINDEX3D(cctkGH,i,j,k)]-KO_strength_falloff_radius-KO_strength_falloff_dr*2.0)/KO_strength_falloff_dr)*(epsdis-KO_strength_final_value)*0.5
						   + (epsdis-KO_strength_final_value)*0.5 + KO_strength_final_value);
	}
      }
  }
  ...
  {
    printf("I should not be here, unless you've set extra_dissipation_at_outerbound=yes!\n");
  ...
  {
    printf("I should not be here, unless you've set extra_dissipation_in_horizon=yes!\n");
    if (verbose) {
