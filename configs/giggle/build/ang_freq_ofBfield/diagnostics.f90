...
!-----------------------------------------------------------------------------
subroutine ang_freq_ofBfield_diagnostics(cctk_dim,cctk_gsh,cctk_lsh,cctk_lbnd,cctk_ubnd,cctk_lssh,cctk_from,cctk_to,cctk_bbox,cctk_&
  ...
  REAL*8  radius_om(101)
  INTEGER*4 num_interp_radii_om
  INTEGER*4 num_theta
  COMMON /ang_freq_ofBfieldpriv/radius_om,num_interp_radii_om,num_theta
  REAL*8  CCTKH0
  REAL*8  dcostheta
  REAL*8  ddrbddr
  REAL*8  dphi
...
31         FORMAT("B_ang_freq.fon",I1)
        ...
        do i=1,num_theta
               ...
               export_data(i,2) = pointcoords(i,1) - bh_posn_x(1)
               export_data(i,3) = pointcoords(i,2) - bh_posn_y(1)
               export_data(i,4) = pointcoords(i,3) - bh_posn_z(1)
               export_data(i,5) = Bfreq1int(i)
               export_data(i,6) = Bfreq2int(i)
          end do
          if(CCTK_MyProc(CCTKGH)==0) call output_multiline_data_to_file(filename,num_cols,data_headers,num_theta,export_data,header&
  &_flag)
     end do
     !--------------------------------------------------------------------------------!
  end if
