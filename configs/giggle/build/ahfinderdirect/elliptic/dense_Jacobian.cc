...
// $Header: /numrelcvs/AEIThorns/AHFinderDirect/src/elliptic/dense_Jacobian.cc,v 1.8 2003/08/18 10:01:02 jthorn Exp $
 ...
 */
integer CCTK_FCALL
  isamax_(const integer* N, const float SX[], const integer* incx);
integer CCTK_FCALL
  idamax_(const integer* N, const double DX[], const integer* incx);

/*
 * ***** LAPACK *****
 */
void CCTK_FCALL
  sgesv_(const integer* N, const integer* NRHS,
		    float A[], const int* LDA,
		    integer IPIV[],
		    float B[], const integer* LDB, integer* info);
void CCTK_FCALL
  dgesv_(const integer* N, const integer* NRHS,
...
void CCTK_FCALL
  sgecon_wrapper_(const integer* norm_int,
			     const integer* N,
			     ...
			     integer* info);
void CCTK_FCALL
  dgecon_wrapper_(const integer* norm_int,
			     const integer* N,
...
// ... max_posn = 1-origin index of A[] element with largest absolute value
  ...
  const integer max_posn = isamax_(&N2, A, &stride);
#elif defined(FP_IS_DOUBLE)
  const integer max_posn = idamax_(&N2, A, &stride);
#else
...
integer info;
#if   defined(FP_IS_FLOAT)
  sgesv_(&N, &NRHS, A, &N, pivot_, x, &N, &info);
#elif defined(FP_IS_DOUBLE)
  dgesv_(&N, &NRHS, A, &N, pivot_, x, &N, &info);
#else
  #error "don't know fp datatype!"
#endif
...
fp rcond;
#if   defined(FP_IS_FLOAT)
  sgecon_wrapper_(&infinity_norm_flag,
			     &N, A, &N, &A_infnorm, &rcond,
			     rwork_, iwork_, &info);
#elif defined(FP_IS_DOUBLE)
  dgecon_wrapper_(&infinity_norm_flag,
			     &N, A, &N, &A_infnorm, &rcond,
			     rwork_, iwork_, &info);
#else
  #error "don't know fp datatype!"
