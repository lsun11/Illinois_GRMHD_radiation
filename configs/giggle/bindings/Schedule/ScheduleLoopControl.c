/*@@
   @file       ScheduleLoopControl.c
   @author     Automatically generated by CreateScheduleBindings.pl
   @desc
               Creates the schedule and parameter recovery bindings 
               for thorn LoopControl
   @enddesc
@@*/

#define THORN_IS_LoopControl

#include "cctk.h"
#include "cctk_Parameters.h"
#include "cctki_ScheduleBindings.h"

/* prototypes for schedule bindings functions to be registered */
/* Note that this is a cheat, we just need a function pointer. */
extern int lc_printstats(void);
extern int lc_demo(void);


void CCTKi_BindingsSchedule_LoopControl(void);
void CCTKi_BindingsSchedule_LoopControl(void)
{
  DECLARE_CCTK_PARAMETERS
if (printstats) {
  {
    int cctkschedulei_tlevelarray[] = {0};
    CCTKi_ScheduleFunction((void *)lc_printstats,
                           "lc_printstats",
                           "LoopControl",
                           "LoopControl",
                           "Output loop control statistics",
                           "CCTK_TERMINATE",
                           "C",
                           0,  /* Number of STORAGE  groups   */
                           0,  /* Number of COMM     groups   */
                           0,  /* Number of TRIGGERS groups   */
                           0,  /* Number of SYNC     groups   */
                           0,  /* Number of Options           */
                           0,  /* Number of BEFORE  routines  */
                           0,  /* Number of AFTER   routines  */
                           0,  /* Number of WHILE   variables */
                           0,  /* Number of IF   variables */
                           cctkschedulei_tlevelarray  /* Array of timelevel data for storage groups */);
  }
}
if (run_demo)
{
  {
    int cctkschedulei_tlevelarray[] = {0};
    CCTKi_ScheduleFunction((void *)lc_demo,
                           "lc_demo",
                           "LoopControl",
                           "LoopControl",
                           "Run embedded wavetoy as a LoopControl demo application",
                           "CCTK_STARTUP",
                           "C",
                           0,  /* Number of STORAGE  groups   */
                           0,  /* Number of COMM     groups   */
                           0,  /* Number of TRIGGERS groups   */
                           0,  /* Number of SYNC     groups   */
                           0,  /* Number of Options           */
                           0,  /* Number of BEFORE  routines  */
                           0,  /* Number of AFTER   routines  */
                           0,  /* Number of WHILE   variables */
                           0,  /* Number of IF   variables */
                           cctkschedulei_tlevelarray  /* Array of timelevel data for storage groups */);
  }
}
}

/*@@
  @routine    CCTKi_BindingsParameterRecovery_LoopControl
  @author     Automatically generated by CreateScheduleBindings.pl
  @desc
              Creates the parameter recovery bindings for thorn LoopControl
  @enddesc
@@*/

int CCTKi_BindingsParameterRecovery_LoopControl(void);
int CCTKi_BindingsParameterRecovery_LoopControl(void)
{
  /* this thorn doesn't define any parameter recovery routines */
  return (0);
}

