...
void thc_sph_grid_free(SphericalGrid * grid) {
    free(grid);
}

void thc_sph_grid_get_origin(SphericalGrid const * grid, CCTK_REAL origin[3]) {
...
void thc_sph_grid_get_delta(
        ...
        CCTK_REAL * dtheta,
        CCTK_REAL * dphi) {
    *dr = thc_sph_grid_get_dr(grid);
    *dtheta = thc_sph_grid_get_dtheta(grid);
    *dphi = thc_sph_grid_get_dphi(grid);
}
...
CCTK_REAL thc_sph_grid_get_r(SphericalGrid const * grid, int const i) {
    assert(grid);
    assert(i >= 0 && i < grid->nrad);
    return grid->dr*i;
}
CCTK_REAL thc_sph_grid_get_theta(SphericalGrid const * grid, int const i) {
    assert(grid);
    assert(i >= 0 && i < grid->nray);
    return grid->dtheta * (1.0 + 0.5*(grid->endpoint ? 0 : 1)) *
        thc_sph_grid_get_itheta(grid, i);
}
CCTK_REAL thc_sph_grid_get_phi(SphericalGrid const * grid, int const i) {
    assert(grid);
    assert(i >= 0 && i < grid->nray);
    return grid->dphi * thc_sph_grid_get_iphi(grid, i);
}
void thc_sph_grid_get_r_theta_phi(
        ...
        CCTK_REAL * theta,
        CCTK_REAL * phi) {
    *r     = thc_sph_grid_get_r(grid, irad);
    *theta = thc_sph_grid_get_theta(grid, iray);
    *phi   = thc_sph_grid_get_phi(grid, iray);
}
void thc_sph_grid_get_x_y_z(
    ...
    thc_sph_grid_get_r_theta_phi(grid, irad, iray, &r, &theta, &phi);
    thc_coord_sph_to_cart(r, theta, phi, x, y, z);
    *x += grid->origin[0];
    *y += grid->origin[1];
    *z += grid->origin[2];
}
