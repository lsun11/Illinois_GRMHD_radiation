...
subroutine BHNS_compute_Aphi(ext,X,Y,Z,PhysR,P,A_phi,Ax,Ay,Az, & 
  ...
  real*8                                         :: Sym_Bz,P_corner,hdX,hdY,hdZ, sintheta,costheta, Ax_temp, Ay_temp, Az_temp, x_before_rot

  real*8, parameter :: Aphi_power = 1.d0
  !ROTATED:
  real*8, parameter :: angle_to_rotate_mag_fields = 0.78539816339744830961 ! = pi/4
  !UNROTATED:
!  real*8, parameter :: angle_to_rotate_mag_fields = 0.d0
  !
     ...
     do j = 1,ext(2)
	   ...
	   if (constrained_transport_scheme==3) then 
	      ...
	      Ax(i,j,k) = -(yp-y_NS_CoM_coord)/pomega2*Aphi_s
              ...
              if(angle_to_rotate_mag_fields.gt.0.D0) then
                 ! Rotate backwards to get x_before_rot, then recompute Aphi_s
                 costheta = cos(-angle_to_rotate_mag_fields)
                 sintheta = sin(-angle_to_rotate_mag_fields)
                 x_before_rot = costheta*(xp-x_NS_CoM_coord) + sintheta*zp

                 ! Note that x_before_rot shifts by x_NS_CoM_coord already:
                 pomega2 = x_before_rot**2 + (yp-y_NS_CoM_coord)**2 + hdX*1.d-13
                 ! We assume that the star is spherical, so the pressure (P_s) depends only on the radius:
                 !    Otherwise, we would need to use the pressure at the coords (x_before_rot,yp,zp)
                 Aphi_s = pomega2 * fac * max(P_s/bhns_P_max - p_c, 0.d0)**Aphi_power
	      ...
	      P_s = 0.25d0*( P(i,j,k) + P(ip1,j,k) + P(i,j,kp1) + P(ip1,j,kp1) )
              ...
              if(angle_to_rotate_mag_fields.gt.0.D0) then
                 ...
                 ! Note that x_before_rot accounts for x_NS_CoM_coord already:
                 Ay(i,j,k) = (x_before_rot)/pomega2*Aphi_s
              end if
		 ...
		 !Aphi_s = pomega2 * fac * max((P(i,j,k)/bhns_P_max - p_c), 0.d0)**1
                 Aphi_s = pomega2 * fac * max((P(i,j,k)/bhns_P_max - p_c), 0.d0)**2
		 ...
		 mhd_u_chi(i,j,k) = 0.d0
           ...
           elseif (em_field_type==0 .or. i==1 .or. j==1 .or. k==1) then 
              ...
              A_phi(i,j,k) = pomega2 * fac * max(P(i,j,k)/bhns_P_max - p_c, 0.d0)**2
              if (enable_trace_field_line==1) then
                 phi = atan2(Y(i,j,k)-y_NS_CoM_coord,X(i,j,k)-x_NS_CoM_coord)
	      ...
	      !A_phi(i,j,k) = pomega2 * fac * max(P_corner/bhns_P_max - p_c, 0.d0)**1
              A_phi(i,j,k) = pomega2 * fac * max(P_corner/bhns_P_max - p_c, 0.d0)**2
