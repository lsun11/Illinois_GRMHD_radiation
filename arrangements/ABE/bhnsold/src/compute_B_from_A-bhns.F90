...
subroutine bhns2_compute_B_from_A(CCTK_ARGUMENTS)
  implicit none
  ...
  integer               :: A_BC_rad_mag_bondi_enable_dummy
  A_BC_rad_mag_bondi_enable_dummy = 0
...
!!$     do j=1,cctk_lsh(2)
!!$        do i=1,cctk_lsh(1)
!!$           if(rho_b(i,j,k).lt.rho_b_atm*100.D0) then
!!$              ! To see this function, go to gnuplot and type: p [0:1] (erf(x*6-3)+1)*.5
!!$              Ax(i,j,k) = 1.D-14
!!$              Ay(i,j,k) = 1.D-14
!!$              Az(i,j,k) = 1.D-14
!!$!              Ax(i,j,k) = Ax(i,j,k)*(erf((rho_b(i,j,k)/(rho_b_atm*1000.D0))*6.D0-3.D0)+1.D0)*0.5D0
!!$!              Ay(i,j,k) = Ay(i,j,k)*(erf((rho_b(i,j,k)/(rho_b_atm*1000.D0))*6.D0-3.D0)+1.D0)*0.5D0
!!$!              Az(i,j,k) = Az(i,j,k)*(erf((rho_b(i,j,k)/(rho_b_atm*1000.D0))*6.D0-3.D0)+1.D0)*0.5D0
!!$           end if
!!$        end do
!!$     end do
!!$  end do
!!$  !$omp end parallel do
  ...
  elseif (Symmetry==EQUATORIAL) then
	...
	Ay_syms(3) = -1
     ...
     ! 
     if (A_BC_rad_mag_bondi_enable_dummy==1) then 
	Ax_syms = 1
	Ay_syms = 1
	Az_syms = 1
  ...
  if (Symmetry==EQUATORIAL .and. A_BC_rad_mag_bondi_enable_dummy==1) then 
...
end subroutine bhns2_compute_B_from_A

subroutine bhns2_mhd_do_symmetry_gz_one_gridfunction_stagger(ext,X,Y,Z,Symmetry,gridfunc,gridfunc_syms, & 
...
end subroutine bhns2_mhd_do_symmetry_gz_one_gridfunction_stagger

subroutine bhns2_apply_bfield_bc_ijkmin(ext,Bx,By,Bz,EM_BC)
  implicit none
...
end subroutine bhns2_apply_bfield_bc_ijkmin
