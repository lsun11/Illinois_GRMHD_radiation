...
subroutine J_surf_integral_offcenter_spin(cctkGH,xcenter,ycenter,zcenter,r_local,output_integral)
  implicit none
  ...
  real*8                                   :: output_integral,r_local
...
!  call CCTK_InterpHandle (interp_handle, "uniform cartesian")
  ...
  do i=1,N_theta
     ...
     do j=1,N_phi
        ...
        xinterp(n) = r_local*sintheta*cos(phiangle)+xcenter
        yinterp(n) = r_local*sintheta*sin(phiangle)+ycenter
        zinterp(n) = r_local*costheta+zcenter
        n = n + 1
     end do
  end do
  ...
  if(ierr.ge.0) then
     ...
     do i=1,N_interp_points
        nxp= (xinterp(i)-xcenter)/r_local
        nyp= (yinterp(i)-ycenter)/r_local
        nzp= (zinterp(i)-zcenter)/r_local
     ...
     output_integral = output_integral * r_local * r_local * dphi * dcostheta / PI*sym_factor 
  else
     output_integral = -9999.D0
  end if

end subroutine J_surf_integral_offcenter_spin
