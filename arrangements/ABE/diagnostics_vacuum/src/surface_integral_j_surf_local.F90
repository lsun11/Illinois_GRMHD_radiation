...
subroutine J_surf_integral_spin(cctkGH,ntheta,nphi,x_cm,y_cm,z_cm,r_local,output_integral)
  implicit none
...
!  call CCTK_InterpHandle (interp_handle, "uniform cartesian")
  ...
  do i=1,ntheta
     ...
     do j=1,nphi
        ...
        if(Nphi_points==1) phiangle = 0.D0
        xinterp(n) = r_local*sintheta*cos(phiangle)+x_cm
        yinterp(n) = r_local*sintheta*sin(phiangle)+y_cm
        zinterp(n) = r_local*costheta+z_cm
        n = n + 1
     end do
  end do
