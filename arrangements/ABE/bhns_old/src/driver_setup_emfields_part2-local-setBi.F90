...
!!$end subroutine BHNS_compute_Aphi
  ...
  subroutine BHNS_compute_Aphi(ext,X,Y,Z,PhysR,P,A_phi,Ax,Ay,Az,           & 
    ...
    real*8 					   :: angle_to_tilt_magnetic_fields,Aphi_power,a0tmp,I0tmp,bhns_P_max_tmp,fac_tmp
             ...
             if (constrained_transport_scheme==3) then 
                ...
                if(angle_to_tilt_magnetic_fields.gt.0.D0) then
                   ...
                   if (em_field_type.eq.0) then
                      ...
                      if((unequalmass.eq.1).and.(bhns_P_max2.lt.bhns_P_max)) then
                         print *, "unequal NSNS systems do not support tilted Bfield "
                         stop
                      end if

                   else if (em_field_type.eq.4) then
                      write(*,*) "Stopping: em_field_type=4 not supported for angle_to_tilt_magnetic_fields > 0, yet"
                      exit
                   end if
