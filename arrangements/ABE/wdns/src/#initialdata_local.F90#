...
subroutine wdns_initialdata_local(CCTK_ARGUMENTS)
  implicit none
  ...
  phi=log(phi)
    ...
    if (lorene_init.eq.0) then	
	  ...
	  Axx= Axx*psi**(-4)    ! The lorene data give K_ij which assuming K=0 is K_ij = A_ij, the BSSN \tilde A_ij = \psi^{-4} A_{ij}
	  Axy= Axy*psi**(-4)
	  Axz= Axz*psi**(-4)
	  Ayy= Ayy*psi**(-4)
	  Ayz= Ayz*psi**(-4)
...
!     Azz=Azz*psi**(-6)
  ...
  !The following chunk of code MUST come AFTER Aij is set !
  ...
  if(Symmetry==AXISYM) then
     write(*,*) "I PITY DA FOO TRIES TO USE AXISYMMETRY FOR ORBITING UNLIKE STARS!"
     stop
  end if

end subroutine wdns_initialdata_local
