...
!-----------------------------------------------------------------------------
subroutine wdns_diagnostics(CCTK_ARGUMENTS)
...
!!$           write(*,*) 'In wdns, ntot_bhns = ',ntot_bhns
...
!!$        end if
     ...
     ! J_ADM volume integral is computed in scheduler of diagnostics_vacuum
     data_headers(15) = "J_ADM_Vol"
     export_data(15) = J_ADM_VolInt*SymmFactor
...
!!$        M0dot_last = M0dot
!!$     end if
!!$     data_headers(9) = "int_M0dot"
!!$     export_data(9) = int_M0dot
...
!!$     if(CCTK_MyProc(CCTKGH)==0) call output_data_to_file(filename,num_cols,data_headers,export_data,header_flag)
     ...
     export_data(6) = Box1denom_VolInt1

     data_headers(7) = 'M0_star_2'
     export_data(7) = Box1denom_VolInt2
     ...
     do jj=1,moncrief_gw_num_radii
        ...
        if(moncrief_gw_num_radii.eq.1) then
           filename = 'wdns-evenmodes.wave'
        else
           write(filename,41)jj
41         FORMAT('wdns-evenmodes.wave.',I1)
        endif
        ...
        if(moncrief_gw_num_radii.eq.1) then
           filename = 'wdns-oddmodes.wave'
        else
           write(filename,51)jj
51         FORMAT('wdns-oddmodes.wave.',I1)
        endif
        num_cols = 2*nmodes_moncrief + 1
...
end subroutine wdns_diagnostics
