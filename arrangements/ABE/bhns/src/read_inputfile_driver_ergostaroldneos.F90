	...
	do ia=1, neos+1
	  ...
	  loc_rho(ia) = rhoi(ia)
       ...
       enddo
        ...
        !loc_pre(neos+2) =  loc_kappa(neos+1)*(loc_rho(neos+2)**loc_gamma(neos+1))
...
!       print  *,"loc_eps1,2 = before loop",loc_eps(1),loc_eps(2), neos
	...
	!loc_eps(neos+2) = loc_eps(neos+1) +  (loc_pre(neos+2)/loc_rho(neos+1) - loc_pre(neos+2)/loc_rho(neos+1))/(loc_gamma(neos+2)-1.0d0)
