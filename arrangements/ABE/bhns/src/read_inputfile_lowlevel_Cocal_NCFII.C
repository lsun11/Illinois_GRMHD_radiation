...
//    printf("./read_bin_ns %.16e %.16e %.16e %.16e %.16e %.16e %d %d %d %d $ascii $order\n",xmin,ymin,zmin,dx,dy,dz,ext[0],ext[1],ext[2],filename_checksum);
  ...
  else 
  {
	...
	for(int i=0;i<nx;i++){
	  ...
	  if (rhoo > 1.e-16){
	    double v2 = gxxl*Uupx*Uupx + gyyl*Uupy*Uupy  + gzzl*Uupz*Uupz + 2.*gxyl*Uupx*Uupy + 2.*gxzl*Uupx*Uupz  + 2.*gyzl*Uupz*Uupy; 
  ...
  cout << " Done with read_inputfile_lowlevel_Cocal_ncf.C "<<endl;
}
