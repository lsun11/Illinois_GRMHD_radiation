...
!!       where(((rho_b - rho_tab(ia))*(rho_b - rho_tab(ia+1))).le.0.0d0) 
     ...
     elsewhere((rho_b - rho_tab(neos+1))/rho_tab(neos+1).ge.1.0d-15)   ! inner core

    
      P = (ergo_sigma*((1.0d0+eps_tab(neos+1)+P_tab(neos+1)/rho_tab(neos+1))/rho_tab(neos+1)**ergo_sigma)*rho_b**(ergo_sigma+1) + P_tab(neos+1) - ergo_sigma*(rho_tab(neos+1)*(eps_tab(neos+1)+1)))/(ergo_sigma+1)
     
      h = ((1.0d0+eps_tab(neos+1)+P_tab(neos+1)/rho_tab(neos+1))/rho_tab(neos+1)**ergo_sigma)*rho_b**ergo_sigma
