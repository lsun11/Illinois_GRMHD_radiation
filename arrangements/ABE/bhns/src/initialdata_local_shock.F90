...
if (i==4.and.j==4.and.k==4) then
    ...
    write(*,*) "E_rad and F_radx are", E_rad(i,j,k), F_radx(i,j,k)
		...
		if (rad_evolve_enable .eq. 1) then
                   tau_rad(i,j,k) = (E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*u0(i,j,k) + 2.0*u0(i,j,k)*F_rad0(i,j,k)-P_rad(i,j,k)
                   S_rad_x(i,j,k) = (E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*u0(i,j,k)*vx(i,j,k) + F_rad0(i,j,k)*u0(i,j,k)*vx(i,j,k) + F_radx(i,j,k)*u0(i,j,k)
                   S_rad_y(i,j,k) = (E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uy_l + F_rad0(i,j,k)*uy_l + F_rady(i,j,k)*u0(i,j,k)
                   S_rad_z(i,j,k) = (E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uz_l + F_rad0(i,j,k)*uz_l + F_radz(i,j,k)*u0(i,j,k)
		else 
		   tau_rad(i,j,k) = 0.d0   
		   S_rad_x(i,j,k) = 0.d0
		   S_rad_y(i,j,k) = 0.d0
