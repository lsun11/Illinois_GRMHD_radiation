...
extern "C" void bhns_regridder_out_allgfs(const cGH *cctkGH,int &checksum,
  ...
  vi=CCTK_VarIndex("mhd_evolve::Bxtilde");bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  // Restagger Ay:
  xoff=0.5*dx_interp;yoff=0.0*dy_interp;zoff=0.5*dz_interp;
  whichpoint=0; for(int k=0;k<Nz_interp;k++) for(int j=0;j<Ny_interp;j++) for(int i=0;i<Nx_interp;i++) {
  	x_interp[whichpoint] = xmin_interp + i*dx_interp+xoff; y_interp[whichpoint] = ymin_interp + j*dy_interp+yoff; z_interp[whichpoint] = zmin_interp + k*dz_interp+zoff; whichpoint++; }
  vi=CCTK_VarIndex("mhd_evolve::Bytilde"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  ...
  whichpoint=0; for(int k=0;k<Nz_interp;k++) for(int j=0;j<Ny_interp;j++) for(int i=0;i<Nx_interp;i++) {
  	x_interp[whichpoint] = xmin_interp + i*dx_interp+xoff; y_interp[whichpoint] = ymin_interp + j*dy_interp+yoff; z_interp[whichpoint] = zmin_interp + k*dz_interp+zoff; whichpoint++; }
  vi=CCTK_VarIndex("mhd_evolve::Bztilde"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  // Restagger psi6phi:
  xoff=0.5*dx_interp;yoff=0.5*dy_interp;zoff=0.5*dz_interp;
  whichpoint=0; for(int k=0;k<Nz_interp;k++) for(int j=0;j<Ny_interp;j++) for(int i=0;i<Nx_interp;i++) {
  	x_interp[whichpoint] = xmin_interp + i*dx_interp+xoff; y_interp[whichpoint] = ymin_interp + j*dy_interp+yoff; z_interp[whichpoint] = zmin_interp + k*dz_interp+zoff; whichpoint++; }
  vi=CCTK_VarIndex("mhd_evolve::Blagrangemultiplier"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  ...
  vi=CCTK_VarIndex("mhd_evolve::rho_star");bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  whichpoint=0; for(int k=0;k<Nz_interp;k++) for(int j=0;j<Ny_interp;j++) for(int i=0;i<Nx_interp;i++) {
	...
	whichpoint++; }
  vi=CCTK_VarIndex("mhd_evolve::tau"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  whichpoint=0; for(int k=0;k<Nz_interp;k++) for(int j=0;j<Ny_interp;j++) for(int i=0;i<Nx_interp;i++) {
	x_interp[whichpoint] = xmin_interp + i*dx_interp; y_interp[whichpoint] = ymin_interp + j*dy_interp; z_interp[whichpoint] = zmin_interp + k*dz_interp; 
	if(i==42 && j==39 && k==4) printf("interped tau: %.15e\n",output[whichpoint]);
	whichpoint++; }
  vi=CCTK_VarIndex("mhd_evolve::mhd_st_x"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("mhd_evolve::mhd_st_y"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("mhd_evolve::mhd_st_z"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  vi=CCTK_VarIndex("shift::shiftx"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("shift::shifty"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("shift::shiftz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("lapse::lapm1"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  vi=CCTK_VarIndex("bssn::phi"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output);  if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::chi"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::trK"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  vi=CCTK_VarIndex("bssn::gxx"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::gxy"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::gxz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::gyy"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::gyz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::gzz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  vi=CCTK_VarIndex("bssn::Axx"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::Axy"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::Axz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::Ayy"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::Ayz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::Azz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  vi=CCTK_VarIndex("bssn::Gammax"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::Gammay"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);
  vi=CCTK_VarIndex("bssn::Gammaz"); bhns_interp_driver_carp(cctkGH,numpoints,x_interp,y_interp,z_interp,vi,output); if(myproc==0) bhns_regridder_out_1gf(numpoints,output,checksum);

  numpoints=1;
  if(myproc==0) bhns_regridder_out_1gf(numpoints,&rho_b_atm,checksum);
  if(myproc==0) bhns_regridder_out_1gf(numpoints,&tau_atm,checksum);
