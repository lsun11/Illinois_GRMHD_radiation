...
subroutine BHNS_setup_emfield_part2_local_setBi(CCTK_ARGUMENTS)
  ...
  if(em_field_type==1) then
     write(*,*) "Sorry, em_field_type==1 (toroidal fields) not yet supported."
     stop
  end if
  ...
  !
  call BHNS_compute_Aphi(ext,X,Y,Z,PhysicalRadius,P,A_phi,Ax,Ay,Az,betam1, & 
       p_c,x_NS_CoM_coord,y_NS_CoM_coord,bhns_P_max,Sym_Bz, & 
...
!------------------------------------------------------------------------
subroutine BHNS_compute_Aphi(ext,X,Y,Z,PhysR,P,A_phi,Ax,Ay,Az,betam1, & 
     p_c,x_NS_CoM_coord,y_NS_CoM_coord,bhns_P_max,Sym_Bz, & 
     constrained_transport_scheme,em_field_type)
  implicit none
  integer, dimension(3)                          :: ext
  ...
  do k = 1,ext(3)
     do j = 1,ext(2)
	   ...
	   if (constrained_transport_scheme==3) then 
	      ...
	      Aphi_s = pomega2 * fac * max(P_s/bhns_P_max - p_c, 0.d0)
	      ...
	      A_phi(i,j,k) = 0.d0
           ...
           elseif (em_field_type==0 .or. i==1 .or. j==1 .or. k==1) then 
              A_phi(i,j,k) = pomega2 * fac * max(P(i,j,k)/bhns_P_max - p_c, 0.d0)
	      ...
	      zp = Z(i,j,k) - hdZ
              !!pomega2 = xp**2 + yp**2 + hdX*1.d-13
              pomega2 = (xp-x_NS_CoM_coord)**2 + (yp-y_NS_CoM_coord)**2 + hdX*1.d-13
	      A_phi(i,j,k) = pomega2 * fac * max(P_corner/bhns_P_max - p_c, 0.d0)
