...
subroutine shocktest_rest_mass_integrand1d(CCTK_ARGUMENTS)
  implicit none
  ...
  Shock_Which_Int = 1001

  !-----------------------------------------------------------------------------
  ! Compute integrand
  !-----------------------------------------------------------------------------
  shock_rest_mass_integrand = 0.D0
  shock_restmass_analytic_initial_integrand = 0.D0
  shock_restmass_analytic_integrand = 0.D0
  ...
  do i = 1,cctk_lsh(1)
     ...
     if(cctk_iteration==0) shock_restmass_analytic_initial_integrand(i,j,k) = rho_shock_analytic(i,j,k)*fudgefactor

     u0_analytic = 1.D0 / sqrt( 1.D0 - vx_shock_analytic(i,j,k)*vx_shock_analytic(i,j,k) )
     shock_restmass_analytic_integrand(i,j,k) = u0_analytic * rho_shock_analytic(i,j,k)*fudgefactor

     !write(*,*) "HI.",shock_rest_mass_integrand(i,j,k),shock_restmass_analytic_initial_integrand(i,j,k)
  end do ! i-loop

  !  write(*,*) "HI",shock_restmass_analytic_initial_integrand(:,4,4)
  !  write(*,*) "HI",rho_star(:,4,4)

end subroutine shocktest_rest_mass_integrand1d
