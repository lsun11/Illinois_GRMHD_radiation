FUNCTION get_rho0(rho0_derivs_s,Rs_s,a2_s,rho0_s,M,Rs,Mdot,K_poly,Gamma,G29_rhs)
  ...
  INTERFACE
     FUNCTION a2_func(a2,M,Rs,Mdot,K_poly,Gamma,G29_rhs)
       USE nrtype
       IMPLICIT NONE
       REAL(DP), INTENT(IN) :: a2,M,Rs,Mdot,K_poly,Gamma,G29_rhs
       REAL(DP) :: a2_func
     END FUNCTION a2_func
  END INTERFACE
   ...
   num=100
     do while (found .eq. 0)
        ...
        do i=1,num
           x1=a2_s-i*delta
           x2=a2_s-(i-1)*delta
           if (a2_func(x1,M,Rs,Mdot,K_poly,Gamma,G29_rhs)*a2_func(x2,M,Rs,Mdot,K_poly,Gamma,G29_rhs) .lt. 0) then
              found=1
              exit
           endif
        end do
     end do
     a2 = rtbis(a2_func,x1,x2,xacc,M,Rs,Mdot,K_poly,Gamma,G29_rhs)
     get_rho0 = (1.d0/K_poly*a2/(Gamma-Gamma/(Gamma-1)*a2))**(1.d0/(Gamma-1.d0))
 endif
  ...
  if (Rs .gt. Rs_s-tiny .and. Rs .lt. Rs_s+tiny) then
     get_rho0=rho0_s+rho0_derivs_s*(Rs-Rs_s)
  endif
...
FUNCTION a2_func(a2,M,Rs,Mdot,K_poly,Gamma,G29_rhs)
  ...
  a2_func = (1.d0 - 2.d0*M/Rs+(Mdot/(4*PI_D*(1.d0/K_poly*a2/(Gamma-Gamma/(Gamma-1)*a2))**(1.d0/(Gamma-1.d0))*Rs*Rs))**2)*(1.d0+a2/(Gamma-1.d0-a2))**2-G29_rhs
END FUNCTION a2_func
