  ...
  if (mod(cctk_iteration,out_every)==0 .and. cctk_iteration .gt. 0) then
     if (find_max_phi1==1) then 
	   ...
	   xc = position_x(1)
        ...
        initial_guess__coord_ellipsoid__x_center(4) = xc
        initial_guess__coord_ellipsoid__y_center(4) = yc
        initial_guess__coord_ellipsoid__z_center(4) = 0.d0 
	...
	position_x(1) = sf_origin_x(3)
        position_y(1) = sf_origin_y(3)
        position_z(1) = 0.d0
        position_x(2) = sf_origin_x(3)
        position_y(2) = sf_origin_y(3)
        position_z(2) = 0.d0
        xc1_prev = sf_origin_x(3)
        yc1_prev = sf_origin_y(3)
        zc1_prev = sf_origin_z(3)
        xc2_prev = sf_origin_x(3)
        yc2_prev = sf_origin_y(3)
        zc2_prev = sf_origin_z(3)
