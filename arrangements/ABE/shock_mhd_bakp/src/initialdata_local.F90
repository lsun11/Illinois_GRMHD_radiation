...
! we just need to fill in others.
        ...
        do k=1,ext(3)
          do j=1,ext(2)
	        ...
	        !ux(i,j,k) = vx(i,j,k)/sqrt(1-vx(i,j,k)**2)
                ...
                F_rady(i,j,k) = ZERO
                F_radz(i,j,k) = ZERO
                P_rad(i,j,k) = E_rad(i,j,k)/3.0

                rho_star(i,j,k) = rho_b(i,j,k) * w(i,j,k)
                h(i,j,k) = 1.d0 + ( npolyshock + 1.d0 ) * P(i,j,k) / rho_b(i,j,k)

		tau(i,j,k) = rho_star(i,j,k)*(h(i,j,k)*w(i,j,k)-1.d0)-P(i,j,k)+sbs_l*(w(i,j,k)*w(i,j,k)-0.5d0)-sb0_l**2


		st_x(i,j,k)  = rho_star(i,j,k)*h(i,j,k)*vx(i,j,k)/sqrt(1-vx(i,j,k)**2)
                st_y(i,j,k)  = rho_star(i,j,k)*h(i,j,k)*uy_l
                st_z(i,j,k)  = rho_star(i,j,k)*h(i,j,k)*uz_l
                mhd_st_x(i,j,k) = st_x(i,j,k) + w(i,j,k)*sbs_l*vx(i,j,k)/sqrt(1-vx(i,j,k)**2) - sb0_l*sbx_l
                ...
                tau_rad(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*u0(i,j,k) + 2.0*u0(i,j,k)*F_rad0(i,j,k)-P_rad(i,j,k))
                S_rad_x(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*vx(i,j,k)/sqrt(1-vx(i,j,k)**2) + F_rad0(i,j,k)*vx(i,j,k)/sqrt(1-vx(i,j,k)**2) + F_radx(i,j,k)*u0(i,j,k))
                S_rad_y(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uy_l + F_rad0(i,j,k)*uy_l + F_rady(i,j,k)*u0(i,j,k))
                S_rad_z(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uz_l + F_rad0(i,j,k)*uz_l + F_radz(i,j,k)*u0(i,j,k))



		!write(*,*) "i,j,k, rho_star, h , st_x, MHD_ST_X=", i,j,k, rho_star(i,j,k), h(i,j,k), st_x(i,j,k), mhd_st_x(i,j,k)

		if (tau(i,j,k) .le. 0.d0) then
		write(*,*) "negative or zero tau!!!", i,j,k,tau(i,j,k), rho_star(i,j,k), mhd_st_x(i,j,k)
		end if
  ...
  call  initialdata_radiation (ext, x, npolyshock, &
	...
	rhosave, Psave, uxsave, Eradsave, Fradsave)
  	...
  	do k=1,ext(3)
		...
		T_fluid(i,j,k) = (E_rad(i,j,k)/rad_const)**(0.25)
                tau_rad(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*u0(i,j,k) + 2.0*u0(i,j,k)*F_rad0(i,j,k)-P_rad(i,j,k))
                S_rad_x(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uxsave(i) + F_rad0(i,j,k)*uxsave(i) + F_radx(i,j,k)*u0(i,j,k))
                S_rad_y(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uy_l + F_rad0(i,j,k)*uy_l + F_rady(i,j,k)*u0(i,j,k))
                S_rad_z(i,j,k) = w(i,j,k)*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uz_l + F_rad0(i,j,k)*uz_l + F_radz(i,j,k)*u0(i,j,k))
	      ...
	      Bx(i,j,k)   = Bx_r
              ...
              F_rady(i,j,k) = ZERO
              F_radz(i,j,k) = ZERO
              P_rad(i,j,k) = Erad_r/3.0
              !T_fluid(i,j,k) = P(i,j,k)*M_B/rho_star(i,j,k)
              T_fluid(i,j,k) = (E_rad(i,j,k)/rad_const)**(0.25)
              tau_rad(i,j,k) = w_r*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*u0(i,j,k) + 2.0*u0(i,j,k)*F_rad0(i,j,k)-P_rad(i,j,k))
              S_rad_x(i,j,k) = w_r*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*ux_r + F_rad0(i,j,k)*ux_r + F_radx(i,j,k)*u0(i,j,k))
              S_rad_y(i,j,k) = w_r*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uy_r + F_rad0(i,j,k)*uy_r + F_rady(i,j,k)*u0(i,j,k))
              S_rad_z(i,j,k) = w_r*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uz_r + F_rad0(i,j,k)*uz_r + F_radz(i,j,k)*u0(i,j,k))
	   end if 
     ...
     do j=1,ext(2)
        do i=1,ext(1)
	     ...
	     !the 3D arrays with this 1D array.
	      ...
	      F_rady(i,j,k) = ZERO
	      F_radz(i,j,k) = ZERO
	      P_rad(i,j,k) = Erad_l/3.0
	      !T_fluid(i,j,k) = P(i,j,k)*M_B/rho_star(i,j,k)
	      T_fluid(i,j,k) = (E_rad(i,j,k)/rad_const)**(0.25)
	      tau_rad(i,j,k) = w_l*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*u0(i,j,k) + 2.0*u0(i,j,k)*F_rad0(i,j,k)-P_rad(i,j,k))
              S_rad_x(i,j,k) = w_l*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*ux_l + F_rad0(i,j,k)*ux_l + F_radx(i,j,k)*u0(i,j,k))
              S_rad_y(i,j,k) = w_l*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uy_l + F_rad0(i,j,k)*uy_l + F_rady(i,j,k)*u0(i,j,k))
              S_rad_z(i,j,k) = w_l*((E_rad(i,j,k) + P_rad(i,j,k))*u0(i,j,k)*uz_l + F_rad0(i,j,k)*uz_l + F_radz(i,j,k)*u0(i,j,k))
           else
              !
              ! "right" side of partition
              !
