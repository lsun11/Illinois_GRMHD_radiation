...
!     ********PATCH******************************************
!     Keep away from the z-axis, artificially.
      if (xp**2 + yp**2 .lt. 1.d-16) then
!         write(6,*) 'patch on axis in ylmder2'
         xp = 0.d0
         yp = 1.d-8
      end if
...
!     Array sinthetal(l) contains [sin(theta)]^l
      sinthetal(-1) = 0.d0
      sinthetal(0) = 1.d0
...
!     zlm0 is Y_lm stripped of exp(i m phiangle) and multiplied by sqrt(4pi).
!     zlm1 and zlm2 are the first and second derivative of zlm0 
!     with respect to theta. l=0 and m=l are treated separately.
!     We only need zlm0,1,2 for positive m.
...
!     Initialize m = l:
!     c1 = sqrt(2l+1) * sqrt(2l!) / 2^l / l! 
!     Y_ll = c1 * sin(theta)^l 
!     Y_llp = c1 * l * sin(theta)^(l-1) * costheta
         m = l
...
!     Recursion relations for the other m:
!     c2 = sqrt[(2l+1)(2l-1)/(l^2-m^2)]
!     c3 = sqrt[((l-1)^2-m^2)(2l=1)/(l^2-m^2)/(2l-3)]
!     Y_lm = c2 Y_l-1,m cos(theta) + c3 Y_l-2,m
!     Y_lmp = c2 Y_l-1,mp cos(theta) + c2 Y_l-1,m + c3 Y_l-2,mp
         m = l-1
...
!     Azimuth angle phiangle is defined.
         cosphi = xp / rhoradius
         sinphi = yp / rhoradius
!     First and second derivatives of theta.
         thetax = costheta * cosphi / er
         thetay = costheta * sinphi / er
         thetaz = - sintheta / er
!     First and second derivatives of phiangle, the angle in the xy-plane.
         phianglex = - sinphi / rhoradius
         phiangley = cosphi / rhoradius
!     Put together arrays of basis functions in the xy plane.
!     The sqrt(2) factor is needed in going from the basis exp(+/-imphiangle)
!     to the basis cos/sin(mphiangle).
         cosmphiangle(0) = 1.d0
         sinmphiangle(0) = 1.d0
...
!     See above for a patch that avoids ever getting here.
          call CCTK_WARN(0,'ylmder2 cannot handle axis')
!     For x = y = 0 the following are not defined.
!         cosphi = 7.d77
!         sinphi = 7.d77
!         thetax = 7.d77
!         thetay = 7.d77
...
!     Assemble the Y_lm from zlm0 and the basis of functions of phiangle.
!     Assemble the Y_lm,i and Y_lm,ij in the corresponding manner.
      do l=0,lmax
         do m=0,l
!     These are the values of indx corresponding to (l,m) and (l,-m).
            indx1 = l**2 + 1 + l + m
            indx2 = l**2 + 1 + l - m
!     Ylm,x
...
!     Derivatives of r...
!     r,i = n_i and r,ij = (delta_ij - n_i n_j) / r = n_ij
      do idir=1,3
         fi(idir) = ni(idir)
      end do
!     ...and derivatives of -h.
      do indx=1,nmax
         do idir=1,3
...
!     Now with indices up, raised by g^ij.
      do idir=1,3
         supi(idir) = 0.d0
         do jdir=1,3
            supi(idir) = supi(idir) &
