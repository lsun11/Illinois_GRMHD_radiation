...
!              Find theta.
               ...
               !following three lines fix bitant symmetry
               if ((CCTK_Equals(domain,"bitant").eq.1).and.
     .            (CCTK_Equals(bitant_plane,"xy").eq.1)) then
                  theta = theta*0.5D0
               end if
...
!              Check if we are within bounds.
     ...
     .             (zp.gt.zmx).or.(zp.lt.zmn)) then
                  interror2 = 1
                  write(*,*) "interror!",xp,yp,zp,xmx,ymx,zmx,xmn,ymn,zmn,theta
               end if
...
!                  write(*,*) "ahfinder_int: ",i,j,exp(i  ,j  ),exp(i+1,j  ),exp(i  ,j+1),exp(i+1,j+1)
