...
subroutine statpunc_bondi_initialdata_local2(CCTK_ARGUMENTS)
  implicit none
  ...
  !for testing purposes, set a radius beyond which there is only atmosphere
  do k=1,ext(3)
     do j=1,ext(2)
        do i=1,ext(1)
           rr=sqrt(X(i,j,k)*X(i,j,k)+Y(i,j,k)*Y(i,j,k)+Z(i,j,k)*Z(i,j,k))
           if (rr .gt. atmosphere_radius) then
              rho_b(i,j,k)=0.d0
              write(*,*) "set to atmosphere: ",X(i,j,k),Y(i,j,k),Z(i,j,k),atmosphere_radius
           endif
        enddo
     end do
  end do
  ...
  !commented out because we are not considering self-gravity
...
!  call CCTK_SyncGroup(ierr,cctkGH,'BSSN::BSSN_vars') 
 ...
 !Initialize these variables:
  ...
  !!rho_b_previous = rho_b
  !!rho_prev_diag = rho_b
  !!drho_dtau = 0.d0
  !!drhodt = 0.d0
  !!rho_grad = 0.d0
  !!write(*,*) "I just set rho_grad to zero!"
  !t_previous = 0.d0

  !initialize these better
  surf1_radii_last_bondi = 0.d0
  surf2_radii_last_bondi = 0.d0
  surf3_radii_last_bondi = 0.d0
  surf4_radii_last_bondi = 0.d0
end subroutine statpunc_bondi_initialdata_local2
