subroutine OS_rad_analytic_new(Fcoeff,z,R_a,tau,RoM,M,Eo4PiB,tau_a,tau_s,eta,Q,rho_b,Ec,Fc,E,F)
  ...
  integer :: n
  interface
     FUNCTION os_func(eta,tau,RoM,M)
       USE nrtype
       IMPLICIT NONE
  ...
  x2 = pi
  xacc = 1.d-6
  eta = abs(rtbis(os_func,x1,x2,xacc,tau,RoM,M))
  ...
  do
     term = (-1)**(n+1)*2.d0*Eo4PiB*sinchi0*exp((chi0*chi0-n*n*pi*pi)*tau_t)*n*pi/(n*n*pi*pi-chi0*chi0)
     if (z < 1.d-7) then
...
FUNCTION os_func(eta,tau,RoM,M)
  ...
  os_func = sqrt(RoM**3*M*M/8.d0)*(eta + sin(eta)) - tau
