/*
  DAGH-based binfile output thorn
*/
...
CCTK_FILEVERSION(outputbinfile)

extern "C" CCTK_INT outputbinfile(const cGH *cctkGH,int *glob_ext,int *loc_ext,int *loc_origin,double cctk_time,double *xyz_glob_min,double *xyz_glob_max,
				  int cctk_iteration,double *f,char *gridfuncname,int Symmetry);

extern "C" void CCTK_FCALL CCTK_FNAME(outputbinfile)
				  ...
				  int cctk_iteration,double *f,char *gridfuncname,int Symmetry)
{
  ...
  */
  ...
  if(CCTK_MyProc(cctkGH) == 0) {
    ...
    for(int k = kmin;k<ext[2]+1;k++) {
      for(int j = jmin;j<ext[1]+1;j++) {
	for(int i = imin;i<ext[0]+1;i++) {
...
extern "C" void CCTK_FCALL CCTK_FNAME(outputbinfile) 
  (CCTK_INT *retval,const cGH **cctkGH,int *glob_ext,int *loc_ext,int *loc_origin,double *cctk_time,double *xyz_glob_min,double *xyz_glob_max,
   int *cctk_iteration,double *f,char *gridfuncname,int *Symmetry)
{
  *retval = outputbinfile(*cctkGH, glob_ext, loc_ext, loc_origin,*cctk_time,xyz_glob_min,xyz_glob_max,
