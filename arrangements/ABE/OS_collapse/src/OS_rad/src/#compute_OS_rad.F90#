...
!
  ...
  real*8, dimension(ex(1),ex(2),ex(3))     :: E_rad,F_rad0,F_radx,F_rady,F_radz,F
  real*8, dimension(ex(1),ex(2),ex(3))     :: tau_rad,S_rad_x,S_rad_y,S_rad_z
!
! Other variables:
!
  ...
  real*8                     :: Po4PiB,PoRho,rho_b_0,E_rad0,F_rad
         ...
         !  write(*,*) "K_poly: ",K_poly
          ...
          ! S(i,j,k) = 0.d0
           ...
           if (width .gt. 0.d0) then
              F_rad  = 0.5d0*E_rad0*exp(-(riso-r_edge)**2/width)
           else
              F_rad = 0.d0
           endif
           
           F_rad0(i,j,k)       = 0.d0
           F_radx(i,j,k)       = F_rad*X(i,j,k)/R
           F_rady(i,j,k)       = F_rad*Y(i,j,k)/R
           F_radz(i,j,k)       = F_rad*Z(i,j,k)/R
           F(i,j,k)            = sqrt(pgxx*F_radx(i,j,k)**2 + 2.d0*pgxy*F_radx(i,j,k)*F_rady(i,j,k) + &
                2.d0*pgxz*F_radx(i,j,k)*F_radz(i,j,k) + pgyy*F_rady(i,j,k)**2 + &
                2.d0*pgyz*F_rady(i,j,k)*F_radz(i,j,k) + pgzz*F_radz(i,j,k)**2)
           tau_rad(i,j,k)      = psi6*E_rad(i,j,k)*F1o3*(FOUR*alpha*alpha*ut*ut-ONE)
           S_rad_x(i,j,k)      = alpha*psi6*ut*(pgxx*F_radx(i,j,k)+pgxy*F_rady(i,j,k)+pgxz*F_radz(i,j,k))
           S_rad_y(i,j,k)      = alpha*psi6*ut*(pgxy*F_radx(i,j,k)+pgyy*F_rady(i,j,k)+pgyz*F_radz(i,j,k))
           S_rad_z(i,j,k)      = alpha*psi6*ut*(pgxz*F_radx(i,j,k)+pgyz*F_rady(i,j,k)+pgzz*F_radz(i,j,k))
