...
subroutine fill_in_bh_Afields_part2_fill_interior(CCTK_ARGUMENTS)
  ...
  !if(mod(cctk_iteration,refill_horizons_magfields_every)==0 .and. cctk_iteration.gt.0 .and. ah_found_flag(1)==1 .and. use_new_bhns_initial_data.ne.3 .or. use_new_bhns_initial_data.ne.4) then
  if((mod(cctk_iteration,refill_horizons_magfields_every)==0).and.(cctk_iteration.gt.0).and.(ah_found_flag(1)==1).and.(use_new_bhns_initial_data.eq.0).or.(use_new_bhns_initial_data.eq.1) &
