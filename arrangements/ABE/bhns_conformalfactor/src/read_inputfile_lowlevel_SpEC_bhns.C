...
extern "C" void CCTK_FCALL CCTK_FNAME(read_inputfile_SpEC_bhns)
  (const cGH **cctkGH,int *genID_cmdline_output_enable,int *fisheye_enable,
   ...
   double *gxx,double *gxy,double *gxz,double *gyy,double *gyz,double *gzz, double *psi,
   double *kxx,double *kxy,double *kxz,double *kyy,double *kyz,double *kzz,
   double *rho_nbar,double *ux,double *uy, double *uz,
   double &gamma_th,double &kappa,double &xbh,double &ybh,double &mirr,double &xns,double &yns,int &read_rotated_data);

extern "C" void read_inputfile_SpEC_bhns(const cGH *cctkGH,int genID_cmdline_output_enable,int fisheye_enable,
					   double xmin,double ymin,double zmin,
					   double dx,double dy,double dz,int *ext,
					   double *lapse,double *shiftx,double *shifty,double *shiftz,
					   double *gxx,double *gxy,double *gxz,double *gyy,double *gyz,double *gzz, double *psi,
  ...
  if(genID_cmdline_output_enable==1) {
    ...
    cout<<"ADM mass="<<mirr<<endl;
    infile1.read((char *) &xbh, sizeof(double));
    cout<<"xbh="<<xbh<<endl;
    infile1.read((char *) &ybh, sizeof(double));
    cout<<"ybh="<<ybh<<endl;
    infile1.read((char *) &doubledum, sizeof(double));
  ...
  if(read_rotated_data==1) {
    cout << "Stopping: read_rotated_data==1 is not supported when use_new_bhns_initial_data=2\n" ; 
    exit(1);
  } else {
    ...
    for(int k=0;k<nz;k++) for(int j=0;j<ny;j++) for(int i=0;i<nx;i++) infile1.read((char *) &gxx[CCTK_GFINDEX3D(cctkGH,i,j,k)],sizeof(double));
    for(int k=0;k<nz;k++) for(int j=0;j<ny;j++) for(int i=0;i<nx;i++) infile1.read((char *) &gxy[CCTK_GFINDEX3D(cctkGH,i,j,k)],sizeof(double));
    for(int k=0;k<nz;k++) for(int j=0;j<ny;j++) for(int i=0;i<nx;i++) infile1.read((char *) &gxz[CCTK_GFINDEX3D(cctkGH,i,j,k)],sizeof(double));
    for(int k=0;k<nz;k++) for(int j=0;j<ny;j++) for(int i=0;i<nx;i++) infile1.read((char *) &gyy[CCTK_GFINDEX3D(cctkGH,i,j,k)],sizeof(double));
    for(int k=0;k<nz;k++) for(int j=0;j<ny;j++) for(int i=0;i<nx;i++) infile1.read((char *) &gyz[CCTK_GFINDEX3D(cctkGH,i,j,k)],sizeof(double));
    for(int k=0;k<nz;k++) for(int j=0;j<ny;j++) for(int i=0;i<nx;i++) infile1.read((char *) &gzz[CCTK_GFINDEX3D(cctkGH,i,j,k)],sizeof(double));
    ...
    //Finally, the extrinsic curvatuve and matter fields
...
extern "C" void CCTK_FCALL CCTK_FNAME(read_inputfile_SpEC_bhns)
  (const cGH **cctkGH,int *genID_cmdline_output_enable,int *fisheye_enable,
   ...
   double &gamma_th,double &kappa,double &xbh,double &ybh,double &mirr,double &xns,double &yns,int &read_rotated_data) 
{  
  read_inputfile_SpEC_bhns(*cctkGH,*genID_cmdline_output_enable,*fisheye_enable,
			     *xmin,*ymin,*zmin,*dx,*dy,*dz,ext,
			     lapse,shiftx,shifty,shiftz,
			     gxx,gxy,gxz,gyy,gyz,gzz,psi,
			     kxx,kxy,kxz,kyy,kyz,kzz,
			     rho_nbar,ux,uy, uz,
			     gamma_th,kappa,xbh,ybh,mirr,xns,yns,read_rotated_data);
}
