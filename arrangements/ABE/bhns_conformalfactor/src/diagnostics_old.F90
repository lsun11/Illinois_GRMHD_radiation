...
!     data_headers(10) = "Tfluid0_0_VolInt"
!     export_data(10) = Tfluid0_0_VolInt*SymmFactor
...
!!$
     ...
     if(alpha_diagnostic==10) then
        if(mod(cctk_iteration,128*out_every)==0) then
           narray = surfxnum*surfphinum*surfznum
           call surface_dens_1d(cctkGH,CCTK_TIME,binary_orb_freq,rotation_phase,bh_posn_x(1),bh_posn_y(1),bh_posn_z(1), &
                 surfxnum,surfphinum,surfznum,surfzmax,surfxmin,surfxmax,Symmetry,CCTK_MyProc(CCTKGH))
           
           call compute_alpha_1d(cctkGH,CCTK_TIME,binary_orb_freq,rotation_phase,bh_posn_x(1),bh_posn_y(1),bh_posn_z(1), &
                 surfxnum,surfphinum,surfznum,surfzmax,surfxmin,surfxmax,Symmetry,CCTK_MyProc(CCTKGH))
        end if
     end if
