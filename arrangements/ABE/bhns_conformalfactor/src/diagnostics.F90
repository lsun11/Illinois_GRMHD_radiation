...
!if(0==1) then
  ...
  if(mod(cctk_iteration,out_every)==0) then
     ...
     if((use_new_bhns_initial_data.eq.0).or.(use_new_bhns_initial_data.eq.1) &
           .or.(use_new_bhns_initial_data.eq.2).or.(use_new_bhns_initial_data.eq.6).or. &
           (abs(bh_posn_y(1))>0.d0)) then
...
31         FORMAT("bhns.jon.",I1)
     ...
     if (track_bhns.eq.1) then
        ...
        r_local=bhns_R_NS
        call J_surf_integral_spin(cctkGH,points_CM(1,1),points_CM(1,2),points_CM(1,3),r_local,export_data(2))

        ! angular momentum star2
        data_headers(3) = "J_star2"
        r_local=bhns_R_NS
        call J_surf_integral_spin(cctkGH,points_CM(2,1),points_CM(2,2),points_CM(2,3),r_local,export_data(3))


        ! angular momentum star1
        data_headers(4) = "J_star1 R at 1.1"
        r_local=1.1*bhns_R_NS
        call J_surf_integral_spin(cctkGH,points_CM(1,1),points_CM(1,2),points_CM(1,3),r_local,export_data(4))

        ! angular momentum star2
        data_headers(5) = "J_star2 R at 1.1"
        r_local=1.1*bhns_R_NS
        call J_surf_integral_spin(cctkGH,points_CM(2,1),points_CM(2,2),points_CM(2,3),r_local,export_data(5))
...
!!$
     ...
     if(alpha_diagnostic.eq.1) then
        if(mod(cctk_iteration,16*out_every)==0) then    
