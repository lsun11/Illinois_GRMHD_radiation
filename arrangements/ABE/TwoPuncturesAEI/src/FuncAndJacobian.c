...
void SpecCoef(int n1, int n2, int n3, int ivar, CCTK_REAL *v, CCTK_REAL *cf)
{
  ...
  // Here v is a pointer to the values of the variable v at the collocation points
	...
	//	values5=dtensor(0,n1,0,n2,0,n3);


	      // Caclulate values3[n,j,k] = a_n^{j,k} = (sum_i^(n1-1) f(A_i,B_j,phi_k) Tn(-A_i))/k_n , k_n = N/2 or N 
	      for(k=0;k<n3;k++) {
		for(j=0;j<n2;j++) {
  //		  for(i=0;i<n1;i++) p[i]=v[Index (ivar, i, j, k, 1, n1, n2, n3)];
		  for(i=0;i<n1;i++) p[i]=v[ivar + (i + n1 * (j + n2 * k))];
    //int Index_fast(int n1, int n2, int n3, int nvar, int ivar, int i, int j, int k)
		    //		    int ind = Index_fast(n1, n2, n3, 1, ivar, i, j, k);
				      //		    p[i]=v[ind];		  
		  chebft_Zeros(p,n1,0); 
		  for (n=0;n<n1;n++)	{
		    values3[n][j][k] = p[n]; 
		...
		for (j = 0; j < n2; j++){
		  ...
		  for (k = 0; k<n3; k++){
		  //	      values5[n][l][m] = p[m];
		    //		    int ind = Index_fast(n1, n2, n3,1, ivar, n, l, m);
		    //	   cf[Index (ivar, n, l, m, 0, n1, n2, n3)] = p[m];
	      ...
	      //	      free_dtensor(values5,0,n1,0,n2,0,n3);
