...
!-----------------------------------------------------------------------------
  subroutine ks_initial_metric_plus_junk_one_BH(ex, x, y, z,       &
                    ...
                    m0, sam, r_junk, lambda, x0,y0,z0,x1,y1,z1, r_shield)
  implicit none
...
!
  ...
  real*8  :: r_junk, lambda, x0,y0,z0,x1,y1,z1,fact_att
  real*8  :: w0, wh_out, wh_in,a, fac_att
  real*8  :: xl,yl,zl
...
! Attenuation factor 
    tmp = ((X(i,j,k)-x1)**2 + (Y(i,j,k)-y1)**2 + (Z(i,j,k)-z1)**2)/r_shield**2
    if (tmp .gt. 7.d0) then 
       fac_att = 1.d0
    else
       fac_att = 1.d0 - exp(-tmp*tmp)
    end if
...
!
     ...
     alpm1(i,j,k) = alpm1(i,j,k) + (aa-one)*fac_att

        Bx(i,j,k) = Bx(i,j,k) + two*aa*aa*hh*el(0)*el(1)*fac_att
        By(i,j,k) = By(i,j,k) + two*aa*aa*hh*el(0)*el(2)*fac_att
        Bz(i,j,k) = Bz(i,j,k) + two*aa*aa*hh*el(0)*el(3)*fac_att

!-- the 3-metric - delta_ij
!

     gxxm1(i,j,k) = gxxm1(i,j,k) + two*hh*el(1)*el(1) *fac_att
     gyym1(i,j,k) = gyym1(i,j,k) + two*hh*el(2)*el(2) *fac_att
     gzzm1(i,j,k) = gzzm1(i,j,k) + two*hh*el(3)*el(3) *fac_att
     gxy(i,j,k) = gxy(i,j,k) + two*hh*el(1)*el(2) *fac_att
     gyz(i,j,k) = gyz(i,j,k) + two*hh*el(2)*el(3) *fac_att
     gxz(i,j,k) = gxz(i,j,k) + two*hh*el(3)*el(1) *fac_att
...
!
     ...
     Axx(i,j,k) = Axx(i,j,k) + (k1 + k2 + k3)*fac_att
     ...
     Axy(i,j,k) = Axy(i,j,k) + (k1 + k2 + k3)*fac_att
     ...
     Axz(i,j,k) = Axz(i,j,k) + (k1 + k2 + k3)*fac_att
     ...
     Ayy(i,j,k) = Ayy(i,j,k) + (k1 + k2 + k3)*fac_att
     ...
     Ayz(i,j,k) = Ayz(i,j,k) + (k1 + k2 + k3)*fac_att
     ...
     Azz(i,j,k) = Azz(i,j,k) + (k1 + k2 + k3)*fac_att
...
!-----------------------------------------------------------------------------
  subroutine ks_initial_metric_bl_junk_one_BH(ex, x, y, z,             &
       gxxm1, gxy, gxz, gyym1, gyz, gzzm1,              &
       Axx, Axy, Axz, Ayy, Ayz, Azz, alpm1, Bx,By,Bz, &
       x0,y0,z0,x1,y1,z1,m0, sam, r_junk, lambda,r_shield)
  implicit none
...
!
  integer, dimension(3), intent(in)                  :: ex
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(in)  :: x,y,z
  real*8,  intent(in)                                :: sam,m0, x0,y0,z0,x1,y1,z1
  real*8,  intent(in)                                :: r_shield
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out) :: gxxm1,gxy,gxz,gyym1,gyz,gzzm1
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out) :: Axx,Axy,Axz,Ayy,Ayz,Azz
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out) :: alpm1,Bx,By,Bz
...
!
  ...
  real*8 :: r_l, x_l, y_l, z_l, fac_att
...
!
	...
	dalp0 = m0*(r0*r0-a*a*cos2th)/Sigma0/Sigma0*alp0**3
     ...
     alpm1(i,j,k) = alpm1(i,j,k) + (aa-1.d0)*fac_att
...
!
	...
	Aphph = r2*(aaa*r2 + bbb*r2*r)
     ...
     !#############################################################################
       ...
       Bx(i,j,k) = Bx(i,j,k) + Lx_r*Br*fac_att
       By(i,j,k) = By(i,j,k) + Ly_r*Br*fac_att
       Bz(i,j,k) = Bz(i,j,k) + Lz_r*Br*fac_att
       ...
       gxxm1(i,j,k) = gxxm1(i,j,k) +  (gxxl - 1.d0)*fac_att
       gxy(i,j,k) = gxy(i,j,k) + gxyl*fac_att
       gxz(i,j,k) = gxz(i,j,k) + gxzl*fac_att
       gyym1(i,j,k) = gyym1(i,j,k) + (gyyl - 1.d0)*fac_att
       gyz(i,j,k) = gyz(i,j,k) + gyzl*fac_att
       gzzm1(i,j,k) = gzzm1(i,j,k) + (gzzl - 1.d0)*fac_att

       Axx(i,j,k) = Axx(i,j,k) + Kxxl*fac_att
       Axy(i,j,k) = Axy(i,j,k) + Kxyl*fac_att
       Axz(i,j,k) = Axz(i,j,k) + Kxzl*fac_att
       Ayy(i,j,k) = Ayy(i,j,k) + Kyyl*fac_att
       Ayz(i,j,k) = Ayz(i,j,k) + Kyzl*fac_att
       Azz(i,j,k) = Azz(i,j,k) + Kzzl*fac_att
...
!
!-----------------------------------------------------------------------------
  subroutine kerr_initial_metric_puncture_one_BH(ex, x, y, z, x0,y0,z0,x1,y1,z1,   &
       psim1,gxxm1, gxy, gxz, gyym1, gyz, gzzm1,             &
       Axx, Axy, Axz, Ayy, Ayz, Azz, alpm1, Bx,By,Bz, &
       sam,m0,r_shield)
  implicit none
...
!
  ...
  real*8,  intent(in)                                :: sam,m0,x0,y0,z0,x1,y1,z1
  real*8,  intent(in)                                :: r_shield
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out) :: psim1,gxxm1,gxy,gxz,gyym1,gyz,gzzm1
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out) :: Axx,Axy,Axz,Ayy,Ayz,Azz
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out) :: alpm1,Bx,By,Bz
!
! Other variables:
!
  integer :: i,j,k,m,n
  integer :: imin,jmin,kmin,imax,jmax,kmax
  real*8  :: k1,k2,k3,aa,hh,r2,w2,W,wx,wy,wz,tmp,tmq,tmr,fac_att
  real*8, parameter :: zeo=0.d0,one=1.d0,two=2.d0,thr=3.d0
  ...
  real*8 :: gxxl,gxyl,gxzl,gyyl,gyzl,gzzl,psim4
  real*8 :: Kxxl,Kxyl,Kxzl,Kyyl,Kyzl,Kzzl
...
!
     ...
     alpm1(i,j,k) = alpm1(i,j,k) + (alp - 1.d0)*fac_att
      Bph = -2.d0*m0*a*r/aa
...
!
     ...
     !#############################################################################
       ...
       Bx(i,j,k) = Bx(i,j,k) + Lx_ph*Bph*fac_att
       By(i,j,k) = By(i,j,k) + Ly_ph*Bph*fac_att
       ...
       psim4 = (gxxl * gyyl * gzzl + &
            gxyl * gyzl * gxzl + &
            gxzl * gxyl * gyzl &
            - gxzl * gyyl * gxzl &
            - gxyl * gxyl * gzzl &
            - gxxl * gyzl * gyzl)**(-1.d0/3.d0)

       psim1(i,j,k) = psim1(i,j,k) + ( psim4**(-0.25d0) - 1.d0)*fac_att
       gxxm1(i,j,k) = gxxm1(i,j,k) + (gxxl*psim4-1.d0)*fac_att
       gxy(i,j,k) = gxy(i,j,k) + gxyl*fac_att*psim4
       gxz(i,j,k) = gxz(i,j,k) + gxzl*fac_att*psim4
       gyym1(i,j,k) = gyym1(i,j,k) + (gyyl*psim4-1.d0)*fac_att
       gyz(i,j,k) = gyz(i,j,k) + gyzl*fac_att*psim4
       gzzm1(i,j,k) = gzzm1(i,j,k) + (gzzl*psim4-1.d0)*fac_att
       Axx(i,j,k) = Axx(i,j,k) + Kxxl*fac_att*psim4
       Axy(i,j,k) = Axy(i,j,k) + Kxyl*fac_att*psim4
       Axz(i,j,k) = Axz(i,j,k) + Kxzl*fac_att*psim4 
       Ayy(i,j,k) = Ayy(i,j,k) + Kyyl*fac_att*psim4
       Ayz(i,j,k) = Ayz(i,j,k) + Kyzl*fac_att*psim4
       Azz(i,j,k) = Azz(i,j,k) + Kzzl*fac_att*psim4
