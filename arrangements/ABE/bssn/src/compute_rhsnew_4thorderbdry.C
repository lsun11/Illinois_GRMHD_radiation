...
//CCTK_FILEVERSION(BSSN_rhs_4)

extern "C" void CCTK_FCALL CCTK_FNAME(BSSN_rhs_4)
...
extern "C" void BSSN_rhs_4(const cGH *cctkGH, double dT, double dx, double dy, double dz,
			   int *nghostzones,int *cctk_lsh,
			   double *PhysicalRadius, double *RadiusDerivative,double *r,
			   double *gxx,double *gxy,double *gxz,double *gyy,double *gyz,double *gzz,
			   double *gupxx,double *gupxy,double *gupxz,double *gupyy,double *gupyz,double *gupzz,
	...
	if(i==2 || j==2 || k==2 || i==cctk_lsh[0]-3 || j==cctk_lsh[1]-3 || k==cctk_lsh[2]-3) {
	  ...
	  if((i==2 && j>2 && k>2 && i<cctk_lsh[0]-3 && j<cctk_lsh[1]-3 && k<cctk_lsh[2]-3) || 
	    ...
	    if(bssn_enable_shift_upwind==2) {
	      if(shiftxL > 0.0) {
		if(i==cctk_lsh[0]-3) {
		  gxx_rhsL += shiftxL*D1gf(gxx,i,j,k);
		  gxy_rhsL += shiftxL*D1gf(gxy,i,j,k);
		  gxz_rhsL += shiftxL*D1gf(gxz,i,j,k);
		  gyy_rhsL += shiftxL*D1gf(gyy,i,j,k);
		  gyz_rhsL += shiftxL*D1gf(gyz,i,j,k);
		  gzz_rhsL += shiftxL*D1gf(gzz,i,j,k);
	  
		  Axx_rhsL += shiftxL*D1gf(Axx,i,j,k);
		  Axy_rhsL += shiftxL*D1gf(Axy,i,j,k);
		  Axz_rhsL += shiftxL*D1gf(Axz,i,j,k);
		  Ayy_rhsL += shiftxL*D1gf(Ayy,i,j,k);
		  Ayz_rhsL += shiftxL*D1gf(Ayz,i,j,k);
		  Azz_rhsL += shiftxL*D1gf(Azz,i,j,k);

		  Gammax_rhsL += shiftxL*D1gf(Gammax,i,j,k);
		  Gammay_rhsL += shiftxL*D1gf(Gammay,i,j,k);
		  Gammaz_rhsL += shiftxL*D1gf(Gammaz,i,j,k);

		  if(chi_evolution_enable==0) {
		    phi_rhsL += shiftxL*D1gf(phi,i,j,k);
		  } else {
		    chi_rhsL += shiftxL*D1gf(chi,i,j,k);
		  }

		  trK_rhsL += shiftxL*D1gf(trK,i,j,k);
		...
		if(i==2) {
	      ...
	      if(shiftyL > 0.0) {
		if(j==cctk_lsh[1]-3) {
		  gxx_rhsL += shiftyL*D2gf(gxx,i,j,k);
		  gxy_rhsL += shiftyL*D2gf(gxy,i,j,k);
		  gxz_rhsL += shiftyL*D2gf(gxz,i,j,k);
		  gyy_rhsL += shiftyL*D2gf(gyy,i,j,k);
		  gyz_rhsL += shiftyL*D2gf(gyz,i,j,k);
		  gzz_rhsL += shiftyL*D2gf(gzz,i,j,k);
	  
		  Axx_rhsL += shiftyL*D2gf(Axx,i,j,k);
		  Axy_rhsL += shiftyL*D2gf(Axy,i,j,k);
		  Axz_rhsL += shiftyL*D2gf(Axz,i,j,k);
		  Ayy_rhsL += shiftyL*D2gf(Ayy,i,j,k);
		  Ayz_rhsL += shiftyL*D2gf(Ayz,i,j,k);
		  Azz_rhsL += shiftyL*D2gf(Azz,i,j,k);

		  Gammax_rhsL += shiftyL*D2gf(Gammax,i,j,k);
		  Gammay_rhsL += shiftyL*D2gf(Gammay,i,j,k);
		  Gammaz_rhsL += shiftyL*D2gf(Gammaz,i,j,k);

		  if(chi_evolution_enable==0) {
		    phi_rhsL += shiftyL*D2gf(phi,i,j,k);
		  } else {
		    chi_rhsL += shiftyL*D2gf(chi,i,j,k);
		  }

		  trK_rhsL += shiftyL*D2gf(trK,i,j,k);
		} else {
		...
		if(j==2) {
	      ...
	      if(shiftzL > 0.0) {
		if(k==cctk_lsh[2]-3) {
		  gxx_rhsL += shiftzL*D3gf(gxx,i,j,k);
		  gxy_rhsL += shiftzL*D3gf(gxy,i,j,k);
		  gxz_rhsL += shiftzL*D3gf(gxz,i,j,k);
		  gyy_rhsL += shiftzL*D3gf(gyy,i,j,k);
		  gyz_rhsL += shiftzL*D3gf(gyz,i,j,k);
		  gzz_rhsL += shiftzL*D3gf(gzz,i,j,k);
	  
		  Axx_rhsL += shiftzL*D3gf(Axx,i,j,k);
		  Axy_rhsL += shiftzL*D3gf(Axy,i,j,k);
		  Axz_rhsL += shiftzL*D3gf(Axz,i,j,k);
		  Ayy_rhsL += shiftzL*D3gf(Ayy,i,j,k);
		  Ayz_rhsL += shiftzL*D3gf(Ayz,i,j,k);
		  Azz_rhsL += shiftzL*D3gf(Azz,i,j,k);

		  Gammax_rhsL += shiftzL*D3gf(Gammax,i,j,k);
		  Gammay_rhsL += shiftzL*D3gf(Gammay,i,j,k);
		  Gammaz_rhsL += shiftzL*D3gf(Gammaz,i,j,k);

		  if(chi_evolution_enable==0) {
		    phi_rhsL += shiftzL*D3gf(phi,i,j,k);
		  } else {
		    chi_rhsL += shiftzL*D3gf(chi,i,j,k);
		  }

		  trK_rhsL += shiftzL*D3gf(trK,i,j,k);
		} else {
