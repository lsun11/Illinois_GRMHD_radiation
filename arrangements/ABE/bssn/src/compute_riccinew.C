...
CCTK_FILEVERSION(BSSN_ricci_and_constraints)


  extern "C" void CCTK_FCALL CCTK_FNAME(BSSN_ricci_and_constraints)
...
extern "C" void BSSN_ricci_and_constraints(const cGH *cctkGH, double dT, double dx, double dy, double dz,
					   int *nghostzones,int *cctk_lsh,
					   double *gxx,double *gxy,double *gxz,double *gyy,double *gyz,double *gzz,
					   double *gupxx,double *gupxy,double *gupxz,double *gupyy,double *gupyz,double *gupzz,
					   double *Rxx,double *Rxy,double *Rxz,double *Ryy,double *Ryz,double *Rzz,
...
#pragma omp for 
	...
	// For proper normalization of H split the nabla
	double nabla_psi_xx = gupxxL * psixx;
	double nabla_psi_yy = gupyyL * psiyy;
	double nabla_psi_zz = gupzzL * psizz;
	double nabla_psi_xy = 2.0 * ( gupxyL * psixy);
	double nabla_psi_xz = 2.0 * ( gupxzL * psixz);
	double nabla_psi_yz = 2.0 * ( gupyzL * psiyz);


	// Add connection terms to create "covariant" Laplace operator:
	nabla_psi = nabla_psi - GammaxL*psix - GammayL*psiy - GammazL*psiz;

	// For proper normalization of H split the connection coefficients
	double nabla_psi_Gammax = GammaxL*psix;
	double nabla_psi_Gammay = GammayL*psiy;
	double nabla_psi_Gammaz = GammazL*psiz;
...
// 			       SQR(0.125*PsiL*trRtilde[index]) + SQR(0.125*Psi5*KK) );
	...
	PsiNorm[index] = sqrt( SQR(2.0*M_PI*Psi5*rho[index]) + 
			       SQR(nabla_psi_xx) + SQR(nabla_psi_yy) + SQR(nabla_psi_zz) + 
			       SQR(nabla_psi_xy) + SQR(nabla_psi_xz) + SQR(nabla_psi_yz) + 
			       SQR(nabla_psi_Gammax) + SQR(nabla_psi_Gammay) + SQR(nabla_psi_Gammaz) +
			       SQR(0.125*PsiL*trRtilde[index]) + SQR(0.125*Psi5*KK) );
...
extern "C" void CCTK_FCALL CCTK_FNAME(BSSN_ricci_and_constraints)
  (const cGH **cctkGH,double *dT, double *dx, double *dy, double *dz,
   ...
   double *psi, double *rho, double *PsiRes, double *PsiNorm,int *compute_constraint_flag)
{
  BSSN_ricci_and_constraints(*cctkGH,  *dT,  *dx,  *dy,  *dz,
