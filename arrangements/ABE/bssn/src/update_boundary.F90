...
subroutine update_boundary(ex,fake_ex,X,Y,Z,R_fish, PhysicalR, RadiusDerivative,Fisheye_Flag, &
  ...
  !BC's for puncture (extrapr: r*f at bdry = 2r*f at bdry-1 - r*f at bdry-2 [good for 1/r falloff!]) :
        ...
        do k = kmin, kmax-1
           do j = jmin, jmax-1
              call weights2(X(i,j,k),Y(i,j,k),Z(i,j,k),PhysicalR(i,j,k),RadiusDerivative(i,j,k),alpha(i,j,k),phi_old(i,j,k)&
                   ...
                   & Xcenter, Ycenter, Zcenter,gxx_old(i,j,k),gxy_old(i,j,k),gxz_old(i,j,k),gyy_old(i,j,k),gyz_old(i,j,k),gzz_old(i,j,k))
              !
              ! Interpolate 
              !
              if (bc_type==WAVELIKE) then
...
!!!$omp end parallel do
   ...
   write(*,*)		    "BSSN BC_TYPE: EXTRAPR"
     ...
     if (Fisheye_Flag==1) then
        fish_to_phys = .FALSE.
        ! FIXME: Shouldn't the ,2, below be ,0,????!!!  -Zach
        call trans_fish_phys(ex, fake_ex, X, Y, Z, PhysicalR, RadiusDerivative, &
             gxx_new,gxy_new,gxz_new,gyy_new,gyz_new,gzz_new, &
             fish_to_phys,2,have_bdry_min,have_bdry_max,use_trans_fish_phys_new)
        call trans_fish_phys(ex, fake_ex, X, Y, Z, PhysicalR, RadiusDerivative, &
...
subroutine rotate(ex,X,Y,Z,alpha,phi_new,K_new,gxx_new,&
  ...
  do k=kmin, kmax
     do i=imin, imax-1
        ...
        if (i==imin) then
           xa(1) = X(im,j,k)-dX
           ya1(1) = phi_new(im,j,k) !symmetric (scalars)
           ya2(1) = alpha(im,j,k) 
           ya3(1) = K_new(im,j,k)
           do l=2,4
              xa(l) = X(im+l-2,j,k)
        ...
        if (i==imin) then
           ya1(1) = gxx_new(im,j,k)  !symmetric 
           ya2(1) = -gxy_new(im,j,k) !antisymmetric 
           ya3(1) = -gxz_new(im,j,k) !antisymmetric 
           ya4(1) = gyy_new(im,j,k)  !symmetric 
           ya5(1) = gyz_new(im,j,k)  !symmetric 
           ya6(1) = gzz_new(im,j,k)  !symmetric
           do l=2,4
        ...
        if (i==imin) then
           ya1(1) = Axx_new(im,j,k)  !symmetric 
           ya2(1) = -Axy_new(im,j,k) !antisymmetric 
           ya3(1) = -Axz_new(im,j,k) !antisymmetric 
           ya4(1) = Ayy_new(im,j,k)  !symmetric 
           ya5(1) = Ayz_new(im,j,k)  !symmetric 
           ya6(1) = Azz_new(im,j,k)  !symmetric
           do l=2,4
        ...
        if (i==imin) then
           ya1(1) = -Gammax_new(im,j,k)  !antisymmetric 
           ya2(1) = Gammay_new(im,j,k) !symmetric 
           ya3(1) = Gammaz_new(im,j,k) !symmetric 
           do l=2,4
...
subroutine trans_fish_phys(ex,fake_ex, X, Y, Z, PhysicalR, RadiusDerivative, &
  ...
  ! We don't want to do the transformation more than once at each grid point.
  ! That's why it's necessary to mark the grid point where the
