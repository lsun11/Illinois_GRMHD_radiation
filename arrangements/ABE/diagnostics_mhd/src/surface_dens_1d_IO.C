...
extern "C" void CCTK_FCALL CCTK_FNAME(surf_dens_1d_IO)
  (const cGH **cctkGH, double &time,
   double &varpi_min,double &dvarpi,double &dz,double &phiangle_BH,
...
extern "C" void surf_dens_1d_IO(const cGH *cctkGH, double &time,
				double &varpi_min,double &dvarpi,double &dz,double &phiangle_BH,
				int &N_varpi,int &N_phi,int &N_Z,
				double *rho_b_int,double *P_int,double *h_int,double *vx_int,double *vy_int,double *vz_int,double *u0_int,
				double *lapse_int,double *shiftx_int,double *shifty_int,double *shiftz_int,
  ...
  cout << "*************" << endl;
  cout << "phiangle_BH: " << phiangle_BH << endl;
  ...
  for (int i = 1; i <= N_varpi; i++){
    ...
    */
    ...
    for (int j = 1; j <= N_phi; j++) {
      double phiangle = (j - 0.5)*dphi + phiangle_BH;
      double SigmaL = 0.0;
	...
	Sigma_avgL = Sigma_avgL + rho_b_intL*lapse_intL*Psi6*u0_intL*dz*dphi/(2.0*M_PI);
	...
	/*	torque_dens1L = torque_dens1L+integrand1*varpi*dz*dphi;
	torque_dens2L = torque_dens2L+integrandtt*varpi*dz*dphi;
	torque_dens3L = torque_dens3L+2.0*(integrandtx+integrandty+integrandtz)*varpi*dz*dphi;
	torque_dens4L = torque_dens4L+(integrandxx+integrandyy+integrandzz+2.0*(integrandxy+integrandxz+integrandyz))*varpi*dz*dphi;
	torque_densL = torque_densL+total_integrand*varpi*dz*dphi;
	torque_dens_oldL = torque_dens_oldL - rho_b_intL*lapse_intL*Psi6*(1.0/lapse_intL/lapse_intL/lapse_intL)*(-yL*lapsex_intL+xL*lapsey_intL)*varpi*dz*dphi;
	*/
	torque_densL = torque_densL+total_integrand*varpi*dz*dphi; 
	n=n+1;
    }
      //insert output here
      outfile2 << varpi << " " << phiangle-phiangle_BH << " " << SigmaL << " " << torque_densL << endl;
    }
    outfile2 << endl;
    //outfile1 << varpi << " " << Sigma_avgL << " " << torque_dens1L <<  " " << torque_dens2L << " " << torque_dens3L << " " << torque_dens4L << " " << torque_densL <<" " << torque_dens_oldL<< endl;
    outfile1 << varpi << " " << Sigma_avgL << " " <<torque_densL << endl; 
    //insert output here
    for (int j = 1; j <= N_phi; j++) {
      double phiangle = (j - 0.5)*dphi+phiangle_BH;
      outfile3 << varpi << " " << phiangle-phiangle_BH << " " << Sigma_avgL << " " << torque_densL << endl;
      //insert output here
    }
    outfile3 << endl;
  }
...
extern "C" void CCTK_FCALL CCTK_FNAME(surf_dens_1d_IO)
  (const cGH **cctkGH, double &time,
   ...
   double *gyyy_int,double *gyzy_int, double *gzzy_int)
{  
  surf_dens_1d_IO(*cctkGH, time,
		  varpi_min,dvarpi,dz,phiangle_BH,
		  N_varpi,N_phi,N_Z,
