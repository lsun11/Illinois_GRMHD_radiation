...
subroutine surface_dens_1d(cctkGH,time,omega_rot,N_varpi,N_phi,N_Z,&
     Z_max,varpi_min,varpi_max,Symmetry,myproc)
  implicit none
...
!  DECLARE_CCTK_PARAMETERS
  ...
  do i=1,N_varpi
     ...
     do j=1,N_phi
        phiangle = (j - 0.5)*dphi+phiangle_BH
        do k=1,N_Z
           ZL = dZ*(k-0.5)
           pointcoords(n,1) = varpi*cos(phiangle)
           pointcoords(n,2) = varpi*sin(phiangle)
           pointcoords(n,3) = ZL
           n = n + 1
        end do
     end do
  ...
  if (myproc.eq.0) then
  call surf_dens_1d_IO(cctkGH,time,&
...
end subroutine surface_dens_1d
