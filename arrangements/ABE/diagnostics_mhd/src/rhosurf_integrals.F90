...
subroutine rhosurf_integrals(cctkGH,F_M0,rho_const_surf,surf_area,ave_radius,t,t_last,xh_last,yh_last,zh_last,surf_radii_last,N_rad_ray,N_theta,N_phi,rhosurf_rmin,rhosurf_rmax,Symmetry,use_origin)
  implicit none
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i=1,N_theta
     ...
     do j=1,N_phi
	   ...
	   ind1 = j + (i-1)*N_phi  ! Equatorial sym --> surf_radii(ind1) = surf_radii(ind0)
	   indm1 = j + (i-2)*N_phi
...
!!$  do i=1,ntot
!!$     write(16,'(5e18.10)') pointcoords(i,1),pointcoords(i,2),pointcoords(i,3),rho_b_int(i),sqrt((pointcoords(i,1)-xh)**2+(pointcoords(i,2)-yh)**2+(pointcoords(i,3)-zh)**2)
!!$  end do
!!$  close(16)
!!$  
