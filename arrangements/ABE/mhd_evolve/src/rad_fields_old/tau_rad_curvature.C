...
extern "C" void CCTK_FCALL CCTK_FNAME(tau_rad_curvature_cpp)
  (const cGH **cctkGH,int *cctk_lsh, int *nghostzones, int *Symmetry,
   ...
   double *dX,double *dY,double *dZ, int *rad_closure_scheme);

void compute_M1_tau(double &Pij,double &Fi,double &Fj,double &Fksq,double &E,double &gij,double &ui, double &uj, double &chi, double &psim4);
...
extern "C" void tau_rad_curvature_cpp(const cGH *cctkGH,int *cctk_lsh, int *nghostzones, int Symmetry,
  ...
  printf("Start tau_rad_curvature_cpp, rad_closure_scheme is %d, Symmetry is %d, dX=%e, dY=%e, dZ=%e \n", rad_closure_scheme,Symmetry, dX, dY, dZ);
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++) for(int j=jstart;j<jend;j++) for(int i=istart;i<iend;i++) {
    ...
    tau_rad_rhs[index] = tau_rad_rhsL + sqrtg*(P_radL)*trKL;
      ...
      double zeta = (F_rad_0L*F_rad0L + F_rad_xL*F_radxL +  F_rad_yL*F_radyL +  F_rad_zL*F_radzL )/SQR(E_radL);
      double chi = 1/3.0 + SQR(zeta)*(6-2*zeta+6*SQR(zeta))/15.0;

      compute_M1_tau(P_radxxL, F_radxL, F_radxL, Fksq, E_radL, gupxx[index], uxL, uxL, chi, Psi4); 
      compute_M1_tau(P_radyyL, F_radyL, F_radyL, Fksq, E_radL, gupyy[index], uyL, uyL, chi, Psi4);
      compute_M1_tau(P_radzzL, F_radzL, F_radzL, Fksq, E_radL, gupzz[index], uzL, uzL, chi, Psi4);
      compute_M1_tau(P_radxyL, F_radxL, F_radyL, Fksq, E_radL, gupxy[index], uxL, uyL, chi, Psi4);
      compute_M1_tau(P_radxzL, F_radxL, F_radzL, Fksq, E_radL, gupxz[index], uxL, uzL, chi, Psi4);
      compute_M1_tau(P_radyzL, F_radyL, F_radzL, Fksq, E_radL, gupyz[index], uyL, uzL, chi, Psi4);
      ...
      tau_rad_rhsL += sqrtg*Psi4* ( 
	                           (AxxL+THIRD*gxxL*trKL)*(P_rad00L*betaxL*betaxL + P_rad0xL*betaxL + P_radxxL) +
	                           (AyyL+THIRD*gyyL*trKL)*(P_rad00L*betayL*betayL + P_rad0yL*betayL + P_radyyL) +    
				   (AzzL+THIRD*gzzL*trKL)*(P_rad00L*betazL*betazL + P_rad0zL*betazL + P_radzzL) +
				   (AxyL+THIRD*gxyL*trKL)*(2.0*P_rad00L*betaxL*betayL + P_rad0xL*betayL + P_rad0yL*betaxL + 2.0*P_radxyL) +
				   (AxzL+THIRD*gxzL*trKL)*(2.0*P_rad00L*betaxL*betazL + P_rad0xL*betazL + P_rad0zL*betaxL + 2.0*P_radxzL) +
				   (AyzL+THIRD*gyzL*trKL)*(2.0*P_rad00L*betazL*betayL + P_rad0zL*betayL + P_rad0yL*betazL + 2.0*P_radyzL));
...
void compute_M1_tau(double &Pij,double &Fi,double &Fj,double &Fksq,double &E,double &gupij,double &ui, double &uj, double &chi, double &psi4) {
  double Pij_thin = Fi*Fj/Fksq*E;
  double Pij_thick = (gupij/psi4+ui*uj)*E/3.0;
