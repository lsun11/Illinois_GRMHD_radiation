...
static int Utoprim_new_body(FTYPE U[], FTYPE gcov[NDIM][NDIM], FTYPE gcon[NDIM][NDIM], FTYPE gdet,  FTYPE prim[], int &neos, int &ergo_star, FTYPE &ergo_sigma, FTYPE *rho_tab, FTYPE *P_tab,  FTYPE *eps_tab,  FTYPE *k_tab,  FTYPE *gamma_tab);

//static int Utoprim_new_body(FTYPE U[], FTYPE gcov[NDIM][NDIM], FTYPE gcon[NDIM][NDIM], FTYPE gdet,  FTYPE prim[]);

static int general_newton_raphson( FTYPE x[], int n, void (*funcd) (FTYPE [], FTYPE [], FTYPE [], FTYPE [][NEWT_DIM], FTYPE *, FTYPE *, int,FTYPE &,FTYPE &,FTYPE &,FTYPE &,FTYPE &, FTYPE &, FTYPE & , int &, int &, FTYPE &,FTYPE *, FTYPE *, FTYPE *,FTYPE *),FTYPE &Bsq,FTYPE &QdotBsq,FTYPE &Qtsq,FTYPE &Qdotn,FTYPE &D,FTYPE &P_cold, FTYPE &eps_cold, int &neos, int &ergo_star, FTYPE &ergo_sigma, FTYPE *rho_tab, FTYPE *P_tab, FTYPE *gamma_tab, FTYPE *eps_tab);
static void func_vsq( FTYPE [], FTYPE [], FTYPE [], FTYPE [][NEWT_DIM], FTYPE *f, FTYPE *df, int n,FTYPE &Bsq,FTYPE &QdotBsq,FTYPE &Qtsq,FTYPE &Qdotn,FTYPE &D, FTYPE &P_cold, FTYPE &eps_cold, int &neos, int &ergo_star, FTYPE &ergo_sigma, FTYPE *rho_tab, FTYPE *P_tab, FTYPE *gamma_tab, FTYPE *eps_tab);
...
int Utoprim_2d(FTYPE U[NPR], FTYPE gcov[NDIM][NDIM], FTYPE gcon[NDIM][NDIM], 
	       FTYPE gdet, FTYPE prim[NPR], int &neos, int &ergo_star, FTYPE &ergo_sigma, FTYPE *rho_tab, FTYPE *P_tab,  FTYPE *eps_tab,  FTYPE *k_tab,  FTYPE *gamma_tab)
{
  ...
  //  if(prim_tmp[UTCON1]==0.0){printf("before Utoprim_new_body, prim_tmp[UTCON1] is 0 \n");}
  //if(prim_tmp[UTCON1+1]==0.0){printf("before Utoprim_new_body, prim_tmp[UTCON1+1] is 0 \n");}
  //if(prim_tmp[UTCON1+2]==0.0){printf("before Utoprim_new_body, prim_tmp[UTCON1+2] is 0 \n");}


    ret = Utoprim_new_body(U_tmp, gcov, gcon, gdet, prim_tmp, neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab);
     

    // if(prim_tmp[UTCON1]==0.0){printf("After Utoprim_new_body, prim_tmp[UTCON1] is 0 \n");}
    //if(prim_tmp[UTCON1+1]==0.0){printf("After Utoprim_new_body, prim_tmp[UTCON1+1] is 0 \n");}
    //if(prim_tmp[UTCON1+2]==0.0){printf("After Utoprim_new_body, prim_tmp[UTCON1+2] is 0 \n");}
...
static int Utoprim_new_body(FTYPE U[NPR], FTYPE gcov[NDIM][NDIM], 
			    FTYPE gcon[NDIM][NDIM], FTYPE gdet,  FTYPE prim[NPR], int &neos, int &ergo_star, FTYPE &ergo_sigma, FTYPE *rho_tab, FTYPE *P_tab, FTYPE *eps_tab,  FTYPE *k_tab,  FTYPE *gamma_tab)
{
  ...
  for(i=1;i<4;i++)
    for(j=1;j<4;j++){
      ...
      //if (utsq==0.0) {printf ("i=,j=,gcov[i][j],prim[UTCON1+i-1]=,prim[UTCON1+j-1]=, %d, %d, %e, %e, %e\n", i, j, gcov[i][j], prim[UTCON1+i-1],prim[UTCON1+j-1]);}
		}
  ...
  rho0 = D / gamma ;
  u = prim[UU] ;
  compute_pcold_epscold_cpp(rho0, P_cold, eps_cold, neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab);
  p = pressure_rho0_u(rho0,u,P_cold, eps_cold) ;
  w = rho0 + u + p ;

  //  if(w <= rho0){printf("[1] w < rho0!! rho0, u, p, w, P_cold, eps_cold = %.17g,%.17g,%.17g,%.17g,%.17g,%.17g \n", rho0, u, p, w, P_cold, eps_cold);} 
  ...
  if(isnan(x_2d[0])) { printf("before general N-R , W is nan. \n");}
  ...
  retval = general_newton_raphson( x_2d, n, func_vsq, Bsq,QdotBsq,Qtsq,Qdotn,D, P_cold, eps_cold, neos, ergo_star, ergo_sigma, rho_tab, P_tab, gamma_tab, eps_tab) ;  
  ...
  // if (w <= rho0) {printf ("w < rho0!!!, rho0 = %.17g, w = %.17g, p = %.17g,  D = %.17g, vsq = %.17g \n", rho0, w, p, D , vsq);}
  //if (w <= rho0){printf ("[2] w < rho0!!, rho0, u, p, w, P_cold, eps_cold = %.17g,%.17g,%.17g,%.17g,%.17g,%.17g \n", rho0, u, p, w, P_cold, eps_cold);}
...
static int general_newton_raphson( FTYPE x[], int n, 
			    void (*funcd) (FTYPE [], FTYPE [], FTYPE [], 
					   ...
					   FTYPE *,FTYPE *),FTYPE &Bsq,FTYPE &QdotBsq,FTYPE &Qtsq,FTYPE &Qdotn,FTYPE &D, FTYPE &P_cold, FTYPE &eps_cold, int &neos, int &ergo_star, FTYPE &ergo_sigma, FTYPE *rho_tab, FTYPE *P_tab, FTYPE *gamma_tab, FTYPE *eps_tab)
{
  ...
  while( keep_iterating ) { 
    ...
    //    if(x[1]==1.0) { printf("before func_vsq, vsq is 1. \n");}
    
    (*funcd) (x, dx, resid, jac, &f, &df, n, Bsq,QdotBsq,Qtsq,Qdotn,D, P_cold, eps_cold, neos, ergo_star, ergo_sigma, rho_tab, P_tab, gamma_tab, eps_tab);  /* returns with new dx, f, df */
...
static void func_vsq(FTYPE x[], FTYPE dx[], FTYPE resid[], 
		     ...
		     FTYPE *rho_tab, FTYPE *P_tab, FTYPE *gamma_tab, FTYPE *eps_tab)
{
  ...
  /*
  if(isnan(W)) { printf("inside func_vsq, W is nan. \n");}
  if(isnan(vsq)) { printf("inside func_vsq , vsq is nan. \n");}
  if(vsq==1.0) {printf("---Inside func_vsq, vsq is 1. \n");}
  */
  ...
  p_tmp  = pressure_W_vsq( W, vsq , D, P_cold, eps_cold);
...
static FTYPE pressure_W_vsq(FTYPE W, FTYPE vsq, FTYPE &D, FTYPE &P_cold, FTYPE &eps_cold) 
{
  ...
  //if(isnan(W)) {printf("---Inside pressure_W_vsq, W is nan, GAMMA_th =%e, W =%e, vsq =%e, D =%e, P_cold =%e, eps_cold =%e \n", GAMMA_th, W, vsq, D, P_cold, eps_cold);}
  if(vsq==1.0) {printf("---Inside pressure_W_vsq, vsq is 1, GAMMA_th =%e, W =%e, vsq =%e, D =%e, P_cold =%e, eps_cold =%e \n", GAMMA_th, W, vsq, D, P_cold, eps_cold);}
...
static FTYPE dpdvsq_calc(FTYPE W, FTYPE vsq, FTYPE &D, FTYPE &P_cold, FTYPE &eps_cold, int &neos, int &ergo_star, FTYPE &ergo_sigma, FTYPE *rho_tab, FTYPE *P_tab, FTYPE *gamma_tab, FTYPE *eps_tab)
{
  ...
  /*
  if(isnan(gtmp)) { printf("inside dpdvsq_calc , gtmp is nan. \n");}
  if(isnan(rho0)) { printf("inside dpdvsq_calc , rho0 is nan. \n");}
  if(isnan(gamma_cold)) { printf("inside dpdvsq_calc , gamma_cold is nan. \n");}
  if(isnan(drho_dvsq)) { printf("inside dpdvsq_calc , drho_dvsq is nan. \n");}
  if(isnan(gammas)) { printf("inside dpdvsq_calc , gammas is nan. \n");}
  */
  ...
  if (ergo_star == 0 || (ergo_star == 1 && rho0 <= rho_tab[neos-1])){
    return ( -(GAMMA_th - 1.) * (drho_dvsq*(1.0+eps_cold+P_cold/rho0) + W ) + (gammas * P_cold * drho_dvsq/rho0 ))/GAMMA_th;
  }
  else {
    ...
    /*
    if(isnan(h_tab)) { printf("inside dpdvsq_calc , h_tab is nan. \n");}
    if(isnan(Con1)) { printf("inside dpdvsq_calc , Con1 is nan. \n");}
    if(isnan(Con2)) { printf("inside dpdvsq_calc , Con2 is nan. \n");}
    */


    //return ( -(GAMMA_th - 1.) * (drho_dvsq* ((1+eps_cold) +  (Con1 + ergo_sigma*(1+eps_tab[neos-1]) * rho_tab[neos-1]/rho0)/(ergo_sigma+1) ) + W ) + Con1*drho_dvsq )/GAMMA_th; 
