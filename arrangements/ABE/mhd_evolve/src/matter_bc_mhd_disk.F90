  ...
  if (Symmetry == AXISYM) then
     if (have_bdry_max(1)==1) then ! do outer x boundary
		...
		E_radl,F_radxl,F_radyl,F_radzl,tau_radl, S_rad_xl, S_rad_yl, S_rad_zl,&
                ...
                neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab, gamma_th,rho_b_atm_gf(i,j,k),pfloor_gf(i,j,k),enable_HARM_energyvariable,enable_OS_collapse,rad_closure_scheme)
...
subroutine recompute_vars_disk(rho_bl,Pl,vxl,vyl,vzl,Bxl,Byl,Bzl,AUX,m,rho_sl,taul, &
		...
		E_radl,F_radxl,F_radyl,F_radzl, tau_radl,S_rad_xl, S_rad_yl, S_rad_zl,&
                sbtl,sbxl,sbyl,sbzl,rhol,Sxl,Syl,Szl,Sxxl,Sxyl,Sxzl,Syyl,Syzl,Szzl, &
                neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab, gamma_th,rho_b_atm,pfloor,enable_HARM_energyvariable, enable_OS_collapse,rad_closure_scheme)
  implicit none
  integer                   :: m 
  integer                   :: neos,ergo_star,enable_OS_collapse,rad_closure_scheme
  real*8, dimension(neos)   :: rho_tab,P_tab,eps_tab
 ...
 ! Add radiation terms in the source.                                                                                                                                                                      
  ...
  S_rad_xl = alpn1*Psi6*(E_radl*u0l*u_x + F_rad0l*u_x + F_rad_yl * u0l + P_rad00l*shift_x + Psi4*(P_rad0xl*gxx + P_rad0yl*gxy + P_rad0zl*gxz))
