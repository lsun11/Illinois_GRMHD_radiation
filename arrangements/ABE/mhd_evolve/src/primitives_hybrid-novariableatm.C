...
(int *ext,int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau,
 ...
 int &tau_stildefix_enable,double &tau_atm,double *failure_tracker,const cGH *cctkGH,int &ignore_ghostzones, int &enable_shocktest_primitive_mode, int &repairs_needed) {
  ...
  compute_pcold_epscold_cpp(rho_b_atm, P_tiny, eps_tiny,neos,rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++)
	...
	double UUguess[5];
	   ...
	   if (rho_b[index] < rho_b_atm) rho_b[index] = rho_b_atm;

	   double alpn1 = alpha[index] + 1.0;
	   double alpn1_inv = 1.0/(alpha[index] + 1.0);
	   
	   double rho_s = rho_star[index];
	   if (rho_b_atm*Psi6*0.99 >= rho_star[index]) rho_s = -1.0;
	   ...
	   //if (rho_s > Tiny && Psi6<50.0) {
	     ...
	     */
	     ...
	     if (rho_bl < rho_b_atm && enable_shocktest_primitive_mode==0) {
	       recom = true;
	       rho_bl = rho_b_atm;
	     ...
	     if ((rho_bl < 100.0*rho_b_atm && enable_shocktest_primitive_mode==0) || (Psi6>10.0)) {
