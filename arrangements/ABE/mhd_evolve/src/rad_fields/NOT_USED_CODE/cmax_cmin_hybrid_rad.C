...
extern "C" void CCTK_FCALL CCTK_FNAME(compute_cmax_cmin_hybrid_rad_cpp)
  (const cGH **cctkGH,int *ext,double *v02r,double *v02l,double *cmax,double *cmin,double *rho_br,double *rho_bl,
   ...
   double *cplusr,double *cminusr,double *u0,double *cplusl,double *cminusl, int &enable_OS_collapse);

double fasterpow_cmaxcmin_rad(double inputvar,double inputpow);

void find_cplus_cminus_rad_cpp(const cGH *cctkGH,int *ext,double *cplus,double *cminus,double *v02,double *u0, 
			   double *vi,double *lapse,double *shifti,double *phi,double *gupii);

void find_v02_hybrid_rad_cpp(const cGH *cctkGH,int *ext,double *v02,double *rho_b,double *P,
			 ...
			 double *lapse,double *shiftx,double *shifty,double *shiftz,double * phi,
			     double *gxx,double *gxy,double *gxz,double *gyy,double *gyz,double *gzz, int &enable_OS_collapse);
...
void compute_cmax_cmin_hybrid_rad_cpp(const cGH *cctkGH,int *ext,double *v02r,double *v02l,double *cmax,double *cmin,double *rho_br,double *rho_bl,
  ...
  //printf("hi inside compute_cmax_cmin_hybrid_cpp m=%d\n",m);

  find_v02_hybrid_rad_cpp(cctkGH,ext,v02r,rho_br,Pr,u0,vxr,vyr,vzr,
		      neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab, gamma_th,
			  Bxr,Byr,Bzr, alpha,betax,betay,betaz, phi,gxx,gxy,gxz,gyy,gyz,gzz, enable_OS_collapse);
    
  if (m==1) {
    find_cplus_cminus_rad_cpp(cctkGH,ext,cplusr,cminusr,v02r,u0,
  ...
  find_v02_hybrid_rad_cpp(cctkGH,ext,v02l,rho_bl,Pl,u0,vxl,vyl,vzl,
		      neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab, gamma_th,
			  Bxl,Byl,Bzl, alpha,betax,betay,betaz, phi,gxx,gxy,gxz,gyy,gyz,gzz, enable_OS_collapse);
  
  if (m==1) {
    find_cplus_cminus_rad_cpp(cctkGH,ext,cplusl,cminusl,v02l,u0,
  ...
  for(int k=kstart;k<kend;k++)
	...
	if(rho_bL <= rho_tab[0]) {
	  if(enable_OS_collapse == 1){
	    ...
	    dPcold_drho = k_tab[0];
	    v02[index] = k_tab[0]*fasterpow_cmaxcmin_rad(rho_bL,gamma_tab[0]);
	    eps_th = 0.0;
	    va2 = 0.0;
	    h = 1.0;
	  }
	  ...
	  if (rho_bL <= 0.0) {
	    ...
	    v02[index] = k_tab[0]*fasterpow_cmaxcmin_rad(rho_bL,gamma_tab[0]);           // v02[index] -> P_cold
	...
	if (rho_bL <= rho_tab[ii] && rho_bL > rho_tab[ii-1]) {
	  v02[index] = k_tab[ii]*fasterpow_cmaxcmin_rad(rho_bL,gamma_tab[ii]);	       // v02[index] -> P_cold
	...
	if(rho_bL > rho_tab[neos-1]) {
	  if (ergo_star == 0){
	  v02[index] = k_tab[neos]*fasterpow_cmaxcmin_rad(rho_bL,gamma_tab[neos]);   // v02[index] -> P_cold
	  ...
	  v02[index] = (ergo_sigma* (1+eps_tab[neos-1]+P_tab[neos-1]/rho_tab[neos-1])/fasterpow_cmaxcmin_rad(rho_tab[neos-1],ergo_sigma)* fasterpow_cmaxcmin_rad(rho_bL, ergo_sigma+1) + P_tab[neos-1] - ergo_sigma*((1+eps_tab[neos-1])*rho_tab[neos-1]) )/(ergo_sigma+1);
          va2 = ((1+eps_tab[neos-1]+P_tab[neos-1]/rho_tab[neos-1])/fasterpow_cmaxcmin_rad(rho_tab[neos-1],ergo_sigma) * fasterpow_cmaxcmin_rad(rho_bL, ergo_sigma+1) - P_tab[neos-1] + ergo_sigma*((1+eps_tab[neos-1])*rho_tab[neos-1]) )/((ergo_sigma+1)*rho_bL)-1;
	  dPcold_drho = gamma_tab[neos]*v02[index]/rho_bL;
	  eps_th = (PL-v02[index])/(gamma_th-1.0)/rho_bL;
	  }
	 h = 1.0 + eps_th + va2 + PL/rho_bL;
   ...
   double *cplusr,double *cminusr,double *u0,double *cplusl,double *cminusl, int &enable_OS_collapse)
{
  compute_cmax_cmin_hybrid_rad_cpp(*cctkGH,ext,v02r,v02l,cmax,cmin,rho_br,rho_bl,
			       ...
			       gamma_tab,gamma_th,
				   cplusr,cminusr,u0,cplusl,cminusl, enable_OS_collapse);
}
