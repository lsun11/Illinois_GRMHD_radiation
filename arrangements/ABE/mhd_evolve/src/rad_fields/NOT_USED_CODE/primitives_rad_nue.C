		 ...
		 struct auxarray_rad &aux,double *fvec)
{
  ...
  for (slope=0.0,i=1;i<=n;i++)
    slope += g[i]*p[i];
  if (slope >= 0.0 && x[1]*x[2]*x[3]!=0.0) {printf("OUCH!  Roundoff problem in lnsrch.  Note that this function has been updated to correct a bug in Numerical Recipes 2.06.  This is version 2.08, with the 2.06->2.08 diff obtained from http://www.numerical-recipes.com/upgrade/upgrade-208.html  x[1-4] = %e,%e,%e,%e \n", xold[1],xold[2],xold[3],xold[4]);
