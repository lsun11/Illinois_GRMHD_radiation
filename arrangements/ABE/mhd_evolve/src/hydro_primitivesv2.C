...
void w_solver_tau_n1(double &w,double &s2,double &tau,double &rho_s,double &h,double &P,double &eps,
...
inline int gsl_poly_solve_quartic (double a, double b, double c, double d,
				   double *x0, double *x1, double *x2, double *x3);

extern "C" void CCTK_FCALL CCTK_FNAME(hydro_primitives)
  (const cGH **cctkGH,int *ext, int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau, 
   ...
   int *Symmetry, int &ignore_ghostzones);

extern "C" void hydro_primitives(const cGH *cctkGH,int *ext, int *nghostzones, double *X, double *Y, double *Z, 
  ...
  if(n_poly != 1.0 && tau_stilde_fix_enable==0) {
...
#pragma omp parallel for
  for(int k=kmin;k<kmax;k++) for(int j=jmin;j<jmax;j++) for(int i=imin;i<imax;i++) {
    ...
    // Exclude the regions that are not in the ghostzones when 
    // ignore_ghostzones==-1.
    if ( ignore_ghostzones==-1) { 
         if ( (k>nghostzones[2]-1 && k<kmax-nghostzones[2]) &&
              (j>nghostzones[1]-1 && j<jmax-nghostzones[1]) &&
              (i>nghostzones[0]-1 && i<imax-nghostzones[0]) ) 
    ...
    if (compute_primitives==1) {
       ...
       if(rho_s>=rho_b_atm*Psi6*0.99 || enable_shocktest_mode==1) {
         ...
         int num_first_roots;
         w_solver_tau_n1(w_l,s_dot_s,tau_l,rho_s,h_l,P_l,eps,Psi6,alpha_l,
         ...
         if(tau_stilde_fix_applied==true && fixed==true) {
           ...
           printf("gij %e %e %e %e %e %e\n",gxxL*Psi4,gxyL*Psi4,gxzL*Psi4,gyyL*Psi4,gyzL*Psi4,gzzL*Psi4);
           printf("lapbeta %e %e %e %e\n",alpha[index],betaxL,betayL,betazL);
...
//------------------------------------------------------------------
void w_solver_tau_n1(double &w,double &s_dot_s,double &tau,double &rho_s,double &h,double &P,double &eps,
		     double Psi6,double alpha,double rho_atm, double k_poly,double n_poly,int ii,int j,int k,int enable_shocktest_mode,bool &fixed,
		     int &num_first_roots,double *first_roots,double *poly_coeffs,bool &tau_stilde_fix_applied) {
  int polish=1;
  ...
  double roots[5];
  int numroots=gsl_poly_solve_quartic (a,b,c,d,&roots[1],&roots[2],&roots[3],&roots[4]);
...
extern "C" void CCTK_FCALL CCTK_FNAME(hydro_primitives)
  (const cGH **cctkGH,int *ext, int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau, 
   ...
   int *Symmetry, int &ignore_ghostzones)
{
  hydro_primitives(*cctkGH,ext, nghostzones, X, Y, Z, rho_star, tau,
		   st_x, st_y, st_z,
		   u0,vx,vy,vz,
		   ...
		   *rho_max, rho_fail_max_step, M_fail_step, *rho_b_atm,
                   *gamma_th,*k_poly,*sdots_o_rhot,*Symmetry,ignore_ghostzones);
}
