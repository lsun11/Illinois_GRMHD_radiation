...
subroutine primitive_vars_hybrid2(ext,nghostzones, X, Y, Z, rho_star, tau, &
  ...
  if(ignore_ghostzones==0 .or. ignore_ghostzones==-1) then
     ...
     write(*,*) "YOU MUST SET ignore_ghostzones (last variable in function call) to zero (compute everywhere on grid) or one (ignore ghostzones) or -1 (only compute inside the ghostzones)."
     stop
  end if
              ...
              if (rho_s .gt. Tiny) then
                 if(tau_stildefix_enable == 1) then
                    ...
                    if(sdots.gt.0.98*rhot) then
                       ! the stuff after the .and. is so that we don't get 
...
!              if(abs(Y(i,j,k)+(dY*0.5D0)).lt.dY*0.001 .and. abs(X(i,j,k)+(dX*0.5D0)).lt.dX*0.001 .and. abs(Z(i,j,k)-(dZ*9.5D0)).lt.dZ*0.001) then
                    ...
                    write(*,*) "inside primitives0", rho_b(i,j,k),mhd_st_x(i,j,k),mhd_st_y(i,j,k),mhd_st_z(i,j,k),h_old(i,j,k),gupxy(i,j,k)
...
!**********   **************************************************************************************************** 
                 else ! Inversion works without Font fix, do the following:
	         ...
	         h_l = 1.d0 + P_l/rho_bl + eps
                 ...
                 if(i==-22 .and. j==24 .and. k==3) then
                    ...
                    write(*,*) uy_l,shifty(i,j,k)*u0(i,j,k),AUX(16)*u_xl + AUX(18)*u_yl + AUX(19)*u_zl,AUX(16)*u_xl,AUX(18)*u_yl,AUX(19)*u_zl,AUX(18),u_yl
                    !write(*,*) uy_l,shifty(i,j,k)*u0(i,j,k),AUX(16)*u_xl +AUX(18)*u_yl + AUX(19)*u_zl
                    !write(*,*) uy_l,shifty(i,j,k),u0(i,j,k),AUX(16),u_xl,AUX(18),u_yl,AUX(19),u_zl,-shifty(i,j,k)*u0(i,j,k),AUX(16)*u_xl +AUX(18)*u_yl + AUX(19)*u_zl
	        ...
	        ! Set rho_b to the atmosphere density and u_i=0
	         ...
	         rho_b(i,j,k) = rho_bl
              ...
              if(i==200000 .and. j==2 .and. k==3) then
                 write(*,*) 'rho_star = ',rho_star(i,j,k),vx(i,j,k),ux_l,shiftx(i,j,k),u0(i,j,k),u_xl,u_yl,u_zl,bb,P_l,alpn1,rho_bl,h_l,B2
                 write(*,*) 'tau = ',tau(i,j,k)
                 write(*,*) 'mhd_st_x = ',mhd_st_x(i,j,k)
                 write(*,*) 'mhd_st_y = ',mhd_st_y(i,j,k)
                 write(*,*) 'mhd_st_z = ',mhd_st_z(i,j,k)
...
!
  ...
  if (rhob .gt. rho_tab(neos)) then
	...
	P_cold = ((ergo_sigma* (1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rhob**(ergo_sigma+1) + P_tab(neos) - ergo_sigma*(1+eps_tab(neos))*rho_tab(neos))/(ergo_sigma+1)
        eps_cold = (((1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rhob**(ergo_sigma+1) - P_tab(neos) + ergo_sigma*((1+eps_tab(neos))*rho_tab(neos)))/((ergo_sigma+1)*rhob)-1
        dPcold_drho = ergo_sigma*(1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma*rhob
	depscold_drho = (ergo_sigma*(1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma*rhob**(ergo_sigma-1) + (P_tab(neos) - ergo_sigma*(1+eps_tab(neos))*rho_tab(neos))/rhob**2)/(ergo_sigma+1)
     end if
  end if
...
!
  ...
  if (rhob .gt. rho_tab(neos)) then
     if (ergo_star .eq. 0) then
        ...
        P = ((ergo_sigma* (1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rhob**(ergo_sigma+1) + P_tab(neos) - ergo_sigma*(1+eps_tab(neos))*rho_tab(neos))/(ergo_sigma+1)
        eps = (((1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma) * rhob**(ergo_sigma+1) - P_tab(neos) + ergo_sigma*((1+eps_tab(neos))*rho_tab(neos)))/((ergo_sigma+1)*rhob)-1
        dP_drho = ergo_sigma*(1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma*rhob
        deps_drho = (ergo_sigma*(1+eps_tab(neos)+P_tab(neos)/rho_tab(neos))/rho_tab(neos)**ergo_sigma*rhob**(ergo_sigma-1) + (P_tab(neos) - ergo_sigma*(1+eps_tab(neos))*rho_tab(neos))/rhob**2)/(ergo_sigma+1)
     end if
  end if
...
!!! Don't repair MHD part when using the harm primitives solver
		 ...
		 call compute_pcold_epscold(rho_bl, P_cold, eps_cold, &
            ...
            ymax_rad: if ((j .ne. jmax) .and. (j .ne. jmin)) then
              ...
              if (rad_closure_scheme.eq.0) then
                 ...
                 tau_rad(i,j,k) = alpn1*alpn1*Psi6*(E_radl*u0(i,j,k)*u0(i,j,k)+2.0*F_rad0l*u0(i,j,k)+P_radl*u0(i,j,k)*u0(i,j,k))-Psi6*P_radl
                 ...
                 Sxx(i,j,k) = Sxx(i,j,k) + (E_radl+P_radl)*(u0(i,j,k)*(beta_x + v_xl))**2 + 2.0*F_rad_xl*u0(i,j,k)*(beta_x + v_xl) + Psi4 * P_radl * gxx(i,j,k)
                 Syy(i,j,k) = Syy(i,j,k) + (E_radl+P_radl)*(u0(i,j,k)*(beta_y + v_yl))**2 + 2.0*F_rad_yl*u0(i,j,k)*(beta_y + v_yl) + Psi4 * P_radl * gyy(i,j,k)
                 Szz(i,j,k) = Szz(i,j,k) + (E_radl+P_radl)*(u0(i,j,k)*(beta_z + v_zl))**2 + 2.0*F_rad_zl*u0(i,j,k)*(beta_z + v_zl) + Psi4 * P_radl * gzz(i,j,k)
                 Sxy(i,j,k) = Sxy(i,j,k) + (E_radl+P_radl)*u0(i,j,k)**2*(beta_x + v_xl)*(beta_y + v_yl) + u0(i,j,k)*(F_rad_xl*(beta_y + v_yl)+F_rad_yl*(beta_x + v_xl)) + Psi4 * P_radl * gxy(i,j,k)
                 Sxz(i,j,k) = Sxz(i,j,k) + (E_radl+P_radl)*u0(i,j,k)**2*(beta_x + v_xl)*(beta_z + v_zl) + u0(i,j,k)*(F_rad_xl*(beta_z + v_zl)+F_rad_zl*(beta_x + v_xl)) + Psi4 * P_radl * gxz(i,j,k)
                 Syz(i,j,k) = Syz(i,j,k) + (E_radl+P_radl)*u0(i,j,k)**2*(beta_y + v_yl)*(beta_z + v_zl) + u0(i,j,k)*(F_rad_yl*(beta_z + v_zl)+F_rad_zl*(beta_y + v_yl)) + Psi4 * P_radl * gyz(i,j,k)                 
              else
                 ...
                 if (E_radl .lt. Erad_atm_cut) then
                    ...
                    if (Fasq .le. 0) then
                       ...
                       P_radxxl = E_radl*((F_radxl**2/Fasq)*(3.0*chi -1.0)/2.0 + (gupxx(i,j,k)/Psi4 - betax(i,j,k)*betax(i,j,k)/alpn1**2 + uxl**2)/2.0*(1.0-chi))
                       P_radyyl = E_radl*((F_radyl**2/Fasq)*(3.0*chi -1.0)/2.0 + (gupyy(i,j,k)/Psi4 - betay(i,j,k)*betay(i,j,k)/alpn1**2 + uyl**2)/2.0*(1.0-chi))
                       P_radzzl = E_radl*((F_radzl**2/Fasq)*(3.0*chi -1.0)/2.0 + (gupzz(i,j,k)/Psi4 - betaz(i,j,k)*betaz(i,j,k)/alpn1**2 + uzl**2)/2.0*(1.0-chi))
                       P_radxyl = E_radl*((F_radxl*F_radyl/Fasq)*(3.0*chi -1.0)/2.0 + (gupxy(i,j,k)/Psi4 - betax(i,j,k)*betay(i,j,k)/alpn1**2 + uxl*uyl)/2.0*(1.0-chi))
                       P_radxzl = E_radl*((F_radxl*F_radzl/Fasq)*(3.0*chi -1.0)/2.0 + (gupxz(i,j,k)/Psi4 - betax(i,j,k)*betaz(i,j,k)/alpn1**2 + uxl*uzl)/2.0*(1.0-chi))
                       P_radyzl = E_radl*((F_radyl*F_radzl/Fasq)*(3.0*chi -1.0)/2.0 + (gupyz(i,j,k)/Psi4 - betay(i,j,k)*betaz(i,j,k)/alpn1**2 ++ uyl*uzl)/2.0*(1.0-chi))
                    end if
                 end if
                 
                 !                 infinity = HUGE(dbl_prec_var)
                 if (alpn1==0.0 .or. isnan(Psi4) .or. isnan(1.0/Psi4)) then 
                    write(*,*) "Inside primitives_hybrid.F90, Psi4 is zero or Inf, Psi4, chi, Fasq, zeta, E_radl, F_radxl are", Psi4, chi, Fasq, zeta, E_radl, F_radxl
                 end if
                 ...
                 S_rad_x(i,j,k) = alpn1*Psi6*(E_radl*u0(i,j,k)*u_xl + F_rad0l*u_xl + F_rad_xl * u0(i,j,k) + P_rad00l*beta_x + Psi4*(P_rad0xl*gxx(i,j,k) + P_rad0yl*gxy(i,j,k) + P_rad0zl*gxz(i,j,k)))
                 S_rad_y(i,j,k) = alpn1*Psi6*(E_radl*u0(i,j,k)*u_yl + F_rad0l*u_yl + F_rad_yl * u0(i,j,k) + P_rad00l*beta_y + Psi4*(P_rad0xl*gxy(i,j,k) + P_rad0yl*gyy(i,j,k) + P_rad0zl*gyz(i,j,k)))
                 S_rad_z(i,j,k) = alpn1*Psi6*(E_radl*u0(i,j,k)*u_zl + F_rad0l*u_zl + F_rad_zl * u0(i,j,k) + P_rad00l*beta_z + Psi4*(P_rad0xl*gxz(i,j,k) + P_rad0yl*gyz(i,j,k) + P_rad0zl*gzz(i,j,k)))
                 ...
                 Sxy(i,j,k) = Sxy(i,j,k) + E_radl*u_xl*u_yl + F_rad_xl*u_yl + F_rad_yl*u_xl +&
                      ...
                      Psi4*(beta_x*(gxy(i,j,k)*P_rad0xl + gyy(i,j,k)*P_rad0yl + gyz(i,j,k)*P_rad0zl) + (beta_y*(gxx(i,j,k)*P_rad0xl + gxy(i,j,k)*P_rad0yl + gxz(i,j,k)*P_rad0zl)) )+&
                      Psi4**2.0*(gxx(i,j,k)*gxy(i,j,k)*P_radxxl + gxy(i,j,k)*gyy(i,j,k)*P_radyyl + gxz(i,j,k)*gyz(i,j,k)*P_radzzl +&
                      (gxx(i,j,k)*gyy(i,j,k) + gxy(i,j,k)*gxy(i,j,k))*P_radxyl + (gxx(i,j,k)*gyz(i,j,k) + gxz(i,j,k)*gxy(i,j,k))*P_radxzl + (gxy(i,j,k)*gyz(i,j,k) + gxz(i,j,k)*gyy(i,j,k))*P_radyzl)
                 
                 Sxz(i,j,k) = Sxz(i,j,k) + E_radl*u_xl*u_zl + F_rad_xl*u_zl + F_rad_zl*u_xl +&
                      beta_x*beta_z*P_rad00l +&
                      Psi4*(beta_x*(gxz(i,j,k)*P_rad0xl + gyz(i,j,k)*P_rad0yl + gzz(i,j,k)*P_rad0zl) + (beta_z*(gxx(i,j,k)*P_rad0xl + gxy(i,j,k)*P_rad0yl + gxz(i,j,k)*P_rad0zl)) )+&
                      Psi4**2.0*(gxx(i,j,k)*gxz(i,j,k)*P_radxxl + gxy(i,j,k)*gyz(i,j,k)*P_radyyl + gxz(i,j,k)*gzz(i,j,k)*P_radzzl +&
                      (gxx(i,j,k)*gyz(i,j,k) + gxy(i,j,k)*gxz(i,j,k))*P_radxyl + (gxx(i,j,k)*gzz(i,j,k) + gxz(i,j,k)*gxz(i,j,k))*P_radxzl + (gxy(i,j,k)*gzz(i,j,k) + gxz(i,j,k)*gyz(i,j,k))*P_radyzl)
                 
                 Syz(i,j,k) = Syz(i,j,k) + E_radl*u_yl*u_zl + F_rad_yl*u_zl + F_rad_zl*u_yl +&
                      beta_y*beta_z*P_rad00l +&
                      Psi4*(beta_y*(gxz(i,j,k)*P_rad0xl + gyz(i,j,k)*P_rad0yl + gzz(i,j,k)*P_rad0zl) + (beta_z*(gxy(i,j,k)*P_rad0xl + gyy(i,j,k)*P_rad0yl + gyz(i,j,k)*P_rad0zl)) )+&
                      Psi4**2.0*(gxy(i,j,k)*gxz(i,j,k)*P_radxxl + gyy(i,j,k)*gyz(i,j,k)*P_radyyl + gyz(i,j,k)*gzz(i,j,k)*P_radzzl +&
                      (gxy(i,j,k)*gyz(i,j,k) + gxz(i,j,k)*gyy(i,j,k))*P_radxyl + (gxy(i,j,k)*gzz(i,j,k) + gyz(i,j,k)*gxz(i,j,k))*P_radxzl + (gyy(i,j,k)*gzz(i,j,k) + gyz(i,j,k)*gyz(i,j,k))*P_radyzl)
