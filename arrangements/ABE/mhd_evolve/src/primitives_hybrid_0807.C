...
void newt2_cpp(double x[],struct auxarray &aux,
	       void (*function_hybrd)(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse),
	       void (*jacobian_hybrd)
...
*/
void function_hybrd(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,int &enable_OS_collapse);
void function_hybrd_font_fix(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab, int &enable_OS_collapse);
void jacobian_hybrd
(int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5], int &enable_OS_collapse);
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++)
	...
	double UUguess[5];
	   ...
	   if (isnan(tau_radl)||isnan(S_rad_xl)){printf("inside primitive hybrid!!! tau_radl=%e, S_rad_xl=%e, i,j,k=%d,%d,%d, \n", tau_radl, S_rad_xl, i,j,k);}

	   /*	 
	   double E_rad_denom = -Psi6*((au0m1+1.0)/3.0 + 1.0/(6.0*(au0m1+1.0)) );
	   ...
	   double F_rad0l = (tau_radl/Psi6- (4.0*SQR(au0m1+1.0) - 1.0)/3.0 * E_radl)/(2.0*(au0m1+1.0)*alpn1);
	   */ 
	   ...
	   double F_rad0_num = 4.0*au0*(1.0 - au0*au0)/(4.0*au0*au0 - 1.0)*tau_radl
	                  +( (shiftxL*u0[index]+ux_l)*S_rad_xl                                                                         
                            +(shiftyL*u0[index]+uy_l)*S_rad_yl                                                                         
			    +(shiftzL*u0[index]+uz_l)*S_rad_zl);
	   double F_rad0l = F_rad0_num/F_rad0_denom;
	   double E_radl = 3.0*(tau_radl/Psi6 - 2.0*alpn1*au0*F_rad0l)/(4.0*au0*au0 - 1.0);
	   double P_radl = E_radl/3.0;
...
void compute_pcold_epscold_cpp(double &rhob, double &P_cold, double &eps_cold, 
  ...
  if (enable_OS_collapse == 1){
    ...
    eps_cold = P_cold/rhob/(gamma_tab[0]-1.0);
...
(int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5], int &enable_OS_collapse) {
  ...
  if (enable_OS_collapse==1)
    ...
    P = P_cold + (aux.gamma_th - 1.0)*(eps - eps_cold)*rhob;
