...
struct auxarray {
  ...
  //vv TESTING vv
  double rhobatm,K_old;
  //^^ TESTING ^^
};

#include "primitives_hybrid_debug.h"
...
void rho_b_atm_variable_falls_off_with_radius(int index,double *X,double *Y,double *Z,double &rhobatm);
void set_pressure_floor_ceiling(double &P_cold,double &eps_cold,double &rho_bl,double &gamma_th,bool &recom,double &P_l,double &eps,double &h_l,
				int &num_Pmaxfixesinhoriz,int &num_Pminfixesinhoriz,double &Psi6,double &Kmin,double &Kmax);
void tau_stilde_fix(int &index,double *tau,double &tau_atm,bool &tau_stilde_fix_applied,double &rho_s,
		    double *mhd_st_x,double *mhd_st_y,double *mhd_st_z,
		    double *gupxx,double *gupxy,double *gupxz,double *gupyy,double *gupyz,double *gupzz,double &Psim4);
void set_AUX_B2(struct auxarray &AUX,double &B2,int &index,double &rho_s,double *tau,double &gamma_th,double &f1o4pa,double &BxL,double &ByL,double &BzL,
		double &gxxL,double &gxyL,double &gxzL,double &gyyL,double &gyzL,double &gzzL,
		double *mhd_st_x,double *mhd_st_y,double *mhd_st_z,double &alpn1,double &B_xl,double &B_yl,double &B_zl,double &Psim4,
		double *gupxx,double *gupxy,double *gupxz,double *gupyy,double *gupyz,double *gupzz,double &Psi2,double &Psi4,double &Psi6);

void velocity_limiter(double &u_xl,double &u_yl,double &u_zl,int &index,struct auxarray &AUX,double &alpn1,double &alpn1_inv,
		      double &au0m1,int &enable_shocktest_primitive_mode,bool &recom,double *u0,double &rho_s,double &rho_bl);
...
extern "C" void CCTK_FCALL CCTK_FNAME(primitive_vars_hybrid2_cpp)
  (int *ext,int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau,
   ...
   int &tau_stilde_fix_enable,double &tau_atm,double *failure_tracker,const cGH **cctkGH,int &ignore_ghostzones, int &enable_shocktest_primitive_mode, int &repairs_needed);
...
#pragma omp parallel for
  ...
  */
  ...
  //
  int num_fontfixesinhoriz=0;
  int num_ptsinhoriz=0;
  int num_Pminfixesinhoriz=0;
  int num_Pmaxfixesinhoriz=0;
  int recom_inhoriz=0;
  int tau_stilde=0;
  int num_fontfixesoverall=0;
  int num_ptsoverall=0;
#pragma omp parallel for reduction(+:num_fontfixesinhoriz,num_Pmaxfixesinhoriz,num_Pminfixesinhoriz,recom_inhoriz,tau_stilde,num_ptsinhoriz,num_fontfixesoverall,num_ptsoverall) schedule(static)
  for(int k=kstart;k<kend;k++)
    for(int j=jstart;j<jend;j++)
      for(int i=istart;i<iend;i++) {
	...
	double UUguess[5];
	  ...
	  if(rho_b_atm<0) rho_b_atm_variable_falls_off_with_radius(index,X,Y,Z,rhobatm);
	  ...
	  if (rho_s > Tiny) {
	    ...
	    if(tau_stilde_fix_enable==1 || (tau_stilde_fix_enable==2 && BxL==0 && ByL==0 && BzL==0)) {
	      //if(tau_stilde_fix_enable==1 || (tau_stilde_fix_enable==2 && BxL==0 && ByL==0 && BzL==0) || (tau_stilde_fix_enable==3 && Psi6>15.0)) {
	      tau_stilde_fix(index,tau,tau_atm,tau_stilde_fix_applied,rho_s,mhd_st_x,mhd_st_y,mhd_st_z,gupxx,gupxy,gupxz,gupyy,gupyz,gupzz,Psim4);
	    }
	    ...
	    if(Psi6>25.0) num_ptsinhoriz++;
	    num_ptsoverall++;

	    set_AUX_B2(AUX,B2,index,rho_s,tau,gamma_th,f1o4pa,BxL,ByL,BzL,
		       gxxL,gxyL,gxzL,gyyL,gyzL,gzzL,mhd_st_x,mhd_st_y,mhd_st_z,
		       alpn1,B_xl,B_yl,B_zl,Psim4,gupxx,gupxy,gupxz,gupyy,gupyz,gupzz,Psi2,Psi4,Psi6);
	    ...
	    int MAXITS_taustildefix=MAXITS_beforefontfix*1; //  <-- Try harder when doing a Font fix; we don't want to fail here!!!
	    ...
	    if(!check) {
	      ...
	      eps  = fabs(UU[4]*eps_scal);
	      double gijuiujL = AUX.gupxx_phys*SQR(u_xl) + 
		...
		AUX.gupzz_phys*SQR(u_zl);
	      double au0m1L = gijuiujL/( 1.0+sqrt(1.0+gijuiujL) );
	      // *** Limit velocity
	      if (au0m1L > MAX_GAMMAM1-1.0) {
		double fac = sqrt((SQR(MAX_GAMMAM1)-1.0)/(SQR(1.0+au0m1L) - 1.0));
		u_xl = fac*u_xl;
		u_yl = fac*u_yl;
		u_zl = fac*u_zl;
		gijuiujL = gijuiujL * SQR(fac);
		au0m1L = gijuiujL/( 1.0+sqrt(1.0+gijuiujL) );
	      }
	      u0[index] = (au0m1L+1.0)*alpn1_inv;
	      if (rho_s < 0.0) u0[index] = -u0[index];
	      rho_bl = rho_s/(alpn1*u0[index]*AUX.Psi6);

	      if(rho_bl<AUX.rhobatm) rho_bl=1.00001*AUX.rhobatm;
	      ...
	      if ((rho_bl < 100.0*AUX.rhobatm) || (AUX.Psi6>15.0)) {
		double Kmin=0.85;
		double Kmax=8.0;
		set_pressure_floor_ceiling(P_cold,eps_cold,rho_bl,AUX.gamma_th,bla,P_l,eps,h_l,blah1,blah2,AUX.Psi6,Kmin,Kmax);
		double P_old=P_l;
		P_l = P_cold + rho_bl*(gamma_th-1.0)*(eps-eps_cold);
		if(fabs(P_l-P_old)/P_old>1e-14) {
		  printf("error: %.16e %.16e\n",P_l,P_old);
		  exit(0);
		}
	      }

	      UU[1] = u_xl/(Psi2*u_scal);
	      UU[2] = u_yl/(Psi2*u_scal);
	      UU[3] = u_zl/(Psi2*u_scal);
	      UU[4] = fabs(eps/eps_scal);
	      //^^ TESTING ^^

	      if(rho_bl<rhobatm) { printf("BAD\n"); exit(0); }
	    }

	    int fontfixed=0;
	    bool quartic_solved=false;
		...
		double sb_y = 0.0;
		double sb_z = 0.0;
		rho_s = alpn1*u0[index]*rho_bl*Psi6;
		rho_star[index] = rho_s;
		mhd_st_x[index] = rho_s*h_l*u_xl + 
		  alpn1*Psi6*u0[index]*sb2*u_xl - alpn1*Psi6*sb0*sb_x;
		mhd_st_y[index] = rho_s*h_l*u_yl +
		  alpn1*Psi6*u0[index]*sb2*u_yl - alpn1*Psi6*sb0*sb_y;
		mhd_st_z[index] = rho_s*h_l*u_zl + 
		  alpn1*Psi6*u0[index]*sb2*u_zl - alpn1*Psi6*sb0*sb_z;
		tau[index] = (au0m1+(P_l/rho_bl+eps)*alpn1*u0[index])*rho_s +
		  Psi6*sb2*SQR(alpn1*u0[index])
		  - Psi6*(P_l+sb2*0.5)-Psi6*SQR(alpn1*sb0);

		set_AUX_B2(AUX,B2,index,rho_s,tau,gamma_th,f1o4pa,BxL,ByL,BzL,
			   gxxL,gxyL,gxzL,gyyL,gyzL,gzzL,mhd_st_x,mhd_st_y,mhd_st_z,
			   alpn1,B_xl,B_yl,B_zl,Psim4,gupxx,gupxy,gupxz,gupyy,gupyz,gupzz,Psi2,Psi4,Psi6);

		double st_xL = AUX.mhd_st_x;
		double st_yL = AUX.mhd_st_y;
		double st_zL = AUX.mhd_st_z;

		double betaxL = shiftx[index];
		double betayL = shifty[index];
		double betazL = shiftz[index];

		double gupxx_pL = AUX.gupxx_phys;
		double gupxy_pL = AUX.gupxy_phys;
		double gupxz_pL = AUX.gupxz_phys;
		double gupyy_pL = AUX.gupyy_phys;
		double gupyz_pL = AUX.gupyz_phys;
		double gupzz_pL = AUX.gupzz_phys;
		
		double w_oldL=0.0; // <- dummy
		int tau_stilde_fix_quartic=1; // Enable tau stilde fix for quartic solver, by default
		quartic_solver(rho_s,st_xL,st_yL,st_zL,betaxL,betayL,betazL,
			       gxxL,gxyL,gxzL,gyyL,gyzL,gzzL,
			       gupxx_pL,gupxy_pL,gupxz_pL,gupyy_pL,gupyz_pL,gupzz_pL,
			       Psi2,Psi4,Psi6,Psim6,Psim4,
			       rhobatm,tau_atm,enable_shocktest_primitive_mode,tau_stilde_fix_quartic,Symmetry,
			       neos, rho_tab, P_tab, eps_tab, k_tab, gamma_tab, gamma_th,
			       index,tau,mhd_st_x,mhd_st_y,mhd_st_z,w_oldL,h,alpha, X,Y,Z,
			       w,rho_b,P,rho,
			       Sx,Sy,Sz,Sxx,Sxy,Sxz,Syy,Syz,Szz,
			       u0,vx,vy,vz,rho_star);

		st_x[index] = mhd_st_x[index];
		st_y[index] = mhd_st_y[index];
		st_z[index] = mhd_st_z[index];
		u0l = u0[index];
		u_xll = (gxxL*(shiftxL+vx[index]) + 
			 gxyL*(shiftyL+vy[index]) + 
			 gxzL*(shiftzL+vz[index]))*Psi4*u0l;
		...
		rho_bl = rho_b[index];
		eps = h[index]-1.0-P[index]/rho_bl;


		P_l = max_val(P[index],P_tiny);
		h_l = max_val(h[index],1.0);

		quartic_solved=true;
		
		if(Psi6>25.0) tau_stilde++;
		//^^ TESTING ^^
	      } else {
		...
		if(tau_stilde_fix_enable == 1 || (tau_stilde_fix_enable == 2 && BxL==0 && ByL==0 && BzL==0)) {
		  //PRINT_TAU_STILDE_DEBUG;
		}

		if(Psi6>25.0) num_fontfixesinhoriz++;
		num_fontfixesoverall++;
		...
		if(check) {
		  ...
		  recom = true;
		  velocity_limiter(u_xl,u_yl,u_zl,index,AUX,alpn1,alpn1_inv,au0m1,enable_shocktest_primitive_mode,recom,u0,rho_s,rho_bl);

		  compute_pcold_epscold_cpp(rho_bl, P_cold, eps_cold,neos,rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
		  // vvTESTINGvv
		  P_cold = AUX.K_old*rho_bl*rho_bl;
		  eps_cold = AUX.K_old*rho_bl;
		  // ^^TESTING^^
		  P_l = P_cold;
		  eps = eps_cold;
		}
		...
		h_l = 1.0 + P_l/rho_bl + eps;
	      ...
	      /*
		SETTING PMAX'S OR PMIN'S OUTSIDE HORIZON WILL CAUSE SYSTEM TO LOSE ANGULAR MOMENTUM SPURIOUSLY!  
		This is a serious problem with, e.g., BHNS evolutions, but should affect all "moving matter" evolutions
	      */
	      if ((rho_bl < 100.0*rhobatm && enable_shocktest_primitive_mode==0) || (Psi6>15.0)) {
		double Pprev=P_l;
		double Kmin = 0.5;
		double Kmax = 10.0;
		set_pressure_floor_ceiling(P_cold,eps_cold,rho_bl,gamma_th,recom,P_l,eps,h_l,num_Pmaxfixesinhoriz,num_Pminfixesinhoriz,Psi6,Kmin,Kmax);
	      }

	      // Re-compute rho_star, mhd_st_i and tau if necessary
	      if (recom) {
		if(Psi6>25.0) recom_inhoriz++;
  ...
  printf("Font fixes Overall: %d / %d pts= %.3e\t In Horizon: %d / %d pts= %.3e Pmaxfxes: %.3e Pminfxes: %.3e recom %.3e\t tau_st %.3e\t",
	 num_fontfixesoverall,num_ptsoverall,num_fontfixesoverall/((double)num_ptsoverall+1e-10),
	 num_fontfixesinhoriz,num_ptsinhoriz,num_fontfixesinhoriz/((double)num_ptsinhoriz+1e-10),
	 num_Pmaxfixesinhoriz/((double)num_ptsinhoriz+1e-10),
	 num_Pminfixesinhoriz/((double)num_ptsinhoriz+1e-10),
	 recom_inhoriz/((double)num_ptsinhoriz+1e-10),
	 tau_stilde/((double)num_ptsinhoriz+1e-10));
  printf("%f solutions/second\n",(iend-istart)*(jend-jstart)*(kend-kstart) / ((double)mtime/1000.0));
...
void function_hybrd(int &n,double *x,double *fvec,struct auxarray &aux,int &neos,double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab) {
  ...
  //vv TESTING vv
  if(rhob<aux.rhobatm) rhob=1.00001*aux.rhobatm;
  //^^ TESTING ^^
  ...
  //vv TESTING vv
  if ((rhob < 100.0*aux.rhobatm) || (aux.Psi6>15.0)) {
    bool rec=false;
    int blah=0,blah2=0;
    double Kmin=0.85;
    double Kmax=8.0;
    set_pressure_floor_ceiling(P_cold,eps_cold,rhob,aux.gamma_th,rec,P,eps,h,blah,blah2,aux.Psi6,Kmin,Kmax);
  }
  //^^ TESTING ^^
...
void function_hybrd_font_fix(int &n,double *x,double *fvec,struct auxarray &aux,int &neos,double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab) {
  ...
  // vvTESTINGvv
  P_cold = aux.K_old*rhob*rhob;
  eps_cold = aux.K_old*rhob;
  // ^^TESTING^^
...
(int &n,double *x,struct auxarray &aux,int &neos,double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5]) {
  ...
  P_cold = aux.K_old*rhob*rhob;
  eps_cold = aux.K_old*rhob;
  dPcold_drho = 2.0*P_cold*rhob;
  depscold_drho = aux.K_old;
  // ^^TESTING^^
...
void set_pressure_floor_ceiling(double &P_cold,double &eps_cold,double &rho_bl,double &gamma_th,bool &recom,double &P_l,double &eps,double &h_l,
				int &num_Pmaxfixesinhoriz,int &num_Pminfixesinhoriz,double &Psi6,double &Kmin,double &Kmax) {
  double Pmin = Kmin*P_cold;
  double Pmax = Kmax*P_cold;
  if (P_l > Pmax) {
    recom = true;
    P_l = Pmax;
    ...
    if(Psi6>25.0) num_Pmaxfixesinhoriz++;
  }
  if (P_l < Pmin) {
    recom = true;
    P_l = Pmin;
    ...
    if(Psi6>25.0) num_Pminfixesinhoriz++;
  }
}
...
extern "C" void CCTK_FCALL CCTK_FNAME(primitive_vars_hybrid2_cpp)
  ...
  primitive_vars_hybrid2_cpp(ext,nghostzones, X, Y, Z, rho_star, tau,
			     ...
			     tau_stilde_fix_enable,tau_atm,failure_tracker,*cctkGH,ignore_ghostzones, enable_shocktest_primitive_mode, repairs_needed);
}
