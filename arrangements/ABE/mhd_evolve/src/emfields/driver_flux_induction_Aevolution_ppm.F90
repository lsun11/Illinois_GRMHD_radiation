...
subroutine flux_induction_Aevolution_ppm_step2(CCTK_ARGUMENTS)
  ...
  do k=1,cctk_lsh(3)
     do j=1,cctk_lsh(2)
        do i=1,cctk_lsh(1)
           if(isnan(Bztilde_or_Az_rhs(i,j,k))) then
              write(*,*) "Inside driver_flux_induction_Aevolution_ppm.F90 Bztilde_or_Az_rhs is nan \n"
              write(*,*) "cmax1(temp10), cmin1(MONOPOLE)= \n", temp10(i,j,k), MONOPOLE(i,j,k)         
              write(*,*) "cmax2(ftxy), cmin2(ftxz)= \n", ftxy(i,j,k), ftxz(i,j,k)
              write(*,*) "fxy, fxz, fyx, fyz, fzx, fzy = \n", fxy(i,j,k), fxz(i,j,k), fyx(i,j,k), fyz(i,j,k), fzx(i,j,k), fzy(i,j,k)
           end if
     end do
  end do
end do
...
subroutine flux_induction_Aevolution_ppm_step3(CCTK_ARGUMENTS)
  ...
  call flux_hll2d_cpp(cctkGH,cctk_lsh,ftxy, ftxz, cmax,cmin, &
    ...
    do k=1,cctk_lsh(3)
     do j=1,cctk_lsh(2)
        do i=1,cctk_lsh(1)
           if(isnan(Bxtilde_or_Ax_rhs(i,j,k))) then
              write(*,*) "Inside driver_flux_induction_Aevolution_ppm.F90 Bxtilde_or_Ax_rhs is nan"
              write(*,*) "cmax, cmin =", cmax(i,j,k), cmin(i,j,k)
           end if
        end do
     end do
  end do
...
subroutine flux_induction_Aevolution_ppm_step3p5(CCTK_ARGUMENTS)
  ...
  do k=1,cctk_lsh(3)
     do j=1,cctk_lsh(2)
        do i=1,cctk_lsh(1)
           if(isnan(Bytilde_or_Ay_rhs(i,j,k))) then
              write(*,*) "Inside driver_flux_induction_Aevolution_ppm.F90 Bytilde_or_Ay_rhs is nan"
              write(*,*) "cmax, cmin =", cmax(i,j,k), cmin(i,j,k)
           end if
        end do
     end do
