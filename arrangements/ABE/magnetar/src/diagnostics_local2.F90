...
!-----------------------------------------------------------------------------
subroutine magnetar_diagnostics_local2(CCTK_ARGUMENTS)
  ...
  if(MOD(cctk_iteration,out_every)==0) then
     ...
     if(magnetar_max_b2.gt.0.D0) then
        !$omp parallel do
        do k=1,cctk_lsh(3)
           do j=1,cctk_lsh(2)
              do i=1,cctk_lsh(1)
                 if(abs((temp8(i,j,k)-magnetar_max_b2)/magnetar_max_b2).lt.1e-12) then
...
end subroutine magnetar_diagnostics_local2
