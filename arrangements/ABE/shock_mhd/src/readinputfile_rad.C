...
extern "C" void radiationshock_readfiles(CCTK_ARGUMENTS)
{
  ...
  double *E_analy, *F_analy, *optd_shock;
  ...
  if (rad_evolve_enable==1){
    ...
    *rad_const = E_ra[0]*(rho0_ra[0]/P_ra[0])*(rho0_ra[0]/P_ra[0])*(rho0_ra[0]/P_ra[0])*(rho0_ra[0]/P_ra[0]);
    ...
    for (int i=istart; i<NS; i++){
      ...
      optd_shock[i] = rad_opacity_abs*rho0_ra[i]*x_ra[i];
      F_analy[i] = -E_ra[NS-1]*exp(-abs(optd_shock[i])*sqrt(3.0))/(2.0*sqrt(3.0));

      if (x_ra[i] > 0.0){
	E_analy[i] = E_ra[NS-1]*(1.0-0.5*exp(-abs(optd_shock[i])*sqrt(3.0))); 
    }
      else{
	E_analy[i] = E_ra[NS-1]*0.5*exp(-abs(optd_shock[i])*sqrt(3.0)) + E_ra[0];
      }
      printf("i=%d, optd_shock[i]=%e, rho0_ra[i]=%e, x_ra[i]=%e,E_analy[i]=%e, F_analy[i]=%e \n", i, optd_shock[i], rho0_ra[i], x_ra[i], E_analy[i], F_analy[i]);
    ...
    for (int i=istart; i<NS; i++){
      outputFile << x_ra[i] << " " << E_analy[i] << "  " << F_analy[i]<< "  " << optd_shock[i] << endl ;
      }
    outputFile.close();
    */
  }
