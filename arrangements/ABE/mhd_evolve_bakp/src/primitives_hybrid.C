...
void newt2_cpp(double x[],struct auxarray &aux,
	       void (*function_hybrd)(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab),
	       void (*jacobian_hybrd)
	       (int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5]),
...
*/
void function_hybrd(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab);
void function_hybrd_font_fix(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab);
void jacobian_hybrd
(int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5]);
...
void lnsrch_newt(int n, double xold[], double fold, double g[], double p[], double x[],
		 ...
		 (double *x,struct auxarray &aux,double *fvec,
		  void (*nrfuncv)(int &n,double *x,double *fvec, struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab),
		  int &n,int &neos,int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab),
		 void (*nrfuncv)
		 (int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab),
...
(int *ext,int *nghostzones, double *X, double *Y, double *Z, double *rho_star, double *tau,
 ...
 double &rad_const) {
  ...
  compute_pcold_epscold_cpp(rho_max, P_cold_dummy, eps_scal,neos, ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++)
	...
	double UUguess[5];
	   ...
	   if(isnan(tau[index])){
	     printf ("Label 1, tau is NAN. tau[index]=%e \n",  tau[index]);
	   }

           if(tau[index] < 0.0){
             printf ("Label 1, tau is negative. tau[index]=%e, set it to zero \n",  tau[index]);
	     tau[index] = 0.0;
           }
	   ...
	   if(rho_b_atm<0) {
	     compute_pcold_epscold_cpp(rhobatm, P_tiny, eps_tiny,neos, ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
	   } else {
	     compute_pcold_epscold_cpp(rhobatm, P_tiny, eps_tiny,neos, ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
	   }
	   ...
	   //if (rho_s > Tiny && Psi6<50.0) {
		 ...
		 */
	       ...
	       printf("mhd_st_y = %e\n",mhd_st_y[index]);
	       printf("mhd_st_z = %e\n",mhd_st_z[index]);
	       printf("AUX variables = %e, %e, %e, %e, %e, %e, %e, %e,%e, %e, %e, %e", AUX.rho_s,AUX.tau,AUX.gamma_th,AUX.mhd_st_x,AUX.mhd_st_y,AUX.mhd_st_z,AUX.gupxx_phys,AUX.gupxy_phys,AUX.gupxz_phys,AUX.gupyy_phys,AUX.gupyz_phys,AUX.gupzz_phys);
	     }
	     ...
	     if(newt_fvec[1] > 1.0e-6){
	     printf("after FIRST newt2_cpp, (i,j,k) = %d, %d, %d, newt_fvec[1]=%.16g,  newt_fvec[2]=%.16g, newt_fvec[3]=%.16g, newt_fvec[4]=%.16g \n", i,j,k, newt_fvec[1],  newt_fvec[2], newt_fvec[3], newt_fvec[4]);
	     }
	     ...
	     */
		 ...
		 //if(MAXITS_fontfix>=MAXITS_beforefontfix*1e4) break;
	   	...
	   	au0m1 = gijuiuj/( 1.0+sqrt(1.0+gijuiuj) );
	         ...
	         compute_pcold_epscold_cpp(rho_bl, P_cold, eps_cold,neos, ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
	     ...
	     if (enable_shocktest_primitive_mode==1) {
	       ...
	       if (isnan(T_fluidl))
		 {
		   printf("inside primtives_hybrid_cpp, T_fluid is nan, E_rad[index], rad_const, T_fluidl are %e, %e, %e, \n", E_rad[index], rad_const, T_fluidl);
		 }
	     }
	     else{
	       compute_T_fluid_shock_cpp2(E_rad[index], rad_const, T_fluidl);
	     ...
	     //	     if(isnan(T_fluidl)){printf("T_fluid is NAN!!!, i=%d, j=%d, k=%d", i,j,k);}
	   ...
	   if (E_radl < 0.0){
	     printf("inside primitives_hybrid_cpp, E_rad is negative!!!!! E_rad_denom=%e, E_rad_num=%e, au0m1=%e, tau_radl=%e, S_rad_xl=%e, S_rad_yl=%e, S_rad_zl=%e \n", E_rad_denom, E_rad_num, au0m1, tau_radl, S_rad_xl, S_rad_yl, S_rad_zl);
	   }
	   ...
	   //	   if(X[index]<1e-10||Y[index]<1e-10||Z[index]<1e-10){printf("check at the shock front!!! E_rad=%e, E_rad_denom is =%e, E_rad_num=%e, tau_radl =%e, S_rad_xl = %e, Psi6= %e, au0m1 = %e,S_rad_x,y,z = %e,%e,%e \n",E_radl, E_rad_denom, E_rad_num, tau_radl, S_rad_xl, Psi6, au0m1, S_rad_xl, S_rad_yl, S_rad_zl);}
	   if (isnan(abs(ux_l))){printf("check 3, ux_l is NAN. \n");}
	   
	   /*
	   if (E_radl <= 0.0){
	     printf("E_rad is negative or 0, which is %e, E_rad_denom is =%e, E_rad_num=%e, tau_radl =%e, S_rad_xl = %e, S_rad_yl=%e, S_rad_zl=%e, Psi6= %e, au0m1 =%e, ux_l=%e, uy_l=%e, uz_l=%e, \n",E_radl, E_rad_denom, E_rad_num, tau_radl, S_rad_xl, S_rad_yl, S_rad_zl, Psi6, au0m1, ux_l, uy_l, uz_l); 
	     printf("X=%e,Y=%e,Z=%e,shiftx=%e,shifty=%e,shiftz=%e \n", X, Y, Z, shiftx, shifty, shiftz); 
	     exit(1);}
	   */
...
void newt2_cpp(double x[],struct auxarray &aux,
	       void (*funcv1)(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab),
	       void (*fdjac1)
	       (int &n,double *x,struct auxarray &aux,int &neos,int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5]),
	       int &n,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,bool &check,
	       int *indx,double *g,double *p,double *xold,double *fvec,int &MAXITS,double &STPMX)
{
  ...
  // printf ("INSIDE newt2_cpp, before fmin_newt, fvec[1]=%e, fvec[2]=%e,fvec[3]=%e, fvec[4]=%e \n", fvec[1], fvec[2],fvec[3], fvec[4]);
 
  f=fmin_newt(x,aux,fvec,funcv1,n,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
  //printf ("INSIDE newt2_cpp, after fmin_newt, fvec[1]=%e, fvec[2]=%e,fvec[3]=%e, fvec[4]=%e \n", fvec[1], fvec[2],fvec[3], fvec[4]);
  ...
  for (its=1;its<=MAXITS;its++) {
    int np=NP;
    fdjac1(n,x,aux,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab,fvec,np,fjac);
    for (i=1;i<=n;i++) {
      for (sum=0.0,j=1;j<=n;j++) sum += fjac[j][i]*fvec[j];
      g[i]=sum;
    }
    //printf ("INSIDE newt2_cpp, after fdjac1, fvec[1]=%e, fvec[2]=%e,fvec[3]=%e, fvec[4]=%e \n", fvec[1], fvec[2],fvec[3], fvec[4]);
    /*
    for (i=1;i<=5;i++)
      for (j=1;j<=5;j++)
	if(isnan(fjac[i][j])) printf("Nan found in fjac[i][j],i=%d,j=%d",i,j);
    */
    ...
    lnsrch_newt(n,xold,fold,g,p,x,&f,stpmax,check,fmin_newt,funcv1,aux,fvec,neos,ergo_star, ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
    test=0.0;
    for (i=1;i<=n;i++)
      if (fabs(fvec[i]) > test) test=fabs(fvec[i]);
    if (test < TOLF) {
		 ...
		 int &n,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab)
{
  ...
  (*nrfuncv)(n,x,fvec,aux,neos,ergo_star, ergo_sigma, rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
		 ...
		 int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab)	    
{
  ...
  for (;;) {
    ...
    //printf("Inside lnsrch_newt!!!!! call fmin(function_hybrid!!!!!!)\n");
    *f=(*fmin)(x,aux,fvec,nrfuncv,n,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
    if (alam < alamin) {
...
void function_hybrd(int &n,double *x,double *fvec,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab) {
  ...
  if(isnan(fvec[1])||isnan(fvec[4])){
    printf("inside function_hybrd. fvec[1] =%e, fvec[4]=%e", fvec[1],fvec[4]);
  printf("FINAL: inside function_hybrd. u_x = %.16g, aux.mhd_st_x = %.16g, eps = %.16g, u0 = %.16g, rhob = %.16g, rho_s = %.16g, P = %.16g \n", u_x, aux.mhd_st_x, eps, u0, rhob, aux.rho_s, P);
  }
  /*
  if (fabs(fvec[1]) > 1.0e-4 || fabs(fvec[4]) > 1.0e-4){
    printf("at the end of function_hybrid, u_x = %.16g, aux.mhd_st_x = %.16g, aux.sti_scal_inv = %.16g, eps = %.16g, u0 = %.16g,rhob = %.16g, rho_s = %.16g, h = %.16g, P = %.16g, fvec[1] = %.16g, fvec[2] = %.16g, fvec[3] = %.16g, fvec[4] = %.16g \n ", u_x, aux.mhd_st_x, aux.sti_scal_inv, eps, u0, rhob, aux.rho_s, h, P, fvec[1], fvec[2],fvec[3], fvec[4]);
     }
  */
}
...
(int &n,double *x,struct auxarray &aux,int &neos, int &ergo_star, double &ergo_sigma, double *rho_tab,double *P_tab,double *eps_tab,double *k_tab,double *gamma_tab,double *fvec,int &np,double fjac[][5]) {
  ...
  /*
  if(isnan(fjac[4][1])){
    printf("fjac[4][1] is NAN, u0=%e, du0dux=%e, dhdux=%e, aux.u_scal_Psi2=%e, aux.tau_scal_inv=%e \n", u0, du0dux, dhdux, aux.u_scal_Psi2, aux.tau_scal_inv);
  }
