...
#pragma omp parallel for
	...
	S_rad_x_rhsL,g4ttm,rho_starL,hL,u0L,al,Psi6,b2,u0L,PL,sbtL);*/
	...
	tau_rad_rhs[index] +=  -al*Psi6*(((E_radL+P_radL)*SQR(u0L) + 2.0*F_rad0L*u0L-P_radL/al/al)*betaxL  
				+((E_radL+P_radL)*SQR(u0L)*vxL+F_rad0L*u0L*vxL+F_radxL*u0L))*alphax_f;
      }
    }
...
#pragma omp parallel for
	...
	tau_rad_rhs[index] +=  -al*Psi6*(((E_radL+P_radL)*SQR(u0L) + 2.0*F_rad0L*u0L -P_radL/al/al)*betayL
				 + ((E_radL+P_radL)*SQR(u0L)*vyL+F_rad0L*u0L*vyL+F_radyL*u0L))*alphay_f;
      }
    }
...
#pragma omp parallel for
	...
	tau_rad_rhs[index] += -al*Psi6*(((E_radL+P_radL)*SQR(u0L) + 2.0*F_rad0L*u0L -P_radL/al/al)*betazL
                                +((E_radL+P_radL)*SQR(u0L)*vzL+F_rad0L*u0L*vzL+F_radzL*u0L))*alphaz_f;
      }
    }
  }


  //Finally, add the scalar radiation term on tau_rad_rhs. Again, it is same as the term in tau_rhs but with a "-";
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
	int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	double al = 1.0 + alpha[index];
	...
	tau_rad_rhs[index] += -al*rho_starL/u0L*(rad_opacity_abs*(E_radL-rad_const*SQR(T_fluidL)*SQR(T_fluidL)) +
