...
#pragma omp parallel for
	...
	for(int i=istart;i<iend;i++) {
	  ...
	  double Fr, Fl, tau_radr, tau_radl;

	  	  	  
	  if (flux_direction==1) {
	    Fr = al*al*psi6*((E_radrL + P_radrL)*u0r*uxr + F_rad0rL*uxr + u0r*F_radxrL) + psi6*P_radrL*betax_fL;
	    Fl = al*al*psi6*((E_radlL + P_radlL)*u0l*uxl + F_rad0lL*uxl + u0l*F_radxlL) + psi6*P_radlL*betax_fL;
	  } else  if (flux_direction==2) {
	    Fr = al*al*psi6*((E_radrL + P_radrL)*u0r*uyr + F_rad0rL*uyr + u0r*F_radyrL) + psi6*P_radrL*betay_fL;
	    Fl = al*al*psi6*((E_radlL + P_radlL)*u0l*uyl + F_rad0lL*uyl + u0l*F_radylL) + psi6*P_radlL*betay_fL;
	  } else if  (flux_direction==3) {
	    Fr = al*al*psi6*((E_radrL + P_radrL)*u0r*uzr + F_rad0rL*uzr + u0r*F_radzrL) + psi6*P_radrL*betaz_fL;
	    Fl = al*al*psi6*((E_radlL + P_radlL)*u0l*uzl + F_rad0lL*uzl + u0l*F_radzlL) + psi6*P_radlL*betaz_fL;
	  }
	  
	  
	  tau_radr =  al*al*psi6*((E_radrL + P_radrL)*u0r*u0r + 2*F_rad0rL*u0r)-psi6*P_radrL;
	  tau_radl =  al*al*psi6*((E_radlL + P_radlL)*u0l*u0l + 2*F_rad0lL*u0l)-psi6*P_radlL;
	  
	  
	  if(isnan(Fr) || isnan(Fl)) {
	    printf("Inside flux_rad. BadFr!!!!!!!!!! al=%e, psi6=%e, E_radrL=%e, P_radrL=%e, F_rad0rL=%e, u0r=%e, uxr=%e, F_radxrL=%e \n", al, psi6, E_radrL, P_radrL, F_rad0rL, u0r, uxr, F_radxrL); 
	    exit(1);
	  }
	  ...
	  double u_xr, u_yr, u_zr;
          ...
          double F_rad_xl, F_rad_yl, F_rad_zl;
          double F_rad_xr, F_rad_yr, F_rad_zr;
	   ...
	   F_rad_xl = psi4*( gxx_fL*(F_radxlL+F_rad0lL*betax_fL) + gxy_fL*(F_radylL+F_rad0lL*betay_fL) +
				  gxz_fL*(F_radzlL+F_rad0lL*betaz_fL) );
	   F_rad_yl = psi4*( gxy_fL*(F_radxlL+F_rad0lL*betax_fL) + gyy_fL*(F_radylL+F_rad0lL*betay_fL) +
				  gyz_fL*(F_radzlL+F_rad0lL*betaz_fL) );
	   F_rad_zl = psi4*( gxz_fL*(F_radxlL+F_rad0lL*betax_fL) + gyz_fL*(F_radylL+F_rad0lL*betay_fL) +
				  gzz_fL*(F_radzlL+F_rad0lL*betaz_fL) );	
	 
	   F_rad_xr = psi4*( gxx_fL*(F_radxrL+F_rad0rL*betax_fL) + gxy_fL*(F_radyrL+F_rad0rL*betay_fL) +
				  gxz_fL*(F_radzrL+F_rad0rL*betaz_fL) );
           F_rad_yr = psi4*( gxy_fL*(F_radxrL+F_rad0rL*betax_fL) + gyy_fL*(F_radyrL+F_rad0rL*betay_fL) +
				  gyz_fL*(F_radzrL+F_rad0rL*betaz_fL) );
	   F_rad_zr = psi4*( gxz_fL*(F_radxrL+F_rad0rL*betax_fL) + gyz_fL*(F_radyrL+F_rad0rL*betay_fL) +
                                  gzz_fL*(F_radzrL+F_rad0rL*betaz_fL) );
	  
           if (fabs(F_rad_xr)>1.0e10||fabs(F_rad_xl)>1.0e10||fabs(F_rad_yr)>1.0e10||fabs(F_rad_yl)>1.0e10||fabs(F_rad_zr)>1.0e10||fabs(F_rad_zl)>1.0e10)
             {
	       printf ("inside flux_rad.C, F_rad_i is large, F_rad_xr, F_rad_xl,F_rad_yr, F_rad_yl,F_rad_zr, F_rad_zl are \n %e, %e, %e, %e, %e, %e", F_rad_xr, F_rad_xl,F_rad_yr, F_rad_yl,F_rad_zr, F_rad_zl );
