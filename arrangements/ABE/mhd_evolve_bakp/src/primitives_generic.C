...
#pragma omp parallel for reduction(+:failures,font_fixes,pointcount,failures_inhoriz,pointcount_inhoriz,error_int_numer,error_int_denom,pressure_cap_hit,rho_star_fix_applied) schedule(static)
  for(int k=0;k<ext[2];k++)
	...
	*/
	...
	if(rho_star[index]<=0.0) {
	  ...
	  */
	  ...
	  if(isnan(tau[index])) { printf("Check1: found nan in tau %d %d %d\n",i,j,k); }
          ...
          if(fabs(phi[index]) > 10.0) { printf("Before apply_tau_floor, phi is unreasonable. phi are %e \n", phi[index]);}
	  ...
	  //	  if(Psi6 < 1.0e-40) { printf("In prim_generic after apply tau floor, psi6 is too small to be correct. Psi6, phi are %e, %e, \n", Psi6, phi);}
	  ...
	  if(fabs(phi[index]) > 10.0) { printf("Before harm_primitives_gammalaw_lowleve, phi is unreasonable. phi are %e \n", phi[index]);}
	...
	compute_T_fluid_cpp(xL,yL,zL, num_CO, xNS1, yNS1, xNS2, xNS2, new_primitives.rho_b_new, rho_starL,
			    new_primitives.P_new, M_B, rad_T_fac, rad_T_cutoff, rad_T_pow, rad_T_floor, rhobatm, T_fluidL);
