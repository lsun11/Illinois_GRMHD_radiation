...
double *tau_rad_flux,int &Symmetry);
...
#pragma omp parallel for
  for(int k=kmin;k<kmax;k++) for(int j=jmin;j<jmax;j++) for(int i=imin;i<imax;i++) {
    ...
    if (flux_direction==1){
      if (Symmetry!=AXISYM) 
	{
	tau_rad_rhs[index] =  tau_rad_rhs[index] + (tau_rad_flux[index]-tau_rad_flux[indexip1])*ddx;
	} 
      else 
	{
	tau_rad_rhs[index] =  tau_rad_rhs[index] + (tau_rad_flux[index]-tau_rad_flux[indexip1])*ddx/X[index];
	}
    }
    else if (flux_direction==2){
      if (Symmetry!=AXISYM) 
	{
        tau_rad_rhs[index] =  tau_rad_rhs[index] + (tau_rad_flux[index]-tau_rad_flux[indexjp1])*ddy;
	} 
      else 
	{
	tau_rad_rhs[index] =  tau_rad_rhs[index] + (tau_rad_flux[index]-tau_rad_flux[indexjp1])*ddy/Y[index];
	}
    }
    else if (flux_direction==3){
      if (Symmetry!=AXISYM) 
	{
        tau_rad_rhs[index] =  tau_rad_rhs[index] + (tau_rad_flux[index]-tau_rad_flux[indexkp1])*ddz;
	}
      else 
	{
	tau_rad_rhs[index] =  tau_rad_rhs[index] + (tau_rad_flux[index]-tau_rad_flux[indexkp1])*ddz/Z[index];
	}
    }
