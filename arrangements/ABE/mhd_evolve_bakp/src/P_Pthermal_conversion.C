...
extern "C" void CCTK_FCALL CCTK_FNAME(convert_P_to_Pthermal_cpp)
...
#pragma omp parallel for
	...
	for(int i=istart;i<iend;i++) {
	  ...
	  compute_pcold_epscold_cpp(rho_i, P_cold, eps_cold,neos,ergo_star,ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
...
extern "C" void CCTK_FCALL CCTK_FNAME(convert_Pthermal_to_P_cpp)
    ...
    for(int k=kstart;k<kend;k++)
	  ...
	  double rhol_i = rho_bl[index];
          ...
          compute_pcold_epscold_cpp(rhor_i, P_cold, eps_cold,neos,
				    ergo_star, ergo_sigma,rho_tab,P_tab,eps_tab,k_tab,gamma_tab);
          Pr[index] += P_cold;
