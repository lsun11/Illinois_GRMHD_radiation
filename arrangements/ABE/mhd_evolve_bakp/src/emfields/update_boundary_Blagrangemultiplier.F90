...
subroutine update_boundary_Blagrangemultiplier(ex,fake_ex,X,Y,Z, PhysicalR, RadiusDerivative, &
  ...
  !WARNING: FOLLOWING WON'T WORK IN AXISYMMETRY.  Have modified below code to ignore jlb,jub, which is necessary in axisymmetry
  jlb = jmin
  jub = jmax - 1
...
!!!$omp parallel do
     do k = kmin, kmax - 1
        do j = jmin, jmax - 1
           ...
           !if(k==kmin .and. j==jmin+1) write(*,*) "max",facRp,Dlx,Dly,Dlz,ONE - (alpha(i,j,k) + ONE)*exp(-TWO*phi_old(i,j,k))*dT/(sqrt(gxx_old(i,j,k)*X(i,j,k)*X(i,j,k) + gyy_old(i,j,k)*Y(i,j,k)*Y(i,j,k) + gzz_old(i,j,k)*Z(i,j,k)*Z(i,j,k) + TWO*(gxy_old(i,j,k)*X(i,j,k)*Y(i,j,k) + gxz_old(i,j,k)*X(i,j,k)*Z(i,j,k) + gyz_old(i,j,k)*Y(i,j,k)*Z(i,j,k))))
        enddo
     enddo
!!!$omp end parallel do
  end if
...
!!!$omp end parallel do
  ...
  ! z = zmax
  !
  !WARNING: FOLLOWING WON'T WORK IN AXISYMMETRY
  !jub = jub + 1
...
!!!$omp parallel do
     do k = kmin, kmax-1
        do j = jmin, jmax-1
           ...
           Blagrangemultiplier_new(i,j,k)   = f_int_Blagrangemultiplier * facRp
           !if(k==kmin .and. j==jmin+1) write(*,*) "min",facRp,Dlx,Dly,Dlz,ONE - (alpha(i,j,k) + ONE)*exp(-TWO*phi_old(i,j,k))*dT/(sqrt(gxx_old(i,j,k)*X(i,j,k)*X(i,j,k) + gyy_old(i,j,k)*Y(i,j,k)*Y(i,j,k) + gzz_old(i,j,k)*Z(i,j,k)*Z(i,j,k) + TWO*(gxy_old(i,j,k)*X(i,j,k)*Y(i,j,k) + gxz_old(i,j,k)*X(i,j,k)*Z(i,j,k) + gyz_old(i,j,k)*Y(i,j,k)*Z(i,j,k))))
        enddo
     enddo
!!!$omp end parallel do
  endif
