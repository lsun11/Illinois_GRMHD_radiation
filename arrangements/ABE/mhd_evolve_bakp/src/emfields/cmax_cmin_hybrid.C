...
extern "C" void CCTK_FCALL CCTK_FNAME(compute_cmax_cmin_hybrid_cpp)
...
// cmin and cmax				    !
//--------------------------------------------------!
void compute_cmax_cmin_hybrid_cpp(const cGH *cctkGH,int *ext,double *v02r,double *v02l,double *cmax,double *cmin,double *rho_br,double *rho_bl,
				  double *Pr,double *Pl,double *vxr,double *vxl,double *vyr,double *vyl,double *vzr,double *vzl,
				  double *Bxr,double *Bxl,double *Byr,double *Byl,double *Bzr,double *Bzl,
				  double *alpha,double *betax,double *betay,double *betaz,double *phi,
				  double *gxx,double *gxy,double *gxz,double *gyy,double *gyz,double *gzz,
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++)
	...
	/*
	if(cmax[index]==0 && cmin[index]==0 && k>=3 && j>=3 && i>=3) {
	  printf("v02l00 %e %e %e %e %e %e %e %e\n",betaz[index], phi[index],gxx[index],gxy[index],gxz[index],gyy[index],gyz[index],gzz[index]);
	  printf("v02l0 %e %e %e %e %e %e\n",Bxl[index],Byl[index],Bzl[index], alpha[index],betax[index],betay[index]);
	  printf("v02l %e %e %e %e %e %e\n",rho_bl[index],Pl[index],u0[index],vxl[index],vyl[index],vzl[index]);
	  printf("cplusl: %e %e %e %e %e %e %e\n",v02l[index],u0[index],vyl[index],alpha[index],betay[index],phi[index],gupyy[index]);
	  printf("ERROR.  cmax==cmin==0! %e %e %e %e  at: %d %d %d\n",cplusl[index],cplusr[index],cminusl[index],cminusr[index],i,j,k);
	  exit(1);
	}
	*/

	/*
	  cmax[index] = max(0.0,cplusl[index],cplusr[index]);
	  cmin[index] = -min(0.0,cminusl[index],cminusr[index]);
	*/
      }
}
...
extern "C" void CCTK_FCALL CCTK_FNAME(compute_cmax_cmin_hybrid_cpp)
  (const cGH **cctkGH,int *ext,double *v02r,double *v02l,double *cmax,double *cmin,double *rho_br,double *rho_bl,
   ...
   double *cplusr,double *cminusr,double *u0,double *cplusl,double *cminusl)
{
  compute_cmax_cmin_hybrid_cpp(*cctkGH,ext,v02r,v02l,cmax,cmin,rho_br,rho_bl,
