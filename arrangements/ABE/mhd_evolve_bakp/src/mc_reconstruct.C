...
extern "C" void CCTK_FCALL CCTK_FNAME(compute_dU_everywhere)
...
extern "C" void CCTK_FCALL CCTK_FNAME(mc_slope_limiter)
  (int *flux_direction,const cGH **cctkGH,int *cctk_lsh,int *nghostzones,int *Symmetry,
   double *dU,double *nabla_U);

extern "C" void CCTK_FCALL CCTK_FNAME(compute_Ur_Ul_general)
...
extern "C" void CCTK_FCALL CCTK_FNAME(compute_Ur_Ul_SPPM)
  (int *flux_direction,const cGH **cctkGH,int *cctk_lsh,int *nghostzones,
   double *X,double *Y,double *Z,int *Symmetry,int *cell_centering_enabled,
...
#pragma omp parallel for
  for(int k=kstart;k<kend;k++) for(int j=jstart;j<jend;j++) for(int i=istart;i<iend;i++) {
    ...
    double dpU=0.0;
    double dmU=0.0;
    // Zach says: Blah, this could & should be optimized.
    /*
      if(dU[index]*dU[indexp1] > 0.0) {
      if(b*fabs(dU[indexp1])<fabs(dU[index])) {
      dpU = b*fabs(dU[indexp1]);
      } else {
      dpU = fabs(dU[index]);
      }
      if(b*fabs(dU[index]) < fabs(dU[indexp1])) {
      dmU = b*fabs(dU[index]); 
      } else {
      dmU = fabs(dU[indexp1]);
      }
