...
!
  ...
  do k = 1, cctk_lsh(3)
     do j = 1, cctk_lsh(2)
        do i = 1, cctk_lsh(1)
           ...
           if(isnan(Ax(i,j,k))==.true.) then
              write(*,*) "FIXING Ax!",i,j,k
              Ax_avg=0; numpts=0
              if(i.gt.1 .and. isnan(Ax(i-1,j,k))==.false.) then
                 Ax_avg=Ax_avg + Ax(i-1,j,k)
                 numpts=numpts + 1
              end if
              if(j.gt.1 .and. isnan(Ax(i,j-1,k))==.false.) then
                 Ax_avg=Ax_avg + Ax(i,j-1,k)
                 numpts=numpts + 1
              end if
              if(k.gt.1 .and. isnan(Ax(i,j,k-1))==.false.) then
                 Ax_avg=Ax_avg + Ax(i,j,k-1)
                 numpts=numpts + 1
              end if
              if(i.lt.cctk_lsh(1) .and. isnan(Ax(i+1,j,k))==.false.) then
                 Ax_avg=Ax_avg + Ax(i+1,j,k)
                 numpts=numpts + 1
              end if
              if(j.lt.cctk_lsh(3) .and. isnan(Ax(i,j+1,k))==.false.) then
                 Ax_avg=Ax_avg + Ax(i,j+1,k)
                 numpts=numpts + 1
              end if
              if(k.lt.cctk_lsh(3) .and. isnan(Ax(i,j,k+1))==.false.) then
                 Ax_avg=Ax_avg + Ax(i,j,k+1)
                 numpts=numpts + 1
              end if
              if(numpts==0) then
           ...
           ! ZACH SAYS: This should happen RARELY.
           if(isnan(Ay(i,j,k))==.true.) then
              write(*,*) "FIXING Ay!",i,j,k
              Ay_avg=0; numpts=0
              if(i.gt.1 .and. isnan(Ay(i-1,j,k))==.false.) then
                 Ay_avg=Ay_avg + Ay(i-1,j,k)
                 numpts=numpts + 1
              end if
              if(j.gt.1 .and. isnan(Ay(i,j-1,k))==.false.) then
                 Ay_avg=Ay_avg + Ay(i,j-1,k)
                 numpts=numpts + 1
              end if
              if(k.gt.1 .and. isnan(Ay(i,j,k-1))==.false.) then
                 Ay_avg=Ay_avg + Ay(i,j,k-1)
                 numpts=numpts + 1
              end if
              if(i.lt.cctk_lsh(1) .and. isnan(Ay(i+1,j,k))==.false.) then
                 Ay_avg=Ay_avg + Ay(i+1,j,k)
                 numpts=numpts + 1
              end if
              if(j.lt.cctk_lsh(3) .and. isnan(Ay(i,j+1,k))==.false.) then
                 Ay_avg=Ay_avg + Ay(i,j+1,k)
                 numpts=numpts + 1
              end if
              if(k.lt.cctk_lsh(3) .and. isnan(Ay(i,j,k+1))==.false.) then
                 Ay_avg=Ay_avg + Ay(i,j,k+1)
                 numpts=numpts + 1
              end if
              if(numpts==0) then
           ...
           ! ZACH SAYS: This should happen RARELY.
           if(isnan(Az(i,j,k))==.true.) then
              write(*,*) "FIXING Az!",i,j,k
              Az_avg=0; numpts=0
              if(i.gt.1 .and. isnan(Az(i-1,j,k))==.false.) then
                 Az_avg=Az_avg + Az(i-1,j,k)
                 numpts=numpts + 1
              end if
              if(j.gt.1 .and. isnan(Az(i,j-1,k))==.false.) then
                 Az_avg=Az_avg + Az(i,j-1,k)
                 numpts=numpts + 1
              end if
              if(k.gt.1 .and. isnan(Az(i,j,k-1))==.false.) then
                 Az_avg=Az_avg + Az(i,j,k-1)
                 numpts=numpts + 1
              end if
              if(i.lt.cctk_lsh(1) .and. isnan(Az(i+1,j,k))==.false.) then
                 Az_avg=Az_avg + Az(i+1,j,k)
                 numpts=numpts + 1
              end if
              if(j.lt.cctk_lsh(3) .and. isnan(Az(i,j+1,k))==.false.) then
                 Az_avg=Az_avg + Az(i,j+1,k)
                 numpts=numpts + 1
              end if
              if(k.lt.cctk_lsh(3) .and. isnan(Az(i,j,k+1))==.false.) then
                 Az_avg=Az_avg + Az(i,j,k+1)
                 numpts=numpts + 1
              end if
              if(numpts==0) then
  ...
  call apply_bfield_bc_ijkmin_onegf_onegoodface(cctk_lsh,Bxr,1,EM_BC)
  ...
  ! Apply BC on staggered \tilde{B}^i
  call apply_bfield_bc_ijkmin_onegf_onegoodface(cctk_lsh,Byr,2,EM_BC)
  ...
  ! Apply BC on staggered \tilde{B}^i
  call apply_bfield_bc_ijkmin_onegf_onegoodface(cctk_lsh,Bzr,3,EM_BC)
...
!!$  call apply_bfield_bc_ijkmin(cctk_lsh,Bxr,Byr,Bzr,EM_BC)
  ...
  do k=2,cctk_lsh(3)
     do j=2,cctk_lsh(2)
        do i=2,cctk_lsh(1)
           Bx(i,j,k) = 0.5D0 * ( (Az(i,j,k)-Az(i,j-1,k))/dY - (Ay(i,j,k)-Ay(i,j,k-1))/dZ + &
                (Az(i-1,j,k)-Az(i-1,j-1,k))/dY - (Ay(i-1,j,k)-Ay(i-1,j,k-1))/dZ )

           By(i,j,k) = 0.5D0 * ( (Ax(i,j,k)-Ax(i,j,k-1))/dZ - (Az(i,j,k)-Az(i-1,j,k))/dX + &
                (Ax(i,j-1,k)-Ax(i,j-1,k-1))/dZ - (Az(i,j-1,k)-Az(i-1,j-1,k))/dX )

           Bz(i,j,k) = 0.5D0 * ( (Ay(i,j,k)-Ay(i-1,j,k))/dX - (Ax(i,j,k)-Ax(i,j-1,k))/dY + &
                (Ay(i,j,k-1)-Ay(i-1,j,k-1))/dX - (Ax(i,j,k-1)-Ax(i,j-1,k-1))/dY)
        end do
     end do
  end do
  !$omp end parallel do
  ...
  do k=1,cctk_lsh(3)
     do j=1,cctk_lsh(2)
	   ...
	   ip1 = min(i+1,cctk_lsh(1))
           jp1 = min(j+1,cctk_lsh(2))
           kp1 = min(k+1,cctk_lsh(3))
           Bx(i,j,k) = Bx(i,j,k)*exp(-3.d0*(phi(i,j,k)+phi(ip1,j,k)))
           By(i,j,k) = By(i,j,k)*exp(-3.d0*(phi(i,j,k)+phi(i,jp1,k)))
           Bz(i,j,k) = Bz(i,j,k)*exp(-3.d0*(phi(i,j,k)+phi(i,j,kp1)))
        end do
     end do
  end do
  !$omp end parallel do
...
subroutine apply_bfield_bc_ijkmin_onegf_onegoodface(ext,gf,goodface,EM_BC)
  implicit none
  integer, dimension(3) :: ext
  integer :: i,j,k, EM_BC, goodface
  integer, parameter :: COPY = 1, FREEZE = 2, EXTRAP = 3, QUAD = 4, PLANAR = 5
  real*8, dimension(ext(1),ext(2),ext(3)) :: gf

  if (EM_BC.ne.COPY) then 
     write(*,*) 'Sorry only COPY BCs are supported at this time.'
     stop
  end if
...
end subroutine apply_bfield_bc_ijkmin_onegf_onegoodface
