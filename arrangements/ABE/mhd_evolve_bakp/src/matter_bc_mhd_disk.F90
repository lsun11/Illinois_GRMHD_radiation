  ...
  if (Symmetry == AXISYM) then
     if (have_bdry_max(1)==1) then ! do outer x boundary
		...
		E_radl,F_rad0l,F_radxl,F_radyl,F_radzl,P_radl,tau_radl, S_rad_xl, S_rad_yl, S_rad_zl,&
                ...
                neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab, gamma_th,rho_b_atm_gf(i,j,k),pfloor_gf(i,j,k),enable_HARM_energyvariable)
...
subroutine recompute_vars_disk(rho_bl,Pl,vxl,vyl,vzl,Bxl,Byl,Bzl,AUX,m,rho_sl,taul, &
		...
		E_radl,F_rad0l,F_radxl,F_radyl,F_radzl,P_radl, tau_radl,S_rad_xl, S_rad_yl, S_rad_zl,&
                sbtl,sbxl,sbyl,sbzl,rhol,Sxl,Syl,Szl,Sxxl,Sxyl,Sxzl,Syyl,Syzl,Szzl, &
                neos, ergo_star, ergo_sigma, rho_tab, P_tab, eps_tab, k_tab, gamma_tab, gamma_th,rho_b_atm,pfloor,enable_HARM_energyvariable)
  implicit none
  integer                   :: m 
  integer                   :: neos,ergo_star
  real*8, dimension(neos)   :: rho_tab,P_tab,eps_tab
  ...
  real*8                    :: E_rad_denom, E_rad_num, E_radl,F_rad0l,F_radxl,F_radyl,F_radzl,P_radl
  ...
  real*8                   :: E_x, E_y, E_z, u_x, u_y, u_z, ux_l, uy_l, uz_l
  real*8                   :: temp_rad, temp_rad1, F_rad_xl, F_rad_yl, F_rad_zl
  ...
  E_rad_denom = -(au0r1+1.0)*Psi6+ Psi6*Psi6*(2.0*(au0r1+1.0)/3.0-1.0/(au0r1+1.0)/6.0)
  E_rad_num = (Psi6/(2.0*(au0r1+1.0)) - (au0r1+1.0)) * tau_radl &
          +(u0l*shiftx+ux_l)*S_rad_xl &
          +(u0l*shifty+uy_l)*S_rad_yl &
          +(u0l*shiftz+uz_l)*S_rad_zl 

  E_radl = E_rad_num/E_rad_denom
  P_radl = E_radl/3.0

  F_rad0l = (tau_radl/Psi6- (4*(au0r1+1.0)*(au0r1+1.0) - 1.0)/3.0 * E_radl)/(2.0*(au0r1+1.0)*alpn1)

  F_radxl = ((gupxx*S_rad_xl + &
              gupxy*S_rad_yl + &
              gupxz*S_rad_zl)/Psi4)/(alpn1*Psi6*u0l)- &
             (E_radl+P_radl)*u0l*(vxl+shiftx) - &
              2.0*(F_rad0l * shiftx) - F_rad0l*vxl 
  F_radyl = ((gupxy*S_rad_xl + &
              gupyy*S_rad_yl + &
              gupyz*S_rad_zl)/Psi4)/(alpn1*Psi6*u0l)- &
             (E_radl+P_radl)*u0l*(vyl+shifty) - &
              2.0*(F_rad0l * shifty) - F_rad0l*vyl 
  F_radzl = ((gupxz*S_rad_xl + & 
              gupyz*S_rad_yl + &
              gupzz*S_rad_zl)/Psi4)/(alpn1*Psi6*u0l)- &
             (E_radl+P_radl)*u0l*(vzl+shiftz) - &
              2.0*(F_rad0l * shiftz) - F_rad0l*vzl 

  temp_rad = alpn1*u0l
  temp_rad1 = temp_rad*temp_rad*(E_radl+P_radl) - P_radl + 2*alpn1*u0l*F_rad0l

  F_rad_xl = Psi4 * (gxx * F_radxl + gxy * F_radyl + gxz * F_radzl)
  F_rad_yl = Psi4 * (gxy * F_radxl + gyy * F_radyl + gyz * F_radzl)
  F_rad_zl = Psi4 * (gxz * F_radxl + gyz * F_radyl + gzz * F_radzl)
