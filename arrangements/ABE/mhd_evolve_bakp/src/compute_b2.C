...
extern "C" void CCTK_FCALL CCTK_FNAME(mhd_evolve_compute_b2_cpp)
...
void mhd_evolve_compute_b2_cpp(const cGH *cctkGH,int *cctk_lsh, double *phi, 
...
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++)
	...
	if(b2[index]>0.1) {
	  printf("WARNING B2 TOO BIG: b2=%e,gij=%e %e %e %e %e %e, shifti=%e %e %e, u0vi=%e %e %e %e,psi6=%e,Bi=%e %e %e\n",b2[index],
		 gxx[index],
		 gxy[index],
		 gxz[index],
		 gyy[index],
...
extern "C" void CCTK_FCALL CCTK_FNAME(mhd_evolve_compute_b2_cpp)
  (const cGH **cctkGH,int *cctk_lsh, double *phi, double *lapm1, 
   ...
   double *b2)
{
  mhd_evolve_compute_b2_cpp(*cctkGH, cctk_lsh, phi, lapm1, shiftx,shifty,shiftz,
