...
!-----------------------------------------------------------------------------
! Fill in U's symmetry ghostzones
!-----------------------------------------------------------------------------
subroutine apply_U_syms(ext,cctk_nghostzones,Symmetry,X,Y,Z,U_syms,U)
  ...
  !Gotta be careful about Z(kmin) in axisymmetry, since we can't assume equatorial+axisymmetry in general:
  ! Still, there's a chance that the grid is split in such a way that (Z(1,1,1).lt.0.D0 .and. Z(1,1,cctk_nghostzones(3)+1).gt.0.D0) this case...
