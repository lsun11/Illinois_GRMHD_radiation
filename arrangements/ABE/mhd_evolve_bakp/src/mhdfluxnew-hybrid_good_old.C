...
extern "C" void CCTK_FCALL CCTK_FNAME(mhdfluxnew_hybrid)
  (int *flux_direction, const cGH **cctkGH,int *cctk_lsh,int *nghostzones,
   ...
   int *neos, int *ergo_star, double *ergo_sigma, double *rho_tab, double *P_tab, double *eps_tab, double *k_tab, double *gamma_tab,double *gamma_th, int *use_central_scheme_instead_of_hll);
...
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++) for(int j=jstart;j<jend;j++) for(int i=0;i<cctk_lsh[0];i++) {
    ...
    if (er > 1.0) {
      ...
      /*
      double giggle = psi4*(gxx_fL*SQR(vxrL + betax_fL) +
		      2.0*gxy_fL*(vxrL + betax_fL)*(vyrL + betay_fL) +
		      2.0*gxz_fL*(vxrL + betax_fL)*(vzrL + betaz_fL) +
		      gyy_fL*SQR(vyrL + betay_fL) +
		      2.0*gyz_fL*(vyrL + betay_fL)*(vzrL + betaz_fL) +
		      gzz_fL*SQR(vzrL + betaz_fL) )/al/al;
      printf("HI ER= %e, giggle= %e \tpsi4= %e\n",er,giggle,psi4);
      */
      er = FAC;
    }
    // ***************************************
    ...
    //if(i==17 && j==17 && k==4) { printf("CPCM00\t%d\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n",flux_direction,v02lL,rho_blL,PlL,dPcold_drho,eps_th,hl1); }

    double cplusr,cminusr;
    if (flux_direction==1) {
      find_cp_cm(cplusl,cminusl,v02lL,u0l,
		 vxlL,alpha_fL,betax_fL,psim4,gupxx_fL);
      //if(i==17 && j==17 && k==4) { printf("CPCM\t%d\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n",flux_direction,cminusl,v02lL,u0l,vxlL,alpha_fL); }
    ...
    //if(i==1 && j==1 && k==1) printf("cplusl\t%d\t%.15e %.15e %.15e %.15e %.15e %.15e %.15e %.15e\n",flux_direction,cplusl,v02lL,u0l,vxlL,alpha_fL,betax_fL,phi_fL,gupxx_fL);
    ...
    //if(i==1 && j==1 && k==1) printf("cmaxcmin\t%.15e %.15e %.15e\n",cminL,cminusl,cminusr);
    ...
    /*
    if(i==17 && j==17 && k==4) { printf("frhoi\t%d\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n",flux_direction,f_rho_i[index],cminusl,Fr,er,el); }    
    if(i==17 && j==17 && k==4) { printf("frhoi\t%d\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n",flux_direction,f_rho_i[index],cminusr,Fr,er,el); }
    if(i==17 && j==17 && k==4) { printf("frhoi\t%d\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n",flux_direction,f_rho_i[index],cminL,Fr,er,el); }
    */
    //if(i==49 && j==26 && k==7) printf("frhoiA\t%d\t%.15e\t%.15e\t%.15e\n",flux_direction,f_rho_i[index],rho_brL,rho_blL);
    //if(i==50 && j==26 && k==7) printf("frhoiB\t%d\t%.15e\t%.15e\t%.15e\n",flux_direction,f_rho_i[index],rho_brL,rho_blL);
    ...
    //if(i==4 && j==1 && k==1) printf("u_zl: %.15e\t%.15e\t%.15e\n%.15e\t%.15e\n",u0l*psi4,gxz_fL*(vxlL+betax_fL),vxlL,vylL,vzlL);
