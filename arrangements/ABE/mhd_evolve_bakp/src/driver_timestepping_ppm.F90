...
write (*,*) "Begin driver_tiemstepping_ppm_step1!!!!!!"
...
if (i==50.and.j==2.and.k==2) then
    write(*,*) " In driver_timestepping_ppm, checkpoint <<1>>  tau(i,j,k) is ", tau(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<1>>  tau_rhs(i,j,k) is ", tau_rhs(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<1>>  tau_rad(i,j,k) is ", tau_rad(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<1>>  tau_rad_rhs(i,j,k) is ", tau_rad_rhs(i,j,k)
end if
	end do
    end do
  end do
  ...
  do k=1, cctk_lsh(3)
     do j=1, cctk_lsh(2)
          do i=1, cctk_lsh(1)
if (i==50.and.j==2.and.k==2) then
    write(*,*) " In driver_timestepping_ppm, checkpoint <<2>>  tau(i,j,k) is ", tau(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<2>>  tau_rhs(i,j,k) is ", tau_rhs(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<2>>  tau_rad(i,j,k) is ", tau_rad(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<2>>  tau_rad_rhs(i,j,k) is ", tau_rad_rhs(i,j,k)
end if
	end do
    end do
  end do
...
subroutine mhd_timestepping_tau_curvature(CCTK_ARGUMENTS)
     ...
     do k=1,cctk_lsh(3)
     	do j=1,cctk_lsh(2)
           ...
           if(isnan(Axx(i,j,k))) then
                 write(*,*) "driver_timestepping_ppm:  Checkpoint << 1 >>, NAN FOUND!"
           end if
           end do
        end do
     end do
  ...
  do k=1, cctk_lsh(3)
     do j=1, cctk_lsh(2)
          do i=1, cctk_lsh(1)
if (i==50.and.j==2.and.k==2) then
    write(*,*) " In driver_timestepping_ppm, checkpoint <<3>>  tau_rhs(i,j,k) is ", tau_rhs(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<3>>  tau_rad_rhs(i,j,k) is ", tau_rad_rhs(i,j,k)
end if
	end do
    end do
  end do
...
!  do k=1,cctk_lsh(3)
!  write(*,*) "Inside driver_timestepping AFTER CHECK <<5>> , tau(0,0,k) is !!! k, tau(0,0,k) =", k, tau(0,0,k)
!  end do
...
!!$                         + (rho_star(i,j,k-2) - 4.D0*rho_star(i,j,k-1) + 6.D0*rho_star(i,j,k) - 4.D0*rho_star(i,j,k+1) + rho_star(i,j,k+2)) / dz)
  ...
  do k=1, cctk_lsh(3)
     do j=1, cctk_lsh(2)
          do i=1, cctk_lsh(1)
if (i==50.and.j==2.and.k==2) then
    write(*,*) " In driver_timestepping_ppm, checkpoint <<4>>  tau_rhs(i,j,k) is ", tau_rhs(i,j,k)
    write(*,*) " In driver_timestepping_ppm, checkpoint <<4>>  tau_rad_rhs(i,j,k) is ", tau_rad_rhs(i,j,k)
end if
	end do
    end do
  end do
