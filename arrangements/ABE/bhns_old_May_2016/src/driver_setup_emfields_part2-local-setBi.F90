...
subroutine BHNS_setup_emfield_part2_local_setBi(CCTK_ARGUMENTS)
    ...
    print *, "INSIDE driver_setup_emfields_part2-local-setBi.F90"
    ...
    if(CCTK_ITERATION.eq.ITERATION_TO_INSERT_MAGNETIC_FIELDS .and. ITERATION_TO_INSERT_MAGNETIC_FIELDS.eq.0) then
       ...
       if (RESET_RHO_B_ATM.eq.1) then 
          ...
          call reset_rhob_atm_f90(cctk_lsh,X,Y,Z,temp2,rho_b,P,vx,vy,vz,shiftx,shifty,bhns_B_v,atm_type,  &
               ...
               bhns_rhob_max,rhobatm_falloff_power,rhob_o_b2,bhns_R_NS,rho_b_atm,bhns_fac_atm,beta_ext,   &
               k_tab,gamma_tab,neos,x_NS_CoM_coord2,y_NS_CoM_coord2,use_new_bhns_initial_data)
...
!!$  end do
          ...
          !        call set_U_syms(Symmetry,1,Sym_Bz,U_syms)
          !        call do_symmetry_gz_one_gridfunction(cctk_lsh,X,Y,Z,Symmetry,tau,U_syms)
...
!!$!------------------------------------------------------------------------
!!$! Compute A_phi = pm^2 max( P/bhns_P_max - p_c, 0) for Sym_Bz = 1  and 
!!$!         A_phi = pm^2 * z/r * max( P/bhns_P_max - p_c, 0) for Sym_Bz = -1.
...
!!$!------------------------------------------------------------------------
!!$subroutine BHNS_compute_Aphi(A_phi,ext,X,Y,Z,r,PhysR,P,bhns_P_max,p_c,x_NS_CoM_coord,y_NS_CoM_coord,Sym_Bz)
!!$  implicit none
!!$  integer, dimension(3)                    :: ext
!!$  real*8, dimension(ext(1),ext(2),ext(3))  :: A_phi,X,Y,Z,P,r,PhysR
!!$  real*8                                   :: bhns_P_max,p_c,Sym_Bz,pm2,x_NS_CoM_coord,y_NS_CoM_coord
...
!!$
!!$	   A_phi(i,j,k) = pm2*max(P(i,j,k)/bhns_P_max - p_c, 0.d0)
...
!!$end subroutine BHNS_compute_Aphi
  ...
  subroutine BHNS_compute_Aphi(ext,X,Y,Z,PhysR,P,A_phi,Ax,Ay,Az,           & 
       ...
       betam1,p_c,x_NS_CoM_coord,y_NS_CoM_coord,bhns_P_max,Sym_Bz,         & 
       enable_trace_field_line,constrained_transport_scheme,em_field_type, &
       angle_to_tilt_magnetic_fields,Aphi_power,a0,I0,two_ns,              &
       x_NS_CoM_coord2,y_NS_CoM_coord2,bhns_P_max2,unequalmass, &
       betam1_2,a02,I02)
    ...
    print *,"two_nw=",two_ns,unequalmass
             ...
             if (constrained_transport_scheme==3) then 
                ...
                if (em_field_type.eq.0) then
                   if(two_ns == 3) then
                      ...
                      if (unequalmass.eq.1) then
                         ! Pick up the  star at the left (x<0)
                         ! which has the smaller pressure

                         if ((bhns_P_max2.lt.bhns_P_max)) then   
                            Aphi_s = pomega2 * fac2 * max(P_s/bhns_P_max2 - p_c, 0.d0)**Aphi_power
                            if (Sym_Bz .lt. 0.d0) Aphi_s = Aphi_s*zp/sqrt(pomega2)
                            Ax(i,j,k) = -(yp-y_NS_CoM_coord)/pomega2*Aphi_s
                         end if
                      end if
                   ...
                   if (unequalmass.eq.1) then
                      ...
                      if (bhns_P_max2.le.bhns_P_max) then  
                         call compute_Aphi_bhns_current_loop_app(xp,yp,zp,x_NS_CoM_coord,y_NS_CoM_coord,a02,I02,Aphi_s)
                         if (Sym_Bz .lt. 0.d0) Aphi_s = Aphi_s*zp/sqrt(pomega2)
                         Ax(i,j,k) = -(yp-y_NS_CoM_coord)/pomega2*Aphi_s
                      end if
                   end if
                ...
                if(angle_to_tilt_magnetic_fields.gt.0.D0) then
                   ! Rotate backwards to get x_before_tilt, then recompute Aphi_s
                   costheta = cos(-angle_to_tilt_magnetic_fields)
                   sintheta = sin(-angle_to_tilt_magnetic_fields)
                   x_before_tilt = costheta*(xp-x_NS_CoM_coord) + sintheta*zp
                   
                   ! Note that x_before_tilt shifts by x_NS_CoM_coord already:
                   pomega2 = x_before_tilt**2 + (yp-y_NS_CoM_coord)**2 + hdX*1.d-13
                   ! We assume that the star is spherical, so the pressure (P_s) depends only on the radius:
                   !    Otherwise, we would need to use the pressure at the coords (x_before_tilt,yp,zp)
                   if (em_field_type.eq.0) then
                      Aphi_s = pomega2 * fac * max(P_s/bhns_P_max - p_c, 0.d0)**Aphi_power
                   else if (em_field_type.eq.3) then 
                      print *,"unequal NSNS systems are not supported with tilted B-field"
                      call compute_Aphi_bhns_current_loop_app(xp,yp,zp,x_NS_CoM_coord,y_NS_CoM_coord,a0,I0,Aphi_s)
                   end if
                   
                   ! Get Ax & Az on the same staggered point as Ax. We don't need Ay.
                   Ax_temp = -(yp-y_NS_CoM_coord)/pomega2*Aphi_s
                   Az_temp = 0.d0 
                   
                   ! Now apply the rotation matrix to set Ax, rotating about the y-axis:
                   costheta = cos(angle_to_tilt_magnetic_fields)
                   sintheta = sin(angle_to_tilt_magnetic_fields)
                   Ax(i,j,k) = costheta*Ax_temp + sintheta*Az_temp
                end if
                ...
                if (em_field_type.eq.0) then
                   ...
                   if (unequalmass.eq.1) then
                      ...
                      ! choose the star at the left (x<0)
                      if (bhns_P_max2.le.bhns_P_max) then  
                         call compute_Aphi_bhns_current_loop_app(xp,yp,zp,x_NS_CoM_coord,y_NS_CoM_coord,a02,I02,Aphi_s)
                         if (Sym_Bz .lt. 0.d0) Aphi_s = Aphi_s*zp/sqrt(pomega2)
                         Ay(i,j,k) = (xp-x_NS_CoM_coord)/pomega2*Aphi_s
                ...
                ! Note that x_before_tilt accounts for x_NS_CoM_coord already:
                   Ay(i,j,k) = (x_before_tilt)/pomega2*Aphi_s
                end if
                ...
                if(angle_to_tilt_magnetic_fields.gt.0.D0) then 
                   if(i==1 .and. j==1 .and. k==1) then
                      write(*,*) "TILTED MAGNETIC FIELDS!"
                   end if
                   
                   ! Notice that Az is zero if we don't rotate the magnetic fields.
                   !   In the case that we actually do rotate them, we need to 
                   !   compute Az from scratch, as follows:
                   xp = xp_s
                   yp = yp_s
                   zp = Z(1,1,k)
                   ...
                   ! Note that x_before_tilt shifts by x_NS_CoM_coord already:
                   pomega2 = x_before_tilt**2 + (yp-y_NS_CoM_coord)**2 + hdX*1.d-13
                   ! We assume that the star is spherical, so the pressure depends only on the radius:
                   !    Otherwise, we would need to use the pressure at the coords (x_before_tilt,yp,zp)
                   P_s = 0.25d0*( P(i,j,k) + P(ip1,j,k) + P(i,jp1,k) + P(ip1,jp1,k) )
                   ...
                   ! Get Ax & Az on the same staggered point as Az. We don't need Ay.
                   Ax_temp = -(yp-y_NS_CoM_coord)/pomega2*Aphi_s
                   Az_temp = 0.d0 
                   
                   ! Now apply the rotation matrix to set Az, rotating about the y-axis:
                   costheta = cos(angle_to_tilt_magnetic_fields)
                   sintheta = sin(angle_to_tilt_magnetic_fields)
                   Az(i,j,k) = -sintheta*Ax_temp + costheta*Az_temp
                end if
                ...
                if (enable_trace_field_line==1) then 
                   ...
                   if (em_field_type.eq.0) then
                      ...
                      if (unequalmass.eq.1) then
                         ! choose the star at the left (x<0)
                         if (bhns_P_max2.le.bhns_P_max) then  
                            Aphi_s = pomega2 * fac2 * max((P(i,j,k)/bhns_P_max2 - p_c), 0.d0)**Aphi_power
                         end if
                      end if
                   else if (em_field_type.eq.3) then 
                      call compute_Aphi_bhns_current_loop_app(xp,yp,zp,x_NS_CoM_coord,y_NS_CoM_coord,a0,I0,Aphi_s)
