...
// $Header: /numrelcvs/AEIThorns/AHFinderDirect/src/driver/find_horizons.cc,v 1.50.2.20 2008/02/12 00:16:50 schnetter Exp $
//
...
if      (CCTK_Equals(metric_type, "physical"))
   ...
   {
     //Zach's modification:
     cgi.use_Cactus_conformal_metric = true;
     //cgi.use_Cactus_conformal_metric = (*conformal_state > 0);
     //printf("HELLO! %d\n",cgi.use_Cactus_conformal_metric);
     //exit(1);
   }
	...
	if (AH_data.found_flag)
           then {
                ...
                if (AH_data.really_initial_find_flag
                   ...
                   then {
			// ************
		        //ADDED BY YUK TUNG (PART 1 OF 2), SO THAT INITIAL GUESS CAN BE RESET IN CASE THE AH FINDER GETS LOST.  THIS IMPROVES AHFD'S BEHAVIOR:
			...
			// ************
		        //ADDED BY YUK TUNG (PART 2 OF 2), SO THAT INITIAL GUESS CAN BE RESET IN CASE THE AH FINDER GETS LOST.  THIS IMPROVES AHFD'S BEHAVIOR:
			// Reset the patch center
			ps.origin_x(origin_x[hn]);
                        ps.origin_y(origin_y[hn]);
                        ps.origin_z(origin_z[hn]);
			// ************
                        if (verbose_info.print_algorithm_highlights) {
