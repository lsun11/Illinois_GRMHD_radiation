/*This code handles prolongation of Ay (y-comp of vec. potential),
  where array element (i,j,k) corresponds to points on the 
  semi-staggered grid [i+1/2,j,k+1/2].
...
#include "coeffs_prolong.h"
  ...
  l8000:
    ...
    // Interpolate to (is-1/4, js, ks-1/4) 
    dst[DSTIND3(id,jd,kd)] = typeprops<T>::fromreal (0);
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) {
	  dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[1][jj]*src[SRCIND3(is-1+(ii-1),js,ks-1+(jj-1))];
        }
    }
    if(ORDER==3) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[4][jj]*coeff[1][kk]*src[SRCIND3(is-2+(ii-1),js-1+(jj-1),ks-2+(kk-1))];
          }
    }
    if(ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[4][jj]*coeff[1][kk]*src[SRCIND3(is-2+(ii-1),js-2+(jj-1),ks-2+(kk-1))];
          }
    }
  ...
  l8001:
    ...
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) {
	  dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[1][jj]*src[SRCIND3(is-1+(ii-1),js,ks-1+(jj-1))];
        }
    }
    if(ORDER==3) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[4][jj]*coeff[1][kk]*src[SRCIND3(is-2+(ii-1)+(4-ORDER),js-1+(jj-1),ks-2+(kk-1))];
          }
    }
    if(ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[4][jj]*coeff[1][kk]*src[SRCIND3(is-2+(ii-1)+(4-ORDER),js-2+(jj-1),ks-2+(kk-1))];
          }
    }
  ...
  l8010:
    ...
    // Interpolate to (is-1/4,js+1/2,ks-1/4)
    dst[DSTIND3(id,jd,kd)] = typeprops<T>::fromreal (0);
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[3][jj]*coeff[1][kk]*src[SRCIND3(is-1+(ii-1),js-1+(jj-1),ks-1+(kk-1))];
	  }
    }
    if(ORDER==3 || ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[3][jj]*coeff[1][kk]*src[SRCIND3(is-2+(ii-1),js-2+(jj-1)+(4-ORDER),ks-2+(kk-1))];
	  }
    }
  ...
  l8011:
    ...
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[3][jj]*coeff[1][kk]*src[SRCIND3(is-1+(ii-1),js-1+(jj-1),ks-1+(kk-1))];
	  }
    }
    if(ORDER==3 || ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[3][jj]*coeff[1][kk]*src[SRCIND3(is-2+(ii-1)+(4-ORDER),js-2+(jj-1)+(4-ORDER),ks-2+(kk-1))];
	  }
    }
  ...
  l8100:
    ...
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) {
	  dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[2][jj]*src[SRCIND3(is-1+(ii-1),js,ks-1+(jj-1))];
        }
    }
    if(ORDER==3) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[4][jj]*coeff[2][kk]*src[SRCIND3(is-2+(ii-1),js-1+(jj-1),ks-2+(kk-1)+(4-ORDER))];
          }
    }
    if(ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[4][jj]*coeff[2][kk]*src[SRCIND3(is-2+(ii-1),js-2+(jj-1),ks-2+(kk-1)+(4-ORDER))];
          }
    }
  ...
  l8101:
    ...
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) {
	  dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[2][jj]*src[SRCIND3(is-1+(ii-1),js,ks-1+(jj-1))];
        }
    }
    if(ORDER==3) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[4][jj]*coeff[2][kk]*src[SRCIND3(is-2+(ii-1)+(4-ORDER),js-1+(jj-1),ks-2+(kk-1)+(4-ORDER))];
          }
    }
    if(ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
            dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[4][jj]*coeff[2][kk]*src[SRCIND3(is-2+(ii-1)+(4-ORDER),js-2+(jj-1),ks-2+(kk-1)+(4-ORDER))];
          }
    }
  ...
  l8110:
    ...
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[3][jj]*coeff[2][kk]*src[SRCIND3(is-1+(ii-1),js-1+(jj-1),ks-1+(kk-1))];
	  }
    }
    if(ORDER==3 || ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[1][ii]*coeff[3][jj]*coeff[2][kk]*src[SRCIND3(is-2+(ii-1),js-2+(jj-1)+(4-ORDER),ks-2+(kk-1)+(4-ORDER))];
	  }
    }
  ...
  l8111:
    ...
    if(ORDER<=2) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[3][jj]*coeff[2][kk]*src[SRCIND3(is-1+(ii-1),js-1+(jj-1),ks-1+(kk-1))];
	  }
    }
    if(ORDER==3 || ORDER==4) {
      for(int ii=1;ii<=ORDER+1;ii++) for(int jj=1;jj<=ORDER+1;jj++) for(int kk=1;kk<=ORDER+1;kk++) {
	    dst[DSTIND3(id,jd,kd)] += coeff[2][ii]*coeff[3][jj]*coeff[2][kk]*src[SRCIND3(is-2+(ii-1)+(4-ORDER),js-2+(jj-1)+(4-ORDER),ks-2+(kk-1)+(4-ORDER))];
	  }
    }
