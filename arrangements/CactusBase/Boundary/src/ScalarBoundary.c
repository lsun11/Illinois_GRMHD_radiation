 /*@@
   ...
   @endhistory
   @version   $Id: ScalarBoundary.c,v 1.42 2006/10/05 17:05:57 schnetter Exp $
 @@*/
...
static const char *rcsid = "$Header: /cactusdevcvs/CactusBase/Boundary/src/ScalarBoundary.c,v 1.42 2006/10/05 17:05:57 schnetter Exp $";
  ...
  for (i=0; i<num_vars; i+=j) {
    ...
    if (faces[i] != CCTK_ALL_FACES)
    {
      CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
                 ...
                 "Applying Scalar bcs to all (external) faces.", (int)faces[i],
                 CCTK_VarName(vars[i]));
    }
