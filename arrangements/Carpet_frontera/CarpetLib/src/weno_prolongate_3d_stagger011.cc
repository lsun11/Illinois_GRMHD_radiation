...
#define DSTIND3(i,j,k)                          \
                        ...
                        ibbox3 const & restrict regbbox)
  {
    ...
    // Ax (the gridfunction we are prolongating) is defined on the semi-staggered grid (i,j+1/2,k+1/2)
    //
    // Interpolation are performed using 5th order WENO scheme to points i-1/4 (staggered coarse grid _array index_ i), 
    // i+1/4, and i+1/2, using information i-2,i-1,i,i+1,i+2.
   ...
   l8000:
    ...
    for (int kk=0; kk<5; kk++) {
        for (int jj=0; jj<5; jj++) {
 	    f1dj[jj] = src[SRCIND3(is,js+jj-2,ks+kk-2)];
        }
        f1dk[kk] = weno_5pt(f1dj,-0.25,0.0,1.0);
    }
    dst[DSTIND3(id,jd,kd)] = weno_5pt(f1dk,-0.25,0.0,1.0);
...
//printf("l8111: %d %d %d %d %d %d %e %e\n",id,jd,kd,is,js,ks,dst[DSTIND3(id,jd,kd)],src[SRCIND3(is,js,ks)]);
  ...
  template <>
  void
  prolongate_3d_stagger011 (CCTK_COMPLEX8 const * restrict const src,
                        ivect3 const & restrict srcext,
                        ...
                        ibbox3 const & restrict regbbox)
  {
    CCTK_WARN (CCTK_WARN_ABORT, "STAGGER011 for complex numbers is not supported");
  }
#endif
  ...
  template <>
  void
  prolongate_3d_stagger011 (CCTK_COMPLEX16 const * restrict const src,
                        ivect3 const & restrict srcext,
  ...
  template <>
  void
  prolongate_3d_stagger011 (CCTK_COMPLEX32 const * restrict const src,
                        ivect3 const & restrict srcext,
