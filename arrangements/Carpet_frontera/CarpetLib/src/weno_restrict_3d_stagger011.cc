...
#define DSTIND3(i,j,k)                                  \
      ...
      for (int j=0; j<regjext; ++j) {
        for (int i=0; i<regiext; ++i) {
	      ...
	      for (int jj=0; jj<5; jj++) {
	          f1dj[jj] = src [SRCIND3(2*i, 2*j+jj-2, 2*k+kk-2)];
	      }
	      f1dk[kk] = weno_5pt(f1dj,0.5,0.0,1.0);
	  }
	  dst [DSTIND3(i, j, k)] = weno_5pt(f1dk,0.5,0.0,1.0);
        }
      }
    }
  }
  ...
  template <>
  void
  restrict_3d_stagger011 (CCTK_COMPLEX8 const * restrict const src,
                   ivect3 const & restrict srcext,
  ...
  template <>
  void
  restrict_3d_stagger011 (CCTK_COMPLEX16 const * restrict const src,
                   ivect3 const & restrict srcext,
  ...
  template <>
  void
  restrict_3d_stagger011 (CCTK_COMPLEX32 const * restrict const src,
                   ivect3 const & restrict srcext,
