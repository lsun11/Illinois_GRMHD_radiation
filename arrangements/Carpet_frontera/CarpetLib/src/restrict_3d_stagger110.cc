/*
  Restriction (fine -> coarse) for Az (z-comp of vec. potential).
  where array element (2*i+1/2,2*j+1/2,2*k) corresponds to array
...
#pragma omp parallel for
    for (int k=0; k<regkext; ++k) {
	  ...
	  /*
	    dst [DSTIND3(i, j, k)] =
	    f0*f0 * src [SRCIND3(2*i-1, 2*j-1, 2*k)]
            + f0*f1 * src [SRCIND3(2*i-1, 2*j  , 2*k)]
            + f0*f2 * src [SRCIND3(2*i-1, 2*j+1, 2*k)]
            + f0*f3 * src [SRCIND3(2*i-1, 2*j+2, 2*k)]
            + f1*f0 * src [SRCIND3(2*i  , 2*j-1, 2*k)]
            + f1*f1 * src [SRCIND3(2*i  , 2*j  , 2*k)]
            + f1*f2 * src [SRCIND3(2*i  , 2*j+1, 2*k)]
            + f1*f3 * src [SRCIND3(2*i  , 2*j+2, 2*k)]
            + f2*f0 * src [SRCIND3(2*i+1, 2*j-1, 2*k)] 
            + f2*f1 * src [SRCIND3(2*i+1, 2*j  , 2*k)]
            + f2*f2 * src [SRCIND3(2*i+1, 2*j+1, 2*k)]
            + f2*f3 * src [SRCIND3(2*i+1, 2*j+2, 2*k)]
            + f3*f0 * src [SRCIND3(2*i+2, 2*j-1, 2*k)] 
            + f3*f1 * src [SRCIND3(2*i+2, 2*j  , 2*k)]
            + f3*f2 * src [SRCIND3(2*i+2, 2*j+1, 2*k)]
            + f3*f3 * src [SRCIND3(2*i+2, 2*j+2, 2*k)];
	  */
	  ...
	  /*
	    ...
	    + f2*f2 * src [SRCIND3(2*i+1 , 2*j+1, 2*k)];
	  */
	  /* DON'T USE THIS ONE.  IT'S NOT CONSISTENT WITH THE f0,f1,f2 ABOVE!
	     f0*f0 * src [SRCIND3(2*i-2 , 2*j-2, 2*k)]
	     + f0*f1 * src [SRCIND3(2*i-2 , 2*j-1, 2*k)]
	     + f0*f2 * src [SRCIND3(2*i-2 , 2*j  , 2*k)]
	     + f1*f0 * src [SRCIND3(2*i -1, 2*j-2, 2*k)]
	     + f1*f1 * src [SRCIND3(2*i -1, 2*j-1, 2*k)]
	     + f1*f2 * src [SRCIND3(2*i -1, 2*j  , 2*k)]
	     + f2*f0 * src [SRCIND3(2*i   , 2*j-2, 2*k)] 
	     + f2*f1 * src [SRCIND3(2*i   , 2*j-1, 2*k)]
	     + f2*f2 * src [SRCIND3(2*i   , 2*j  , 2*k)];
	  */
        }
      }
    }
