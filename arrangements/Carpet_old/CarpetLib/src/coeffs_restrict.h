// We are interpolating to point i-1/2 (a coarse gridpoint) using staggered fine grid _array index_ i.
//
// Interpolation coefficients are set by solving the following equation for (a,b,c)=(f11,f12,f13):
//  THE FOLLOWING DESCRIPTION IS ONLY CORRECT FOR INTERPOLATING TO STAGGERED GRIDPOINTS.
//  PLEASE SEE THE MATHEMATICA WORKSHEETS (.math files) IN THIS DIRECTORY FOR THE ACTUAL ALGORITHM.
...
/*
      double f0 = -0.125; // 2*i-2 on fine grid (there is a +1 offset below due to the staggering)
      double f1 = 0.75;   // 2*i-1 on fine grid
      double f2 = 0.375;  // 2*i   on fine grid
*/

/*
      double f0 = -1.0/16.0; // 2*i-2 on fine grid (there is a +1 offset below due to the staggering)
