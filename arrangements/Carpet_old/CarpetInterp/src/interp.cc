...
#pragma omp critical
    ...
    #pragma omp parallel for
    for (int n = 0; n < N_interp_points; n++) {
      int const idx = indices[n];
      assert ((idx+1) * ndims <= coords_buffer.size());
      for (int d = 0; d < N_dims; d++) {
...
#pragma omp parallel for
      for (int n = 0; n < N_points_local; n++) {
        source_map[n] = static_cast<int>(coords_buffer[ndims*n + N_dims]);
      }
    } else {
...
#pragma omp parallel for
        for (int n = 0; n < homecnts.AT(c); n++) {
          for (int d = 0; d < N_dims; d++) {
            tmp.AT(d*homecnts[c] + n + N_dims*offset) =
