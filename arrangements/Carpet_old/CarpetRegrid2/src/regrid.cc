                        ...
                        CCTK_INT              const force)
  {
    ...
    if (force) {
      ...
      if (regrid_every == -1) {
        ...
        do_recompose =  (cctk_iteration == 0 or
			 (cctk_iteration > 1 and 
			 (cctk_iteration - 1) % regrid_every == 0 and 
			 (cctk_iteration > * last_iteration or
			   (cctk_iteration == * last_iteration and
			    Carpet::map > * last_map))));
	CCTK_VInfo (CCTK_THORNSTRING, "regrid_every =%d,cctk_iteration is %d, do_recompose is %d", regrid_every, cctk_iteration, do_recompose);
	/*
          (cctk_iteration == 0 or
                            ...
                            CCTK_INT              const force)
  {
    ...
    if (force) {
      ...
      if (regrid_every == -1) {
        ...
        do_recompose =
	  (cctk_iteration == 0 or
	   (cctk_iteration > 1 and                                                                                                                         (cctk_iteration - 1) % regrid_every == 0 and                                                                                                    cctk_iteration > * last_iteration)); 
